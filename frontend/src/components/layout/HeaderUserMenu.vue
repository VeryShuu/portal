...
// Пока настройки не загружены — показываем (true), после загрузки скрываем
// при выключенном модуле, чтобы пункт не вёл в пустое действие.
const showReplayTour = computed(
  () => !onboardingSettings.loaded || onboardingSettings.onboardingEnabled,
)

const userMenuOptions = computed(() => [
  { label: t('nav.profile'), key: 'profile' },
  { type: 'divider', key: 'd1' },
  ...(showReplayTour.value
    ? [
