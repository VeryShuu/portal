...
import { useParticipantAbsenceSync } from './useParticipantAbsenceSync'
...
export function useMeetingFormState(props: Readonly<MeetingFormProps>, emit: MeetingFormEmit) {
  ...
  function makeForm(): FormState {
    if (props.booking) {
      return {
        ...
        // Копируем каждый участника, а не только массив: бейджи отсутствия
        // обновляются in-place (useParticipantAbsenceSync) и не должны мутировать
        // объекты бронирования из кэша TanStack Query.
        invited_users: props.booking.invited_users.map(u => ({ ...u })),
        recurrence: null,
        apply_to: 'this',
  ...
  // Бейджи отсутствия — на дату встречи (как в сводке), а не «на сегодня».
  useParticipantAbsenceSync(form, () => props.show)
