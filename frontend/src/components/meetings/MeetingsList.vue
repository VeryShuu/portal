      ...
      <div class="booking-detail">
        ...
        >
          ...
          <div class="booking-detail__participants">
            ...
            >
              <span class="booking-detail__participant-main">
                <n-tooltip
                  v-if="!showEmails"
                  trigger="hover"
                  placement="top-start"
                ...
                <span
                  v-else
                  class="booking-detail__participant-name"
                >{{ u.full_name }}</span>
                <span
                  v-if="u.rsvp"
                  class="booking-detail__participant-rsvp"
                  :class="`rsvp--${u.rsvp.status}`"
                  :title="t(`meetings.rsvp.${u.rsvp.status}`)"
                >{{ rsvpIcon(u.rsvp.status) }}</span>
              </span>
              <a
                v-if="showEmails"
...
/** Символ RSVP-отметки: ответ на приглашение из почтового клиента (расшифровка — в title). */
function rsvpIcon(status: 'accepted' | 'declined' | 'tentative'): string {
...
/* Строка «ФИО + RSVP-отметка»: отметка рядом с именем, а не отдельной строкой. */
.booking-detail__participant-main {
  display: flex;
  align-items: center;
  gap: 5px;
  min-width: 0;
...
/* RSVP-отметка участника: компактный цветной символ (галочка/минус/±), текст — в title. */
.booking-detail__participant-rsvp {
  display: inline-flex;
  align-items: center;
...
.booking-detail__participant-rsvp.rsvp--accepted {
  color: #15803d;
  background: rgba(34, 197, 94, 0.12);
}
.booking-detail__participant-rsvp.rsvp--declined {
  color: #b91c1c;
  background: rgba(239, 68, 68, 0.12);
}
.booking-detail__participant-rsvp.rsvp--tentative {
  color: #b45309;
  background: rgba(245, 158, 11, 0.12);
}
