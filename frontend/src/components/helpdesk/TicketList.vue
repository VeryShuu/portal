...
/* На мобильном строки — двухстрочные карточки (TicketListItem, media ≤720px),
   а шапка остаётся десктоп-гридом с фиксированными ширинами (~908px):
   «Тема» сжимается до 0, хвост колонок обрезается overflow:hidden, шапка
   не соответствует карточкам под ней. Карточки самодостаточны (номер/
   статус/тема/дата) — шапку и «шестерёнку» настройки колонок (десктоп-фича)
   прячем. Сортировка на мобильном — дефолтная (клики по скрытой шапке
   недоступны; мобильная сортировка — отдельная задача, если понадобится). */
@media (max-width: 720px) {
  .ticket-table__head,
  .ticket-table__settings {
    display: none;
  }
  /* «Шестерёнки» нет — строкам не нужен компенсирующий отступ справа
     (перебивает 38px выше; тот же селектор позже по каскаду). */
  .ticket-table--has-settings .ticket-table__body .ticket-row {
    padding-right: 14px;
  }
}
