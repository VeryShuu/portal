<template>
  <HtmlImageLightbox class="ticket-chat">
    <div
    ...
    >
      ...
      >
        ...
        <n-alert
          v-if="agentMode && msg.attachment_warnings?.length"
          type="warning"
          :show-icon="false"
          class="chat-bubble__attachment-warnings"
        >
          <div
            v-for="warning in msg.attachment_warnings"
            :key="warning"
          >
            {{ warning }}
          </div>
...
import { NAlert, NTag, NIcon, NAvatar } from 'naive-ui'
...
function attachmentUrl(att: HelpdeskMessage['attachments'][number]): string {
  return att.download_url ?? helpdeskAttachmentUrl(att.id)
}
...
.chat-bubble__attachment-warnings {
  margin-top: 10px;
  font-size: 13px;
}
/* Картинки в теле сообщения: ограничены шириной пузыря. Cursor zoom-in и
   полноэкранный lightbox — в общем HtmlImageLightbox. */
.chat-bubble__body :deep(img) {
  max-width: 100%;
  height: auto;
  border-radius: 6px;
