...
interface AvatarUser {
  ...
  // Фокальная точка + зум (миграция 096): как у обложек новостей —
  // object-position + scale, файл не режется.
...
/** Стиль фокала для <img> внутри n-avatar (object-position + scale). */
const imgProps = computed<ImgHTMLAttributes>(() => ({
  style: focalImageStyle(
    props.user.avatar_focal_x,
    props.user.avatar_focal_y,
    props.user.avatar_focal_zoom,
  ),
}))
