...
const props = withDefaults(defineProps<{
  /** Prevent a preview click from triggering a parent interactive control. */
  stopImageClick?: boolean
}>(), {
  stopImageClick: false,
})
...
function onClick(e: MouseEvent) {
  ...
  const previewTarget = target.closest<HTMLElement>('[data-lightbox-src], img')
  if (!previewTarget) return
  const img = previewTarget instanceof HTMLImageElement ? previewTarget : null
  const imgSrc = previewTarget.dataset.lightboxSrc || img?.getAttribute('src') || img?.currentSrc
  if (!imgSrc) return
  e.preventDefault()
  if (props.stopImageClick) e.stopPropagation()
  src.value = imgSrc
  alt.value = previewTarget.dataset.lightboxAlt || img?.getAttribute('alt') || ''
  open.value = true
}
</script>
