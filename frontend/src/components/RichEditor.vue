...
// Список видео-хостингов — runtime-настройка video_iframe_origins (Admin UI →
// System), в компоненты попадает как bootstrap.allowed_iframe_origins.
// Расширение TipTap конфигурируется один раз при создании редактора (к этому
// моменту bootstrap уже загружен при старте приложения); диалог читает список
// на момент вставки — он всегда свежий.
const allowedIframeOrigins = () =>
  brandingStore.settings.allowed_iframe_origins ?? DEFAULT_VIDEO_IFRAME_ORIGINS

const editor = useEditor({
  content: props.modelValue,
  extensions: buildEditorExtensions(props.placeholder ?? '', allowedIframeOrigins()),
  onUpdate({ editor }) {
...
const { showVideoDialog, videoUrl, insertVideo } = useEditorVideoDialog(editor, allowedIframeOrigins)
