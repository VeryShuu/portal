<template>
  <div class="step-item">
    ...
    <div
      v-if="seenCount !== null && totalUsers !== null && step.id"
      class="step-item__seen-hint"
    >
      {{ t('admin.modules.onboarding.stepSeenCount', { seen: seenCount, total: totalUsers }) }}
      <template v-if="step.is_new">
        · {{ t('admin.modules.onboarding.stepUnseenCount', { count: Math.max(totalUsers - seenCount, 0) }) }}
      </template>
    </div>
  </div>
</template>
...
defineProps<{
  ...
  /** Сколько сотрудников видели этот шаг (null — статистика не загружена). */
