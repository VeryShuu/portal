...
/** Агрегаты прохождения экскурса (GET /admin/system/settings/onboarding/stats). */
export interface OnboardingStats {
  total_users: number
  completed: number
  completed_via_finished: number
  completed_via_skipped: number
  seen_step_counts: Record<string, number>
}
...
export function useOnboardingDraft() {
  ...
  const stats = ref<OnboardingStats | null>(null)
  const statsLoading = ref(false)

  async function loadStats(): Promise<void> {
    statsLoading.value = true
    try {
      stats.value = await api<OnboardingStats>('/admin/system/settings/onboarding/stats')
    } catch (e) {
  ...
  const statsViaUnknown = computed(() => {
    const s = stats.value
    if (!s) return 0
    return Math.max(
      0,
      s.completed - s.completed_via_finished - s.completed_via_skipped,
    )
  })

  const statsNotCompleted = computed(() => {
    const s = stats.value
    return s ? Math.max(0, s.total_users - s.completed) : 0
  })
