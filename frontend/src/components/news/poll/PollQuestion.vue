<template>
  ...
  >
    ...
    <p
      v-if="question.is_multiple && isInteractive && question.max_choices"
    ...
    <div
      v-if="hasImages"
      class="news-poll__options-toolbar"
    >
      <span class="news-poll__image-hint">
        <n-icon
          size="14"
          class="news-poll__image-hint-icon"
        >
          <ExpandOutline />
        </n-icon>
        {{ t('news.poll.imageHint') }}
      </span>
      <div
        class="news-poll__preview-size"
        role="group"
        :aria-label="t('news.poll.previewSize')"
      >
        <n-button
          v-for="s in previewSizes"
          :key="s"
          size="tiny"
          :type="previewSize === s ? 'primary' : 'default'"
          quaternary
          :title="t(sizeTitleKeys[s])"
          @click="setPreviewSize(s)"
        >
          {{ s.toUpperCase() }}
        </n-button>
      </div>
    ...
    <div :class="['news-poll__options', { 'news-poll__options--grid': hasImages, [`news-poll__options--preview-${previewSize}`]: hasImages }]">
      <PollOption
        v-for="(opt, oIdx) in sortedOpts"
        :key="opt.id"
        :opt="opt"
        :question="question"
...
import { usePollPreviewSize, type PollPreviewSize } from './composables/usePollPreviewSize'

const { t } = useI18n()
const { previewSize, setPreviewSize } = usePollPreviewSize()

const previewSizes: PollPreviewSize[] = ['s', 'm', 'l']
const sizeTitleKeys: Record<PollPreviewSize, string> = {
  s: 'news.poll.previewSizeSmall',
  m: 'news.poll.previewSizeMedium',
  l: 'news.poll.previewSizeLarge',
}
...
.news-poll__image-hint-icon {
  flex-shrink: 0;
}

.news-poll__preview-size {
  display: inline-flex;
  gap: 2px;
}
...
.news-poll__options--preview-s {
  grid-template-columns: repeat(4, 1fr);
}

.news-poll__options--preview-l {
  grid-template-columns: repeat(2, 1fr);
}

/* 3 фиксированные колонки на телефоне ужмут превью до ~85px — уходим на 2 */
