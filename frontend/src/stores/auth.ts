...
export const useAuthStore = defineStore('auth', () => {
  ...
  // Косметический флаг методиста (learning_admins, из bootstrap) — только для
  // меню/guard'ов; бэкенд перепроверяет членство на каждом запросе.
  const isLearningAdmin = ref(false)
  ...
  async function loadBootstrap(): Promise<LoadUserResult> {
    ...
    try {
      ...
      isLearningAdmin.value = Boolean(data.is_learning_admin)
      setSessionAuthSource(user.value?.auth_source)
  ...
  return {
    ...
    isAuthenticated, isEditor, isAdmin, isLocalUser, isHelpdeskAgent, isLearningAdmin,
