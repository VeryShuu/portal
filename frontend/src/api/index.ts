...
function _handle401(): void {
  ...
  // Learn-контур (ADR-051): Keycloak/портал-SKO на нём нет — редирект на
  // /api/v1/auth/login увёл бы обучаемого на SSO портал-домена. Страница
  // слушает auth:expired и сама ведёт на форму входа learn-контура.
  if (import.meta.env.VITE_LEARN_CONTOUR) return
...
export async function api<T = unknown>(
  ...
  try {
    ...
    // Learn-контур (ADR-051): портального silent-refresh здесь нет и быть
    // не может (/auth/refresh не входит в allowlist) — сразу к редирект-логике.
    if (!import.meta.env.VITE_LEARN_CONTOUR) {
