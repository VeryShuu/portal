...
export function useAgentInboxQuery(
  ...
  return useQuery({
    ...
    staleTime: 0,
    // Поллинг (для живого обновления секций инбокса «Новые заявки»/«В работе»
    // и их счётчиков). ``refetchIntervalInBackground`` не указан — дефолт
    // false: не гоняем трафик в неактивной вкладке (тот же принцип, что у
    // счётчиков меню). Поиск (search-режим) поллинг не передаёт — on-demand.
