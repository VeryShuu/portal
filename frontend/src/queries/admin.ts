...
import { fetchMatrixBot, putMatrixBot, type MatrixBotSettingsIn } from '../api/matrixBot'
import {
  fetchMessengerOutbox, fetchMessengerOutboxItem, retryMessengerOutboxItem, cancelMessengerOutboxItem,
...
// ── Matrix bot settings (вкладка «Корпоративный чат», группа «Уведомления») ─

export function useMatrixBotQuery() {
  return useQuery({
    queryKey: queryKeys.admin.matrixBot(),
    queryFn: () => fetchMatrixBot(),
    staleTime: 30_000,
  })
}

export function usePutMatrixBotMutation() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (dto: MatrixBotSettingsIn) => putMatrixBot(dto),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.admin.matrixBot() }),
  })
}

// ── Messenger outbox (вкладка «Очередь мессенджеров») ───────────────────────

export function useMessengerOutboxQuery(params: MaybeRefOrGetter<MessengerOutboxFilters>) {
  return useQuery({
    queryKey: computed(() =>
      queryKeys.admin.messengerOutbox(toValue(params) as Record<string, unknown>),
    ),
    queryFn: () => fetchMessengerOutbox(toValue(params) ?? {}),
    staleTime: 0,
    placeholderData: (prev) => prev,
  })
}

export function useMessengerOutboxItemQuery(id: MaybeRefOrGetter<string | null>) {
  return useQuery({
    queryKey: computed(() => queryKeys.admin.messengerOutboxItem(String(toValue(id) ?? ''))),
    queryFn: () => fetchMessengerOutboxItem(String(toValue(id))),
    enabled: computed(() => !!toValue(id)),
    staleTime: 0,
  })
}

export function useRetryMessengerOutboxMutation() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => retryMessengerOutboxItem(id, true),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin', 'messenger-outbox'] }),
  })
}

export function useCancelMessengerOutboxMutation() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (id: string) => cancelMessengerOutboxItem(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin', 'messenger-outbox'] }),
  })
}
