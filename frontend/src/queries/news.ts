...
export function useDeleteNewsMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (_, id) => {
      // Вычистим ВСЕ per-id ключи до invalidateQueries(news.all): ключи вложены
      // под ['news'], иначе invalidate перезапустит активный запрос (например,
      // комментарии ещё открытой страницы) по уже soft-deleted новости → 404.
      ...
      qc.removeQueries({ queryKey: queryKeys.news.comments(id) })
      qc.removeQueries({ queryKey: queryKeys.news.poll(id) })
      qc.invalidateQueries({ queryKey: queryKeys.news.all })
    },
  })
}
