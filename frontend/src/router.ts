    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          meta: { requiresAuth: true, title: 'nav.learning' },
        },
        {
          path: ROUTES.APPROVALS,
          name: 'approvals',
          component: () => import('./pages/approvals/ApprovalsPage.vue'),
          meta: { requiresAuth: true, title: 'nav.approvals' },
        },
        {
          path: '/learning/courses/:slug',
          name: 'learning-course',
          component: () => import('./pages/learning/LearnerCoursePage.vue'),
          meta: { requiresAuth: true, title: 'learning.course.title' },
        },
        {
          path: ROUTES.LEARNING_ADMIN,
          name: 'learning-admin',
          component: () => import('./pages/learning/LearningAdminPage.vue'),
          meta: { requiresLearningAdmin: true, title: 'nav.learningAdmin' },
        },
        {
          path: ':pathMatch(.*)*',
          name: 'not-found',
...
function requireRole(to: RouteTo): GuardOutcome {
  ...
  if (to.meta.requiresLearningAdmin && !auth.isLearningAdmin && !auth.isAdmin) {
    return { name: 'home' }
  }
...
const MODULE_ROUTES: ReadonlyArray<{
  prefix: string
  module: 'nextcloud' | 'photos' | 'meetings' | 'signature' | 'helpdesk' | 'learning' | 'approvals'
}> = [
  ...
  { prefix: '/learning', module: 'learning' },
  { prefix: ROUTES.APPROVALS, module: 'approvals' },
]
