...
const MatrixTab = defineAsyncComponent(() => import('./admin/tabs/MatrixTab.vue'))
const MessengerOutboxTab = defineAsyncComponent(() => import('./admin/tabs/MessengerOutboxTab.vue'))
...
const DirectumTab = defineAsyncComponent(() => import('./admin/tabs/DirectumTab.vue'))
const ApprovalsTab = defineAsyncComponent(() => import('./admin/tabs/ApprovalsTab.vue'))
...
const GROUPS: GroupDef[] = [
  ...
  },
  {
    ...
    tabs: [
      ...
      { name: 'matrix', label: 'admin.tabs.matrix', component: MatrixTab },
      { name: 'email-outbox', label: 'admin.tabs.emailOutbox', component: EmailOutboxTab },
      { name: 'messenger-outbox', label: 'admin.tabs.messengerOutbox', component: MessengerOutboxTab },
    ],
  },
  {
    key: 'system',
    tabs: [
      ...
      { name: 'directum', label: 'admin.tabs.directum', component: DirectumTab },
      { name: 'approvals', label: 'admin.tabs.approvals', component: ApprovalsTab },
...
// Ширина обёртки по вкладке: аналитика — дашборд с таблицами, ей тесно в
// --content-wide (1600px) на широких экранах; остальным вкладкам админки
// хватает wide-капа.
const FULL_WIDTH_TABS = new Set(['analytics'])
const wrapVariant = computed(() =>
  FULL_WIDTH_TABS.has(activeTab.value) ? 'u-page-wrap--full' : 'u-page-wrap--wide',
)
