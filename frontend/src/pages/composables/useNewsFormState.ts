...
export function useNewsFormState(options: {
  ...
  watch(editNewsData, (news) => {
    if (news && !formInitialized.value) {
      ...
      form.value.cover_focal_x = news.cover_focal_x ?? null
      form.value.cover_focal_y = news.cover_focal_y ?? null
      form.value.cover_focal_zoom = news.cover_focal_zoom ?? null
      coverImageUrl.value = news.cover_image_url ?? null
      markPristine()
    }
  }, { immediate: true })
