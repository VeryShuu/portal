...
// Поллинг 60 c (синхронно со счётчиком меню): пока страница открыта, новые
// заявки/ответы и цифры секций подтягиваются без F5 — в неактивной вкладке
// TanStack приостанавливает поллинг (refetchIntervalInBackground=false по
// умолчанию). Поиск — on-demand, без поллинга.
...
const newQ = useAgentInboxQuery(newParams, {
...
const inWorkQ = useAgentInboxQuery(inWorkParams, {
  enabled: () => !isSearchMode.value && (assignmentScope.value === 'all' || !!myId.value),
  refetchInterval: INBOX_POLL_MS,
})
