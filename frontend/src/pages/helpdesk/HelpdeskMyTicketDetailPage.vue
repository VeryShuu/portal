...
async function load() {
  ...
  try {
    ...
    // После успеха инвалидируем счётчик меню — красный бейдж «Поддержка»
    // гаснет сразу, не дожидаясь поллинга (60 c).
    void markMyTicketRead(ticketId)
      .then(() => qc.invalidateQueries({ queryKey: queryKeys.helpdesk.myTicketCounts() }))
      .catch(() => {
