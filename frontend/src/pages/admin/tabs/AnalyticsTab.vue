  ...
  <div class="analytics-wrap">
    ...
    >
      ...
      <div class="series-card">
        <div class="series-card__title">
          {{ t('admin.analytics.approvals.title') }}
        </div>
        <div class="kpi-row">
          <span>{{ t('admin.analytics.approvals.approved') }}</span>
          <b>{{ dashboard.approvals.approved }}</b>
        </div>
        <div class="kpi-row">
          <span>{{ t('admin.analytics.approvals.rejected') }}</span>
          <b>{{ dashboard.approvals.rejected }}</b>
        </div>
        <div class="kpi-row">
          <span>{{ t('admin.analytics.approvals.activeUsers') }}</span>
          <b>{{ dashboard.approvals.active_users }}</b>
        </div>
        <div class="kpi-row">
          <span>{{ t('admin.analytics.approvals.lastEvent') }}</span>
          <b>{{ formatDateTime(dashboard.approvals.last_event_at) || '—' }}</b>
        </div>
        <div
          v-if="approvalsDaily.length"
          class="sparkline sparkline--dual"
          role="img"
          :aria-label="t('admin.analytics.approvals.title')"
        >
          <div
            v-for="(p, i) in approvalsDaily"
            :key="`appr-${i}`"
            class="sparkline__pair"
            :title="`${p.day}: ${p.approved} / ${p.rejected}`"
          >
            <div
              class="sparkline__bar sparkline__bar--approved"
              :style="{ height: sparkHeightOf(p.approved, approvalsMax) }"
            />
            <div
              class="sparkline__bar sparkline__bar--rejected"
              :style="{ height: sparkHeightOf(p.rejected, approvalsMax) }"
            />
          </div>
        </div>
        <div
          ...
          class="spark-legend"
        >
          <span class="spark-legend__item">
            <i class="spark-legend__dot spark-legend__dot--approved" />
            {{ t('admin.analytics.approvals.approved') }}
          </span>
          <span class="spark-legend__item">
            <i class="spark-legend__dot spark-legend__dot--rejected" />
            {{ t('admin.analytics.approvals.rejected') }}
          </span>
        </div>
      </div>
    </div>
...
const approvalsDaily = computed(() => dashboard.value?.approvals.daily ?? [])
const approvalsMax = computed(() =>
  Math.max(0, ...approvalsDaily.value.map((p) => p.approved + p.rejected)),
)

function sparkHeightOf(value: number, max: number): string {
  if (max <= 0) return '2%'
...
/* Двухцветный ряд согласований: пара баров на день (согласовано/отклонено). */
.sparkline--dual {
  gap: 2px;
}
...
.sparkline__pair .sparkline__bar {
  flex: 0 1 8px;
}

.sparkline__bar--approved {
  background: var(--color-success, #18a058);
}

.sparkline__bar--rejected {
...
.spark-legend__dot--approved {
  background: var(--color-success, #18a058);
}

.spark-legend__dot--rejected {
  background: var(--color-brand-red, #d92e2e);
}
</style>
