    ...
    <div class="branding-section">
      ...
      >
        <div class="branding-fields">
          ...
          >
            <n-input
              v-model:value="emailForm.from_name"
              :placeholder="t('admin.email.fromNamePlaceholder')"
              clearable
            />
          </n-form-item>
          <div class="email-row-2">
...
watch(emailSettingsData, (data) => {
  if (data) {
    emailForm.value = {
      ...
      from_name: (data as { from_name?: string }).from_name ?? '',
...
async function saveEmailSettings() {
  ...
  try {
    ...
    const data = await api<{
      host: string; port: number; from_address: string; from_name: string; username: string; password_set: boolean
