  ...
  <div class="branding-wrap">
    <div class="branding-section">
      ...
      <div class="branding-fields">
        ...
        <n-form-item
          :label="t('admin.system.learningBaseUrl')"
          style="margin-bottom:0"
        >
          <n-input
            v-model:value="sysForm.learning_base_url"
            :placeholder="t('admin.system.learningBaseUrlPlaceholder')"
          />
        </n-form-item>
        <n-form-item
    ...
    <div class="branding-section">
      ...
      <div class="branding-fields">
        ...
        <n-form-item
          :label="t('admin.system.videoIframeOrigins')"
          style="margin-bottom:0"
        >
          <n-input
            v-model:value="videoOriginsText"
            type="textarea"
            :rows="4"
            :placeholder="t('admin.system.videoIframeOriginsPlaceholder')"
          />
        </n-form-item>
        <div style="font-size:12px;color:var(--color-text-secondary)">
          {{ t('admin.system.videoIframeOriginsHint') }}
        </div>
        <n-form-item
...
const sysForm = ref({
  ...
  phone_extract_regex: '',
  video_iframe_origins: [] as string[],
})

// Список origin'ов в UI — текст «по одному в строке»; в настройке — массив.
const videoOriginsText = computed({
  get: () => sysForm.value.video_iframe_origins.join('\n'),
  set: (v: string) => {
    sysForm.value.video_iframe_origins = v
      .split('\n')
      .map((s) => s.trim())
      .filter(Boolean)
  },
...
watch(sysSettingsData, (data) => {
  if (data) {
    ...
    sysForm.value.learning_base_url = data.learning_base_url
    ...
    sysForm.value.video_iframe_origins = data.video_iframe_origins ?? []
...
async function saveSystemSettings() {
  ...
  try {
    await saveSettingsMut.mutateAsync({
      ...
      learning_base_url: sysForm.value.learning_base_url,
      ...
      video_iframe_origins: sysForm.value.video_iframe_origins,
    })
