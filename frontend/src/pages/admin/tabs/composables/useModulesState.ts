...
export function useModulesState() {
  ...
  const directumToggling = ref(false)
  const learningToggling = ref(false)
  const approvalsToggling = ref(false)
  ...
  watch(modulesData, (data) => {
    if (data) {
      ...
      if (data.directum) modulesForm.value.directum.enabled = data.directum.enabled
      if (data.learning) modulesForm.value.learning.enabled = data.learning.enabled
  ...
  async function onToggleSignature(value: boolean) {
    signatureToggling.value = true
    try {
      await api('/admin/modules/signature', {
        method: 'PUT',
        body: { enabled: value },
      })
  ...
  async function onToggleDirectum(value: boolean) {
    directumToggling.value = true
    try {
      await api('/admin/modules/directum', {
        method: 'PUT',
        body: { enabled: value },
      })
      modulesForm.value.directum.enabled = value
      qc.invalidateQueries({ queryKey: queryKeys.admin.modules() })
      message.success(t('admin.modules.saved'))
    } catch (e) {
      message.error(parseApiError(e, t))
    } finally {
      directumToggling.value = false
    }
  }

  function goToDirectum() {
    router.push({ path: ROUTES.ADMIN, query: { tab: 'directum' } })
  }

  async function onToggleLearning(value: boolean) {
    learningToggling.value = true
    try {
      await api('/admin/modules/learning', {
        method: 'PUT',
        body: { enabled: value },
      })
      modulesForm.value.learning.enabled = value
      qc.invalidateQueries({ queryKey: queryKeys.admin.modules() })
      message.success(t('admin.modules.saved'))
    } catch (e) {
      message.error(parseApiError(e, t))
    } finally {
      learningToggling.value = false
    }
  }

  async function onToggleApprovals(value: boolean) {
    approvalsToggling.value = true
    try {
      await api('/admin/modules/approvals', {
        method: 'PUT',
        body: { enabled: value },
      })
      modulesForm.value.approvals.enabled = value
      qc.invalidateQueries({ queryKey: queryKeys.admin.modules() })
      message.success(t('admin.modules.saved'))
    } catch (e) {
      message.error(parseApiError(e, t))
    } finally {
      approvalsToggling.value = false
    }
  }

  function goToApprovals() {
    router.push({ path: ROUTES.ADMIN, query: { tab: 'approvals' } })
  }

  function goToLearning() {
    router.push(ROUTES.LEARNING_ADMIN)
  }
