...
export default [
  {
    ignores: [
      "dist/**",
      "node_modules/**",
      // tests/e2e больше НЕ игнорируется (аудит 2026-08-21, P0.6):
      // E2E-код проходит тот же ESLint, что и остальной фронтенд.
      "playwright-report/**",
      "coverage/**",
      "**/*.d.ts",
      "scripts/**",
  ...
  },
  {
    ...
    rules: {
      ...
      // намеренно не типизируют пропы и не выносятся в отдельный файл — это
  ...
  {
    // Аудит тестирования 2026-08-23 (P2): e2e-спеки обязаны импортировать
    // test из './lib/test' — авто-фикстура роняет тест на pageerror и
    // console.error. Прямой импорт '@playwright/test' в спеках обнулял бы
    // гейт незаметно. fixtures/ — типизированные хелперы без своих тестов,
    // им прямой импорт разрешён.
    files: ["tests/e2e/**/*.spec.ts", "tests/e2e/auth.setup.ts"],
    rules: {
          ...
          paths: [
            {
              ...
              message:
                "Импортируй test/expect из './lib/test' — там авто-гейт pageerror/console.error (аудит 2026-08-23).",
            },
          ],
        },
      ],
