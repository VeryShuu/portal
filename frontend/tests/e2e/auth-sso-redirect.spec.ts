...
test.describe('Auto-SSO redirect for guests', () => {
  test('гость на / → редирект на /api/v1/auth/login', async ({ page }) => {
    let capturedUrl = ''
    ...
    // Аудит-4: у корня redirect= легитимно ОТСУТСТВУЕТ (корень — цель по
    // умолчанию, прод не прокидывает параметр; проверено живым прогоном
    // run #863). Сохранение redirect= для НЕкорневых путей — тест 2 ниже.
  })
