...
function makeUser(id: string, dept: string | null, hidden = false): UserPublic {
  return {
    id,
    email: `${id}@x.test`,
    full_name: `User ${id}`,
...
describe('cov-media useStaffEdit', () => {
  ...
  it('enterEdit handles loading guard and fetch error branch', async () => {
    // Резолвер в массиве: присваивание внутри callback'а TS не отслеживает
    // (control-flow narrowing сузил бы let до null).
    ...
    pendingResolves[0]?.({ items: [makeUser('a1', 'A')], total: 1 })
    await p1
    await p2
