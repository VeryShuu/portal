...
describe('links/bookmarks API client', () => {
  ...
  it('createLink POSTs body', async () => {
    await createLink({ title: 't', url: 'http://u', is_active: true, show_on_home: true, sort_order: 0, supports_sso: false })
    expect(apiMock).toHaveBeenCalledWith('/links', {
      method: 'POST',
      body: { title: 't', url: 'http://u', is_active: true, show_on_home: true, sort_order: 0, supports_sso: false },
    })
  })
