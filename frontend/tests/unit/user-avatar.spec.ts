...
vi.mock('naive-ui', () => ({
  NAvatar: {
    ...
    template:
      '<div class="n-avatar" :data-size="size" :data-src="src" :data-focal="JSON.stringify(imgProps)"><slot /></div>',
  },
}))
...
describe('UserAvatar', () => {
  ...
  it('passes focal style to avatar img (object-position + scale)', () => {
    const wrapper = mount(UserAvatar, {
      props: {
        user: {
          full_name: 'A B',
    ...
    const focal = JSON.parse(wrapper.find('.n-avatar').attributes('data-focal') ?? '{}')
    expect(focal.style.objectPosition).toBe('30% 70%')
    expect(focal.style.transform).toBe('scale(1.5)')
    expect(focal.style.transformOrigin).toBe('30% 70%')
  })

  it('focal defaults to center when fields are null', () => {
    const wrapper = mount(UserAvatar, {
      props: {
        user: { full_name: 'A B', avatar_url: 'http://x/a.webp', avatar_focal_x: null },
        size: 40,
      },
    })
    const focal = JSON.parse(wrapper.find('.n-avatar').attributes('data-focal') ?? '{}')
    expect(focal.style.objectPosition).toBe('50% 50%')
    expect(focal.style.transform).toBe('none')
  })
