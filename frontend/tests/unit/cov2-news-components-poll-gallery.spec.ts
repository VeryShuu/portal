...
import type { PollForm } from '../../src/components/news/poll-panel/composables/usePollPanelState'
...
describe('cov2 PollOption.vue', () => {
  ...
  it('opens an image preview without emitting a vote selection', async () => {
    const Cmp = (await import('../../src/components/news/poll/PollOption.vue')).default
    const w = mount(Cmp, {
      props: {
        opt: { id: 'o3', text: '', image_url: '/full.png', votes_percent: null, votes_count: 0, sort_order: 0 },
        question,
        hasImages: true,
        selected: false,
        hasVoted: false,
    ...
    await w.find('.news-poll__option-image-trigger').trigger('click')
    expect(w.find('.lightbox').exists()).toBe(true)
    expect(w.find('.lightbox__img').attributes('src')).toBe('/full.png')
    expect(w.emitted('opt-change')).toBeUndefined()
    expect(w.find('input').attributes('aria-label')).toContain('news.poll.optionNumber')
  })
})
