...
describe('src/queries/helpdesk', () => {
  ...
  it('useAgentInboxQuery forwards refetchInterval (живое обновление секций)', async () => {
    const { useAgentInboxQuery } = await import('../../src/queries/helpdesk')
    useAgentInboxQuery({}, { refetchInterval: 60_000 })
    expect(capturedQueries[0].refetchInterval).toBe(60_000)
  })

  it('useAgentInboxQuery без опций не включает поллинг (поиск — on-demand)', async () => {
    const { useAgentInboxQuery } = await import('../../src/queries/helpdesk')
    useAgentInboxQuery()
    expect(capturedQueries[0].refetchInterval).toBeUndefined()
  })
