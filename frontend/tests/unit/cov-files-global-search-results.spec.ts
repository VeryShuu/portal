...
type ResultsApi = ReturnType<(typeof import('../../src/composables/useGlobalSearchResults'))['useGlobalSearchResults']>
...
describe('useGlobalSearchResults', () => {
  ...
  it('aborts in-flight request on query change and ignores AbortError', async () => {
    ...
    // Резолверы складываем в массив: присваивание внутри callback'а TS не
    // отслеживает (control-flow narrowing), let-переменная сузилась бы до null.
    ...
    pendingResolves[0]?.({ news: [{ id: 'late' }], kb: [{ id: 'late' }], users: [{ id: 'late' }] })
    await flush()
  })
