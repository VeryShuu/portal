...
// usePhotoUpload регистрирует onMounted/onBeforeUnmount — вызов вне setup()
// даёт Vue.warn «no active component instance» (аудит 2026-08-21, этап 6;
// baseline — 0 warnings). withSetup монтирует одноразовый host-компонент.
const composableHosts: VueWrapper[] = []
...
function withSetup<T>(composable: () => T): T {
  ...
  composableHosts.push(mount(Host))
  return result
}

afterEach(() => {
  composableHosts.splice(0).forEach(w => w.unmount())
})
...
describe('usePhotoUpload composable', () => {
  it('has correct interface shape', async () => {
    ...
    const result = withSetup(() => usePhotoUpload(folderId, onSuccess))
  ...
  it('uploadingActive is false when queue is empty', async () => {
    ...
    const onSuccess = vi.fn().mockResolvedValue(undefined)
    const { uploadingActive, uploadQueue } = withSetup(() => usePhotoUpload(folderId, onSuccess))
  ...
  it('uploadDoneCount counts done items', async () => {
    ...
    const folderId = ref<string | null>('folder-1')
    const onSuccess = vi.fn().mockResolvedValue(undefined)
    const { uploadQueue, uploadDoneCount } = withSetup(() => usePhotoUpload(folderId, onSuccess))

    uploadQueue.value = [
      { file: new File([''], 'a.jpg'), status: 'done', progress: 100 },
      { file: new File([''], 'b.jpg'), status: 'error', progress: 50 },
      { file: new File([''], 'c.jpg'), status: 'done', progress: 100 },
    ]
  ...
  it('onDrop ignores non-file drag types', async () => {
    ...
    const folderId = ref<string | null>('folder-1')
    const onSuccess = vi.fn().mockResolvedValue(undefined)
    const { onDrop } = withSetup(() => usePhotoUpload(folderId, onSuccess))
  ...
  it('runUploadQueue does nothing when folderId is null', async () => {
    ...
    const { runUploadQueue } = withSetup(() => usePhotoUpload(folderId, onSuccess))
