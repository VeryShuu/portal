...
import type { FileShareList, FilesSubjectSearchResult, PermissionList } from '../../src/api/files'

type ShareRow = FileShareList['items'][number]
type PermRow = PermissionList['items'][number]
...
const fileApiMock = vi.hoisted(() => ({
  fetchFileShares: vi.fn(async (): Promise<{ items: ShareRow[] }> => ({ items: [] })),
  createFileShare: vi.fn(async () => ({})),
  revokeFileShare: vi.fn(async () => ({})),
  searchFilesSubjects: vi.fn(async (): Promise<FilesSubjectSearchResult[]> => []),
  fetchPermissions: vi.fn(async (): Promise<{ items: PermRow[] }> => ({ items: [] })),
...
const apiIndexMock = vi.hoisted(() => ({
  api: vi.fn(async (): Promise<FilesSubjectSearchResult[]> => []),
}))
...
function makeSubject(subject_id: string, subject_type: 'user' | 'group' = 'user'): FilesSubjectSearchResult {
...
function makePerm(overrides: Partial<PermRow> = {}): PermRow {
  return {
    id: 'p-1',
    folder_id: 'f-1',
    subject_type: 'user',
