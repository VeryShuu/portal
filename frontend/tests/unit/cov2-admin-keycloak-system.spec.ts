...
vi.mock('naive-ui', () => ({
  ...
  NInput: {
    template: `<textarea v-if="type === 'textarea'" :value="value" :placeholder="placeholder" @input="$emit('update:value', $event.target.value)" />
      <input v-else :value="value" :placeholder="placeholder" @input="$emit('update:value', $event.target.value)" />`,
...
vi.mock('@tanstack/vue-query', () => ({
  ...
  useMutation: vi.fn((opts: { mutationFn?: (arg: unknown) => Promise<unknown>; onSuccess?: () => void }) => {
...
describe('cov2 admin tabs: keycloak + system', () => {
  ...
  it('SystemTab: textarea видео-origin\'ов заполняется из настроек и парсится при вводе', async () => {
    useQueryMock
      .mockReturnValueOnce(qResult({
        portal_base_url: 'https://portal.local',
        timezone: 'Europe/Moscow',
        ...
        kb_import_max_size_mb: 25,
        phone_extract_regex: '',
        video_iframe_origins: ['https://video.mage.ru', 'https://rutube.ru'],
      }))
      .mockReturnValueOnce(qResult(undefined, { isLoading: true }))
    ...
    // getter: список из настроек отрисован построчно (стаб NInput: type=textarea → textarea)
    const textarea = wrapper.find('textarea')
    expect((textarea.element as HTMLTextAreaElement).value).toBe(
      'https://video.mage.ru\nhttps://rutube.ru',
    )

    // setter: ввод парсится в массив (пустые строки отбрасываются) и уходит в PATCH
    await textarea.setValue(' https://video.mage.ru \n\nhttps://rutube.ru')
    apiMock.mockResolvedValue({ ok: true })
    const save = wrapper.findAll('button').find((b) => b.text().includes('admin.system.save'))!
    expect(save).toBeTruthy()
    await save.trigger('click')
    await flushPromises()

    const patchCall = apiMock.mock.calls.find(
      (c) => String(c[0]).includes('/admin/system/settings'),
    )
    expect(patchCall).toBeTruthy()
    expect(patchCall![1].body.video_iframe_origins).toEqual([
      'https://video.mage.ru',
      'https://rutube.ru',
    ])
  })
