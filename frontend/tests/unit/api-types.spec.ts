...
describe('api/photos.ts — типы из OpenAPI схемы', () => {
  it('PhotoFolderTreeNode имеет поле children (опциональное в схеме)', () => {
    // FolderTreeNode.children в OpenAPI опционален; клиентское пересечение с
    // обязательным children аннигилирует optionality (undefined & X[] = never)
    // — фактический тип строго PhotoFolderTreeNode[]. toExtend ловит возврат
    // опциональности и смену типа элемента. Изменение — осознанный дрейф.
    expectTypeOf<PhotoFolderTreeNode>().toHaveProperty('children')
    expectTypeOf<PhotoFolderTreeNode['children']>().toExtend<PhotoFolderTreeNode[]>()
  })
  ...
  it('BulkActionResponse имеет errors и processed (affected удалён)', () => {
    expectTypeOf<BulkActionResponse>().toHaveProperty('errors')
    expectTypeOf<BulkActionResponse>().toHaveProperty('processed')
    expectTypeOf<BulkActionResponse['errors']>().toEqualTypeOf<string[]>()
    expectTypeOf<BulkActionResponse['processed']>().toBeNumber()
  })
...
describe('api/kb.ts — типы из OpenAPI схемы', () => {
  it('KbSection имеет поле children (опциональное в схеме)', () => {
    // KbSectionPublic.children опционален — как у PhotoFolderTreeNode выше.
    expectTypeOf<KbSection>().toHaveProperty('children')
    expectTypeOf<KbSection['children']>().toExtend<KbSection[]>()
  })
  ...
  it('KbComment имеет поле body', () => {
    ...
    expectTypeOf<KbComment['body']>().toExtend<string | null>()
  })

  it('KbVersion имеет поле version (переименовано из version_number)', () => {
    expectTypeOf<KbVersion>().toHaveProperty('version')
    expectTypeOf<KbVersion['version']>().toBeNumber()
  })
