...
import { sanitizeHtml, sanitizeHtmlAllowIframe, sanitizeKbHtml } from '../../src/utils/sanitize'
...
describe('sanitizeHtmlAllowIframe', () => {
  ...
  it('allows the trusted Mage video origin', () => {
    const out = sanitizeHtmlAllowIframe(
      '<iframe src="https://video.mage.ru/videos/embed/test"></iframe>',
      ['https://video.mage.ru'],
    )
    expect(out).toContain('<iframe')
  })
...
describe('sanitizeKbHtml', () => {
  // Профиль для KB-статей: <iframe> гейтится общим парсером (origin из
  // video_iframe_origins, только embed-форматы), плюс <details>/<summary>/<figure>/<figcaption>.
  ...
  // Единый гейт = CSP frame-src: проходят только канонические embed-URL
  // (watch-форматы конвертируются), произвольные пути больше не «разрешены
  // по хосту» — такие iframe браузер всё равно блокирует.
  it('youtube embed конвертируется в youtube-nocookie (embed-origin из CSP)', async () => {
    const { sanitizeKbHtml } = await import('../../src/utils/sanitize')
    const out = sanitizeKbHtml('<iframe src="https://www.youtube.com/embed/abc"></iframe>')
    expect(out).toContain('<iframe')
    expect(out).toContain('https://www.youtube-nocookie.com/embed/abc')
  })

  it('youtu.be watch-ссылка конвертируется в embed', async () => {
    const { sanitizeKbHtml } = await import('../../src/utils/sanitize')
    const out = sanitizeKbHtml('<iframe src="https://youtu.be/abc"></iframe>')
    expect(out).toContain('src="https://www.youtube-nocookie.com/embed/abc"')
  })

  it('rutube: канонический embed остаётся, произвольный путь удаляется', async () => {
    const { sanitizeKbHtml } = await import('../../src/utils/sanitize')
    const embed = sanitizeKbHtml('<iframe src="https://rutube.ru/play/embed/abc123def0"></iframe>')
    expect(embed).toContain('<iframe')
    const arbitrary = sanitizeKbHtml('<iframe src="https://rutube.ru/play/abc"></iframe>')
    expect(arbitrary).not.toContain('<iframe')
  })

  it('vk: полный video_ext.php остаётся, без id — удаляется', async () => {
    const { sanitizeKbHtml } = await import('../../src/utils/sanitize')
    const full = sanitizeKbHtml('<iframe src="https://vk.com/video_ext.php?oid=1&id=2"></iframe>')
    expect(full).toContain('<iframe')
    expect(full).toContain('vkvideo.ru')
    const incomplete = sanitizeKbHtml('<iframe src="https://vk.com/video_ext.php?oid=1"></iframe>')
    expect(incomplete).not.toContain('<iframe')
  })
...
describe('video iframe healing (общий парсер в sanitize-гейте)', () => {
  it('новости: legacy watch-ссылка PeerTube конвертируется в embed при рендере', () => {
    const out = sanitizeHtmlAllowIframe(
      '<iframe src="https://video.mage.ru/w/oqiQGxn9m9ygf4mkK6Rosv"></iframe>',
      ['https://video.mage.ru'],
    )
    expect(out).toContain('src="https://video.mage.ru/videos/embed/oqiQGxn9m9ygf4mkK6Rosv"')
    expect(out).not.toContain('/w/')
  })

  it('новости: youtube watch-ссылка конвертируется в youtube-nocookie embed', () => {
    const out = sanitizeHtmlAllowIframe(
  ...
  it('БЗ: корпоративный origin теперь разрешён (раньше список был без video.mage.ru)', () => {
    const out = sanitizeKbHtml(
      '<iframe src="https://video.mage.ru/videos/embed/oqiQGxn9m9ygf4mkK6Rosv"></iframe>',
      ['https://video.mage.ru'],
    )
    expect(out).toContain('<iframe')
  })

  it('БЗ: watch-ссылка лечится, чужой origin удаляется', () => {
    const healed = sanitizeKbHtml(
      '<iframe src="https://rutube.ru/video/abc123def0/"></iframe>',
      ['https://rutube.ru'],
    )
    expect(healed).toContain('src="https://rutube.ru/play/embed/abc123def0"')

    const removed = sanitizeKbHtml('<iframe src="https://evil.example.com/x"></iframe>', ['https://rutube.ru'])
    expect(removed).not.toContain('<iframe')
  })

  it('дубль sandbox из legacy-контента схлопывается в один атрибут', () => {
    const out = sanitizeHtmlAllowIframe(
      '<iframe src="https://video.mage.ru/videos/embed/x" sandbox="allow-scripts allow-same-origin" loading="lazy" sandbox="allow-scripts allow-same-origin"></iframe>',
      ['https://video.mage.ru'],
    )
    expect(out.match(/sandbox=/g)?.length).toBe(1)
  })
})
