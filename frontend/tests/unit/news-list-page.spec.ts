...
describe('NewsListPage.vue', () => {
  ...
  it('shows editor action buttons (create, categories, mailing) when isEditor is true', async () => {
  ...
  it('does not show trash button for editor (trash endpoint is admin-only)', async () => {
    mockAuthStore.isEditor = true
    mockAuthStore.isAdmin = false

    const wrapper = await mountPage()

    // GET /api/v1/news/trash требует admin: чип не показываем editor-у,
    // иначе клик ведёт к 403.
    const buttons = wrapper.findAll('.u-page-head__actions .n-button')
    expect(buttons).toHaveLength(3)
    expect(wrapper.find('.trash-tab-stub').exists()).toBe(false)
  })

  it('toggles trash view via trash button and renders TrashNewsTab for admins', async () => {
    mockAuthStore.isEditor = true
