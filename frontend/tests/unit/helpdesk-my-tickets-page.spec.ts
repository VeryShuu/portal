...
describe('HelpdeskMyTicketsPage (двухблочный вид)', () => {
  ...
  it('запрашивает два блока с правильными фильтрами (unassigned/assigned)', async () => {
    ...
    const calls = fetchMyTicketsMock.mock.calls.map((c) => c[0] as Record<string, unknown>)
    const waitingCall = calls.find((c) => c.unassigned === true)
    const inWorkCall = calls.find((c) => c.assigned === true)
