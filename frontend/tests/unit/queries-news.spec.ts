...
describe('src/queries/news', () => {
  ...
  describe('useDeleteNewsMutation', () => {
    ...
    it('onSuccess removes all per-id caches (incl. comments) and invalidates all', async () => {
      const { useDeleteNewsMutation } = await import('../../src/queries/news')
      useDeleteNewsMutation()
      await _capturedMutations[0].onSuccess(undefined, 'n1')

      // Комментарии обязаны вычищаться до invalidateQueries(['news']):
      // их ключ вложен под ['news'], иначе invalidate перезапустит активный
      // запрос комментариев уже soft-deleted новости → 404 в консоли.
      expect(mockRemove).toHaveBeenCalledTimes(5)
      const removedKeys = mockRemove.mock.calls.map((call) => {
        const arg = call[0] as { queryKey: unknown[] }
        return JSON.stringify(arg.queryKey)
      })
      expect(removedKeys).toContain(JSON.stringify(['news', 'detail', 'n1']))
      expect(removedKeys).toContain(JSON.stringify(['news', 'gallery', 'n1']))
      expect(removedKeys).toContain(JSON.stringify(['news', 'attachments', 'n1']))
      expect(removedKeys).toContain(JSON.stringify(['news', 'comments', 'n1']))
      expect(removedKeys).toContain(JSON.stringify(['news', 'poll', 'n1']))
      expect(mockInvalidate).toHaveBeenCalledWith({ queryKey: ['news'] })
    })
  })
