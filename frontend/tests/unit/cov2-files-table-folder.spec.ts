...
import type { FileIcon } from '../../src/api/files'
...
const fileApiMock = vi.hoisted(() => ({
  ...
  fileIcon: vi.fn((_item: unknown, _custom?: (ext: string) => string | null): FileIcon => ({ kind: 'emoji', char: '📎' })),
  formatFileSize: vi.fn((n: number) => `${n}b`),
  isCollaboraFile: vi.fn((_item: unknown) => false),
  isPreviewableImage: vi.fn((_item: unknown) => false),
  isPreviewablePdf: vi.fn((_item: unknown) => false),
}))
...
describe('FilesTable', () => {
  ...
  it('last_modified column: dir → dash, file without date → dash, file with date → formatted', async () => {
    const wrapper = await mountTable()
    const dt = wrapper.findComponent({ name: 'NDataTable' })
    const columns = dt.props('columns') as Array<Record<string, any>>
    const modCol = columns.find((c) => c.key === 'last_modified')!

    // Директория — прочерк.
    expect(modCol.render(makeItem({ is_dir: true }))).toBe('—')
    // Файл без last_modified (undefined) — форматтер получает null и тоже
    // отдаёт прочерк, а не Invalid Date.
    expect(modCol.render(makeItem({ last_modified: undefined }))).toBe('—')
    // Файл с датой — форматированная строка «01.01.2024, 10:00» (год присутствует).
    const formatted = modCol.render(makeItem({ last_modified: '2024-01-01T10:00:00Z' }))
