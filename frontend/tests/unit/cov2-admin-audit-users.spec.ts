...
const globalPlugins = {
  plugins: [i18n],
  // AuditTab использует n-tooltip/n-switch без локального импорта (в app
  // они регистрируются глобально naive-ui) — в тесте регистрируем глобально,
  // иначе Vue.warn «Failed to resolve component» (baseline: 0 warnings).
  components: {
    'n-tooltip': { template: '<div class="n-tooltip-stub"><slot /></div>' },
    'n-switch': {
      props: ['value'],
      emits: ['update:value'],
      template: '<input type="checkbox" class="n-switch-stub" :checked="value" @change="$emit(\'update:value\', !value)" />',
    },
  },
  stubs: {
