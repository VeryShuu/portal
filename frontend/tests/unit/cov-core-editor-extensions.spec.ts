...
describe('src/components/editor/extensions/IframeEmbed', () => {
  it('allows the trusted Mage video host', async () => {
    const { IframeEmbed } = await import('../../src/components/editor/extensions/IframeEmbed')
    const options = (IframeEmbed as any).config.addOptions()

    expect(options.allowedOrigins).toContain('https://video.mage.ru')
  })
})
