...
// Композаблы с lifecycle-хуками (usePhotoListing: onMounted/onBeforeUnmount/
// onUnmounted) обязаны вызываться внутри setup() — иначе Vue.warn «no active
// component instance» (аудит 2026-08-21, этап 6; baseline — 0 warnings).
// withSetup монтирует одноразовый host-компонент и возвращает результат.
...
describe('usePhotoListing', () => {
  it('onSortChange resets to first page and clears photos', () => {
    const l = withSetup(() => usePhotoListing({ selectedFolderId: ref('f1') }))
...
describe('useZipExport', () => {
  ...
  it('startZip is a no-op when no folder is selected', async () => {
    const z = withSetup(() => useZipExport(ref<string | null>(null)))
    await z.startZip()
  ...
  it('startZip opens download immediately when the job is already done', async () => {
    ...
    const z = withSetup(() => useZipExport(ref<string | null>('f1')))
...
describe('useImportScan', () => {
  ...
  it('confirmImportScan aborts when declined', async () => {
    const { confirmImportScan } = withSetup(() => useImportScan(vi.fn().mockResolvedValue(undefined)))
    h.confirmMock.mockResolvedValue(false)
  ...
  it('confirmImportScan queues a scan and polls to completion', async () => {
    ...
    const { confirmImportScan } = withSetup(() => useImportScan(onTreeChanged))
    h.confirmMock.mockResolvedValue(true)
  ...
  it('confirmImportScan reports a queue failure', async () => {
    const { confirmImportScan } = withSetup(() => useImportScan(vi.fn()))
    h.confirmMock.mockResolvedValue(true)
