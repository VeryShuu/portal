...
function makeTicket(over: Partial<HelpdeskTicketListItem> = {}): HelpdeskTicketListItem {
  return {
    id: 't1',
    number: 42,
    subject: 'Subj',
...
describe('TicketList.vue', () => {
  ...
  it('рендерит одну TicketListItem на элемент и пробрасывает takingId/taking', () => {
    ...
    const rows = wrapper.findAllComponents(TicketListItem)
  ...
  it('пробрасывает open/take события от строк наверх', async () => {
    ...
    const row = wrapper.findComponent(TicketListItem)
    row.vm.$emit('open', 'a')
    row.vm.$emit('take', 'a')
