...
describe('TicketMessageList — chat UI', () => {
    ...
    const wrapper = mountList(
      [
        makeMsg({
          direction: 'outbound',
          cc: [
            { email: 'a@x.local', name: 'Иван', is_requester: false },
            { email: 'b@y.local', name: null, is_requester: false },
          ],
        }),
      ],
      true,
  ...
  it('hides cc badge in requester (non-agent) mode even with cc', () => {
    ...
    const wrapper = mountList(
      [makeMsg({ cc: [{ email: 'a@x.local', name: null, is_requester: false }] })],
      false,
    )
    expect(wrapper.find('.chat-bubble__cc').exists()).toBe(false)
  })
