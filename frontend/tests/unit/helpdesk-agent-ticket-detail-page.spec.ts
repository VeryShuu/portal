...
const ru = {
  helpdesk: {
    ...
    takenByOtherToast: 'Заявку уже взял в работу {name}.',
    statusLocked: 'Статус может менять только ответственный',
    closedNoReply: 'Закрытая заявка доступна только для просмотра',
    statuses: {
      open: 'В работе',
      pending: 'Ожидание',
      closed: 'Закрыто',
...
vi.mock('naive-ui', () => ({
  ...
  NPopconfirm: {
    ...
    mounted() {
      ...
      ;(this as unknown as { $el: HTMLElement }).$el.querySelector('.n-button')?.addEventListener('click', () => {
        ;(this as unknown as { $emit: (event: string) => void }).$emit('positive-click')
      })
    },
  },
  useMessage: () => ({ error: messageError, success: messageSuccess, info: messageInfo }),
}))
...
// Страница инвалидирует agentTicketCounts после markTicketRead (красный бейдж
// меню гаснет сразу) — нужен QueryClient в контексте; сохраняем для spy-проверок.
...
function mountPage() {
  ...
  return mount(HelpdeskAgentTicketDetailPage, {
    global: { plugins: [i18n, [VueQueryPlugin, { queryClient: lastQueryClient }]], stubs },
  })
}
...
describe('HelpdeskAgentTicketDetailPage', () => {
  ...
  it('не показывает мутации и форму ответа для архивного тикета', async () => {
    fetchAgentTicketMock.mockResolvedValue(
      makeTicket({ status: 'closed', assignee_user_id: 'me', archived: true }),
    )
    const wrapper = mountPage()
    await flushPromises()
    ...
    expect(wrapper.text()).toContain('Закрытая заявка доступна только для просмотра')
  })
  ...
  it('после 409 при Take перезагружает карточку и показывает имя агента', async () => {
    fetchAgentTicketMock.mockResolvedValueOnce(makeTicket({ assignee_user_id: null }))
    takeTicketMock.mockRejectedValue(
      Object.assign(new Error('conflict'), { response: { status: 409 } }),
    )
    fetchAgentTicketMock.mockResolvedValueOnce(
    ...
    await wrapper.findAll('.n-button').find((b) => b.text().includes('Взять'))!.trigger('click')
    await flushPromises()

    expect(fetchAgentTicketMock).toHaveBeenCalledTimes(1)
    expect(messageError).toHaveBeenCalledWith('Заявку уже взял в работу Иван Петров.')
  })
  ...
  it('после markTicketRead инвалидирует agentTicketCounts (красный бейдж гаснет)', async () => {
    markTicketReadMock.mockResolvedValue(undefined)
    // Инвалидация происходит внутри onMounted-load — шпионим прототип
    // (queryClient создаётся в mountPage).
    const invalidateSpy = vi.spyOn(QueryClient.prototype, 'invalidateQueries')
    try {
      mountPage()
      await flushPromises()

      expect(markTicketReadMock).toHaveBeenCalledWith('ticket-99')
  ...
  it('ошибка markTicketRead не валит карточку (best-effort, без инвалидации)', async () => {
    markTicketReadMock.mockRejectedValue(new Error('boom'))
    const invalidateSpy = vi.spyOn(QueryClient.prototype, 'invalidateQueries')
    try {
      mountPage()
      await flushPromises()

      // Карточка загружена и функциональна — read-state косметика.
      expect(fetchAgentTicketMock).toHaveBeenCalledWith('ticket-99')
