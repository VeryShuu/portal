...
vi.mock('../../src/components/meetings/PasteParticipantsModal.vue', () => ({
  default: {
    ...
    mounted() {
      pasteModalAdd.mockImplementation((items: InvitedUser[]) =>
        (this as unknown as { $emit: (event: string, ...args: unknown[]) => void }).$emit('add', items))
    },
  },
}))
...
function mountPicker(modelValue: InvitedUser[] = []) {
  activeWrapper = mount(ParticipantPicker, {
...
// Unmount после теста: иначе render-effect ParticipantPicker продолжает звать t()
// (vue-i18n resolveMessageFormat → обращение к window) ПОСЛЕ jsdom-teardown →
// неперехваченный 'window is not defined' → vitest exit 1 (флюк, проявлялся на CI).
