...
describe('src/stores/onboarding', () => {
  ...
  it('defaultOnboardingSteps returns steps of key sections with required fields', async () => {
    ...
    for (const step of steps) {
      ...
      expect(typeof step.is_new).toBe('boolean')
    }
    // Новые разделы помечены новинками для delta-тура уже прошедших сотрудников
    const newIds = steps.filter((s) => s.is_new).map((s) => s.id)
  ...
  it('exposes defaults before loading', async () => {
    ...
    expect(store.onboardingSteps).toHaveLength(9)
  })
