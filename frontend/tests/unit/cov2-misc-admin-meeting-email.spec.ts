...
vi.mock('naive-ui', () => ({
  ...
  NInput: { template: '<input :value="value" @input="$emit(\'update:value\', $event.target.value)" />', props: ['value', 'placeholder', 'maxlength', 'clearable', 'size'], emits: ['update:value'] },
...
vi.mock('../../src/queries/admin', () => ({
  useSystemSettingsQuery: (...args: Parameters<typeof systemSettingsQueryMock>) => systemSettingsQueryMock(...args),
