...
import type { CreateNewsPollRequest } from '../../src/api/news'
...
describe('news API client', () => {
  ...
  describe('likes', () => {
    it('likeNews POSTs /news/:id/like and returns the like state', async () => {
      const state = { liked_by_me: true, like_count: 5 }
      apiMock.mockResolvedValueOnce(state)
      const res = await likeNews('n-40')
      expect(apiMock).toHaveBeenCalledWith('/news/n-40/like', { method: 'POST' })
      expect(res).toEqual(state)
    })

    it('unlikeNews DELETEs /news/:id/like and returns the like state', async () => {
      const state = { liked_by_me: false, like_count: 4 }
      apiMock.mockResolvedValueOnce(state)
      const res = await unlikeNews('n-41')
      expect(apiMock).toHaveBeenCalledWith('/news/n-41/like', { method: 'DELETE' })
      expect(res).toEqual(state)
    })
  })
  ...
  describe('polls API client', () => {
    ...
    it('createNewsPoll POSTs /news/:id/poll', async () => {
      const dto: CreateNewsPollRequest = { is_anonymous: true, allow_revote: false, results_visibility: 'after_vote', questions: [] }
    ...
    it('voteNewsPoll POSTs /news/:id/poll/vote', async () => {
      const dto = { answers: [{ question_id: 'q-1', option_ids: ['opt-1'] }] }
