...
describe('meetings API client', () => {
  ...
  describe('rooms', () => {
    ...
    it('createRoom POSTs', async () => {
      await createRoom({ name: 'X', kind: 'physical', sort_order: 0, timezone: 'Europe/Moscow' })
      expect(apiMock).toHaveBeenCalledWith('/meetings/rooms', {
        method: 'POST',
        body: { name: 'X', kind: 'physical', sort_order: 0, timezone: 'Europe/Moscow' },
      })
    })
  ...
  describe('bookings', () => {
    ...
    it('deleteBooking defaults to apply_to this (backend default)', async () => {
  ...
  describe('participants', () => {
    ...
    it('fetchParticipantAbsences POSTs emails + on_date', async () => {
      apiMock.mockResolvedValueOnce({ 'a@x.com': { category: 'vacation', start_date: '2026-08-18', end_date: '2026-08-25' } })
      const res = await fetchParticipantAbsences(['a@x.com', 'b@x.com'], '2026-08-20')
      expect(apiMock).toHaveBeenCalledWith('/meetings/participants/absences', {
        method: 'POST',
        body: { emails: ['a@x.com', 'b@x.com'], on_date: '2026-08-20' },
      })
      expect(res['a@x.com'].category).toBe('vacation')
    })
  })
