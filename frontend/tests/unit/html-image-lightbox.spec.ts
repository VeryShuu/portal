...
describe('HtmlImageLightbox — общий лайтбокс для inline-картинок', () => {
  ...
  it('поддерживает доступный trigger и останавливает всплытие по опции', async () => {
    let parentClicks = 0
    const Host = defineComponent({
      setup() {
        return () => h('div', { onClick: () => { parentClicks += 1 } }, [
          h(HtmlImageLightbox, { stopImageClick: true }, {
            default: () => h('button', {
              'data-lightbox-src': '/img/a.png',
              'data-lightbox-alt': 'вариант',
            }, 'Открыть'),
          }),
        ])
      },
    ...
    expect(wrapper.find('.lightbox__img').attributes('src')).toBe('/img/a.png')
