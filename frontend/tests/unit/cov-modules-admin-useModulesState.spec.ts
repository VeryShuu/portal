...
vi.mock('../../src/router', () => ({ ROUTES: { PHOTOS: '/photos', MEETINGS: '/meetings', STAFF: '/staff', LEARNING_ADMIN: '/learning/admin' } }))
...
describe('useModulesState (src/pages/admin/tabs/composables)', () => {
  ...
  it('onToggleLearning covers success/error and goToLearning routes', async () => {
    const { useModulesState } = await import('../../src/pages/admin/tabs/composables/useModulesState')
    const state = useModulesState()

    mockApi.mockResolvedValueOnce({})
    await state.onToggleLearning(true)
    expect(mockApi).toHaveBeenCalledWith('/admin/modules/learning', { method: 'PUT', body: { enabled: true } })
    expect(state.modulesForm.value.learning.enabled).toBe(true)
    expect(mockMessageSuccess).toHaveBeenCalledWith('admin.modules.saved')

    mockApi.mockRejectedValueOnce(new Error('x'))
    await state.onToggleLearning(false)
    expect(mockMessageError).toHaveBeenCalledWith('errors.generic')

    state.goToLearning()
    expect(mockRouterPush).toHaveBeenCalledWith('/learning/admin')
  })
