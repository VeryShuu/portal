...
describe('OnboardingTour.vue', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    localStorage.clear()
    // Реальные цели шагов в DOM: тур фильтрует шаги, чей селектор ничего не находит
    document.body.innerHTML = '<div id="tour-s1"></div><div id="tour-s2"></div>'
    ...
    onboardingStoreState.onboardingSteps = [
      { id: 's1', title: 'Step 1', body: 'Body 1', selector: '#tour-s1' },
      { id: 's2', title: 'Step 2', body: 'Body 2', selector: '#tour-s2', is_new: true },
    ]
  })
  ...
  it('auto-starts for incomplete onboarding and finishes through primary button', async () => {
    ...
    expect(wrapper.find('.tour-highlight').exists()).toBe(true)
