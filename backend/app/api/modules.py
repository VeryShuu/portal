...
class DirectumModuleOut(BaseModel):
    enabled: bool
...
class ApprovalsModuleOut(BaseModel):
    enabled: bool
...
class AllModuleSettingsOut(BaseModel):
    ...
    directum: DirectumModuleOut
    learning: LearningModuleOut
    approvals: ApprovalsModuleOut
...
class MeetingsModuleIn(BaseModel):
    ...
    rsvp_ingest_enabled: bool = False
...
class DirectumModuleIn(BaseModel):
    enabled: bool = False
...
class ApprovalsModuleIn(BaseModel):
    enabled: bool = False
...
def _meetings_out(m: MeetingsModuleSettings) -> MeetingsModuleOut:
    return MeetingsModuleOut(
        ...
        rsvp_ingest_enabled=m.rsvp_ingest_enabled,
    )
...
async def get_modules_for_ui(_: CurrentUser, redis: RedisDep) -> AllModuleSettingsOut:
    ...
    return AllModuleSettingsOut(
        ...
        directum=DirectumModuleOut(enabled=m.directum.enabled),
        learning=LearningModuleOut(enabled=m.learning.enabled),
        approvals=ApprovalsModuleOut(enabled=m.approvals.enabled),
    )
...
async def update_meetings_module(
    ...
    updated = MeetingsModuleSettings(
        ...
        rsvp_ingest_enabled=data.rsvp_ingest_enabled,
    )
    m.meetings = updated
    _save_modules(m)
    await bump_version(redis, _CACHE_VERSION_KEY)
...
@router.put("/admin/modules/directum", response_model=DirectumModuleOut)
async def update_directum_module(
    data: DirectumModuleIn,
    admin: AdminDep,
    redis: RedisDep,
) -> DirectumModuleOut:
    m = await load_modules_shared(redis)
    m.directum = DirectumModuleSettings(enabled=data.enabled)
    _save_modules(m)
    await bump_version(redis, _CACHE_VERSION_KEY)
    await _emit_audit(
        redis,
        event_type="modules.toggled",
        user_id=str(admin.id),
        resource_id="directum",
        metadata={"module": "directum", "enabled": data.enabled},
    )
    logger.info("modules.directum_updated", enabled=data.enabled)
    return DirectumModuleOut(enabled=data.enabled)


@router.put("/admin/modules/learning", response_model=LearningModuleOut)
async def update_learning_module(
    data: LearningModuleIn,
    admin: AdminDep,
    redis: RedisDep,
) -> LearningModuleOut:
    m = await load_modules_shared(redis)
    m.learning = LearningModuleSettings(enabled=data.enabled)
    _save_modules(m)
    await bump_version(redis, _CACHE_VERSION_KEY)
    await _emit_audit(
        redis,
        ...
        metadata={"module": "learning", "enabled": data.enabled},
    )
    logger.info("modules.learning_updated", enabled=data.enabled)
    return LearningModuleOut(enabled=data.enabled)


@router.put("/admin/modules/approvals", response_model=ApprovalsModuleOut)
async def update_approvals_module(
    data: ApprovalsModuleIn,
    admin: AdminDep,
    redis: RedisDep,
) -> ApprovalsModuleOut:
    m = await load_modules_shared(redis)
    m.approvals = ApprovalsModuleSettings(enabled=data.enabled)
    _save_modules(m)
    await bump_version(redis, _CACHE_VERSION_KEY)
    await _emit_audit(
        redis,
        ...
        metadata={"module": "approvals", "enabled": data.enabled},
    )
    logger.info("modules.approvals_updated", enabled=data.enabled)
    return ApprovalsModuleOut(enabled=data.enabled)
