...
async def ready(request: Request) -> JSONResponse:
    ...
    if modules.nextcloud.enabled:
        ...
        if not sys_settings.nextcloud_url:
            ...
            # audit [PA-009]: «модуль включён, URL не задан» — misconfiguration,
            # а не готовность: Files-интеграция неработоспособна. Раньше /ready
            # отвечал 200 и Compose считал backend (и зависящий nginx) healthy.
            # Fail-closed: 503, пока оператор не задаст nextcloud_url или не
            # выключит модуль (docs/monitoring.md обещает 503 при NC failure).
            logger.warning("readiness_check.nextcloud_unconfigured_but_enabled")
            failed = True
        else:
            try:
                nc = get_nc_service()
    ...
    # PA-023: прямой readiness-сигнал для Prometheus. Docker health status
    # ни на что не влияет в рантайме (restart policy — только на exit),
    # а up{job="portal"} доказывает лишь живость процесса.
    backend_ready.set(0 if failed else 1)
    return JSONResponse(
