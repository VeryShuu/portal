...
def _set_sso_attempts_cookie(
    request: Request, response: RedirectResponse, attempts: list[float]
) -> None:
    response.set_cookie(
...
def _build_session_cookie_response(
    request: Request, redirect_target: str, session_id: str
) -> RedirectResponse:
