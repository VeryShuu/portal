...
async def audit_queue_depth(_admin: AdminDep, redis: RedisDep) -> dict[str, int]:
    try:
        pending = int(await redis.llen(AUDIT_QUEUE_KEY))  # type: ignore[misc]  # redis-py async-overload typing
        processing = int(await redis.llen("audit_processing"))  # type: ignore[misc]  # redis-py async-overload typing
