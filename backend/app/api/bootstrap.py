...
_DEFAULT_MODULES = AllModuleSettingsOut(
    ...
    directum=DirectumModuleOut(enabled=False),
    learning=LearningModuleOut(enabled=False),
    approvals=ApprovalsModuleOut(enabled=False),
)
...
class BootstrapOut(BaseModel):
    ...
    # Аналогично для методистов (learning_admins, require_learning_admin).
    is_learning_admin: bool = False


def _build_branding() -> BrandingSettingsOut:
    s = branding_assets.load_settings()
    sys = load_system_settings()
    iframe_origins = list(sys.video_iframe_origins)
    if sys.video_gallery_url and sys.video_gallery_url not in iframe_origins:
...
async def _is_learning_admin(user_id: uuid.UUID, role: str | None) -> bool:
    """Членство в ``learning_admins`` (методист); admin — суперсет. Собственная
    сессия — по той же причине, что и ``_is_helpdesk_agent`` (asyncio.gather)."""
    if role == "admin":
        return True
    ...
    async with AsyncSessionLocal() as session:
        res = await session.execute(
            select(LearningAdmin.user_id).where(LearningAdmin.user_id == user_id)
        )
        return res.first() is not None
...
async def bootstrap(
    ...
    modules_res, gallery_res, unread_res, is_agent_res, is_ladmin_res = await asyncio.gather(
        _get_modules(),
        _get_gallery_links(),
        _fetch_unread_count(user.id),
        _is_helpdesk_agent(user.id, user.role),
        _is_learning_admin(user.id, user.role),
    ...
    if isinstance(is_ladmin_res, BaseException):
        logger.warning("bootstrap.is_learning_admin_failed", error=str(is_ladmin_res))
        is_ladmin_res = False
    ...
    return BootstrapOut(
        ...
        is_learning_admin=is_ladmin_res,
    )
