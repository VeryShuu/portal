...
from app.services import files_shares as files_shares_service
...
async def revoke_file_share(
    ...
    safe_filename = sanitize_name(filename)
    # Транзакционная часть (отзыв + инвалидация кэша) — в production-сервисе:
    # её же драйвит integration-тест (audit-review 2026-08-22, P1).
    share = await files_shares_service.revoke_share(
        db, redis, folder_id=folder_id, filename=safe_filename, share_id=share_id
    )
