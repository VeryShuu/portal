...
async def _get_trashed_folder_or_404(
    db: DbDep, folder_id: uuid.UUID, *, not_trashed_detail: str
) -> PhotoFolder:
    folder = await folder_repo.fetch_folder_any(db, folder_id)
    if not folder:
        raise HTTPException(status_code=404, detail="Folder not found")
    if folder.deleted_at is None:
        raise HTTPException(status_code=400, detail=not_trashed_detail)
    return folder


async def _emit_folder_audit(
    redis: RedisDep,
    *,
    event_type: str,
    user: CurrentUser,
...
def _build_folder_tree(
    accessible_with_perm: list[tuple[PhotoFolder, str]],
) -> list[FolderTreeNode]:
...
async def list_folder_tree(db: DbDep, user: CurrentUser, redis: RedisDep) -> FolderTree:
    ...
    return FolderTree(items=_build_folder_tree(list(accessible_with_perm)))
...
async def get_folder(
    ...
    folder = await _get_active_folder_or_404(db, folder_id)
...
async def restore_folder(
    ...
    folder = await _get_trashed_folder_or_404(
        db, folder_id, not_trashed_detail="Folder is not deleted"
    )
...
async def purge_folder(
    ...
    folder = await _get_trashed_folder_or_404(
        db, folder_id, not_trashed_detail="Folder is not in trash"
    )
    await require_folder_permission(user, folder, PERM_MANAGER, db, redis)
