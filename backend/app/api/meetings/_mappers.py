...
from app.services.meetings.rsvp_ingest import load_rsvp_map


async def booking_to_out(db: AsyncSession, booking: Any) -> BookingOut:
    """Конвертация ORM-бронирования в ``BookingOut`` с enrich отсутствий и RSVP.
    ...
    RSVP-статусы — из ``meeting_rsvp`` (IMIP-ответы, :mod:`rsvp_ingest`).
    """
    ...
    await _apply_rsvp(db, booking, invited)
...
async def bookings_to_out(db: AsyncSession, bookings: list[Any]) -> list[BookingOut]:
    ...
    # Один bulk-запрос RSVP-статусов на все встречи списка.
    rsvp_map = await load_rsvp_map(db, [b.id for b in bookings])

    out: list[BookingOut] = []
    for booking, invited in prepared:
        absences = absences_by_date[booking.start_time.date()]
        booking_rsvp = rsvp_map.get(booking.id, {})
        for u in invited:
            u.absence = absences.get(u.email.lower())
            u.rsvp = booking_rsvp.get(u.email.lower())
        rooms = [RoomOut.model_validate(br.room) for br in booking.rooms]
...
async def _apply_rsvp(db: AsyncSession, booking: Any, invited: list[InvitedUser]) -> None:
    """Заполнить ``InvitedUser.rsvp`` для одного бронирования (in-place)."""
    if not invited:
        return
    booking_rsvp = (await load_rsvp_map(db, [booking.id])).get(booking.id, {})
    for u in invited:
        u.rsvp = booking_rsvp.get(u.email.lower())
