...
async def count_comments(db: AsyncSession, news_id: uuid.UUID) -> int:
    """Считает ВСЕ комментарии новости, включая soft-deleted placeholders.

    Согласовано с ``list_comments`` (он возвращает placeholders для якорей
    «комментарий удалён») и с KB-зеркалом ``app.api.kb.comments_repo``:
    раньше count фильтровал deleted — UI получал ``total``, не соответствующий
    фактической переборке страниц (audit PA-014).
    """
    res = await db.execute(select(func.count()).where(NewsComment.news_id == news_id))
