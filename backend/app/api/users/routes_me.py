...
@router.delete("/me/avatar", response_model=UserMe, summary="Удалить свой аватар")
async def delete_avatar(user: CurrentUser, db: DbDep) -> UserMe:
    updated = await users_me_service.delete_avatar(db, user)
    return cast(UserMe, UserMe.model_validate(updated))
