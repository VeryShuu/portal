...
def user_ref(user: User | None) -> KbUserRef | None:
    if user is None:
        return None
    # model_validate (from_attributes) — автовыбор новых полей (фокал аватара),
    # чтобы не дублировать список полей на каждом месте конструирования.
    return KbUserRef.model_validate(user)
...
def _article_to_public(
    ...
    return KbArticlePublic(
        ...
        created_by=KbUserRef.model_validate(creator) if creator else None,
        updated_by=KbUserRef.model_validate(updater) if updater else None,
