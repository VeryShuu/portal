...
async def _fetch_ticket_number_for_serving(
    db: DbDep, *, ticket_id: uuid.UUID, user: CurrentUser, is_agent: bool
) -> int:
    """Read live ticket first, then immutable archive for historical images."""
    try:
        return (
            await _fetch_ticket_for_media(db, ticket_id=ticket_id, user=user, is_agent=is_agent)
        ).number
    except HTTPException as exc:
        if exc.status_code != status.HTTP_404_NOT_FOUND:
            raise
    archived = await archive_service.fetch_archived_ticket(db, ticket_id=ticket_id)
    if archived is None or (not is_agent and archived.row.requester_user_id != user.id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ticket not found")
    return archived.row.number
...
async def upload_ticket_inline_media(
    ...
    # Upload remains live-only: archived tickets are immutable.
...
async def serve_ticket_inline_media(
    ...
    is_agent = await _is_helpdesk_agent(db, user=user)
    ticket_number = await _fetch_ticket_number_for_serving(
        db, ticket_id=ticket_id, user=user, is_agent=is_agent
    )
    ...
    internal_path = f"/internal/helpdesk-media/TKT-{ticket_number}/inline/{quote(filename)}"
    return Response(
