...
class TicketListItemOut(BaseModel):
    ...
    # ``True`` означает, что live-строка уже перенесена в read-only архив.
    # Поле additive: старые клиенты продолжают работать, UI выключает мутации.
    archived: bool = False
...
class AttachmentOut(BaseModel):
    ...
    size_bytes: int
    created_at: datetime
    # Additive URL for an immutable archived snapshot. Live attachments retain
    # the established ``GET /attachments/{id}`` contract.
...
class MessageOut(BaseModel):
    ...
    # Диагностика несохранённых вложений входящего письма. Поле наполняется
    # только в agent-detail, в requester-detail всегда пусто.
