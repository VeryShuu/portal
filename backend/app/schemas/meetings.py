...
class RsvpInfo(BaseModel):
    """RSVP-статус участника — ответ на приглашение из почтового клиента.
...
class InvitedUser(BaseModel):
    ...
    # Ответ на приглашение (принял/отклонил из Outlook и др.); None = не отвечал.
    # Тоже только выдача, при записи игнорируется (см. RsvpInfo).
    rsvp: RsvpInfo | None = None
...
class ParticipantAbsencesRequest(BaseModel):
    """Запрос действующих отсутствий участников на дату встречи.
    ...
    """Email'ы сотрудников. Внешних участников caller не отправляет; неизвестные
    email (нет в ``users``) просто не попадут в ответ."""

    on_date: _Date
    """Дата, на которую ищем действующее отсутствие (дата встречи)."""
