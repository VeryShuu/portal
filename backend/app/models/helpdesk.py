...
class HelpdeskMessage(Base):
    ...
    # Email Cc — участники-получатели письма (миграция 083). Формат JSONB:
    # ``[{"email": "a@x", "name": "Иван"}, ...]``. Для inbound заполняется из
    # заголовков ``To`` ∪ ``Cc`` входящего письма (``threading.extract_cc``);
    # для outbound — из формы ответа агента (чекбокс «Ответить всем»).
    # ``None`` для старых сообщений и для ответов без участников —
    # сериализатор нормализует в ``[]``.
    ...
    # Причины, по которым обычные вложения входящего email не были сохранены.
    # Только агентский UI: заявителю не раскрываем внутренние детали фильтрации.
    attachment_warnings: Mapped[list[str]] = mapped_column(JSONB, nullable=False, default=list)
    source: Mapped[str] = mapped_column(String(20), nullable=False)
