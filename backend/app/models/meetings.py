...
class MeetingBooking(Base):
    ...
    # Идемпотентность финальной RSVP-сводки: метка «за 15 мин до встречи уже
    # отправлена» (NULL = ещё не отправлена). См. docs/meetings.md §RSVP.
    rsvp_final_digest_sent_at: Mapped[datetime | None] = mapped_column(
...
class MeetingRsvp(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('accepted', 'declined', 'tentative')", name="ck_meeting_rsvp_status"
        ),
        UniqueConstraint(
            "booking_id", "participant_email", name="uq_meeting_rsvp_booking_participant"
        ),
        Index("idx_meeting_rsvp_booking", "booking_id"),
        Index("idx_meeting_rsvp_updated", "updated_at"),
    )
    ...
    source: Mapped[str] = mapped_column(String(16), nullable=False, server_default="email")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=text("NOW()")
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class MeetingRsvpEmailLog(Base):
    ...
    __tablename__ = "meeting_rsvp_email_log"
