...
def _looks_like_header(raw_date_col: str, raw_gender_col: str) -> bool:
    ...
    (встречаются и краткие варианты: «Дата | Пол»). Без распознавания заголовок
    попадал бы в errors (дата/пол не парсятся), засоряя каждый импорт одной
    лишней «ошибкой».

    Признаки (оба, AND):

    * 2-я колонка содержит «дата» (CI-подстрока — покрывает «Дата», «Дата
      рождения», «Физическое лицо.Дата рождения») И при этом сама не парсится
      как валидная дата (защита от значения «не-дата»: оно содержит «дата»,
      но ``parse_birth_date`` его не примет — а мы дополнительно проверяем,
      что это НЕ валидная дата, чтобы отличить заголовок от битых данных).
    * 3-я колонка оканчивается на «пол» (с учётом префикса «Физическое лицо.»)
      И сама не парсится как пол (защита от значения «пол» как данных — хоть
      это и маловероятно).

    Двойная проверка (подстрока И непарсимость) надёжнее, чем каждая по
    отдельности: «не-дата» содержит «дата», но не парсится как дата → это
    битые данные, а не заголовок (вторая колонка-признак «пол» не совпадёт).
    """
    d = (raw_date_col or "").strip().lower()
    g = (raw_gender_col or "").strip().lower()
    date_col_is_header = "дата" in d and parse_birth_date(raw_date_col) is None
    gender_col_is_header = g.endswith("пол") and parse_gender(raw_gender_col) is None
