...
def _build_nginx_csp(
    nextcloud_url: str,
    video_gallery_url: str = "",
    video_iframe_origins: list[str] | tuple[str, ...] | None = None,
) -> str:
    ...
    ``video_iframe_origins`` — runtime-настройка ``system.json``;
    ``None`` подставляет дефолт (``DEFAULT_VIDEO_IFRAME_ORIGINS``).
    """
    from urllib.parse import urlparse as _urlparse

    if video_iframe_origins is None:
        video_iframe_origins = DEFAULT_VIDEO_IFRAME_ORIGINS
    frame_src_parts = ["'self'", *video_iframe_origins]
