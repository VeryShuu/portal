...
async def sse_generator(
    ...
    - Читает все три потока (personal/meetings/photos) параллельными XREAD
      с ОДИНАКОВОЙ конечной блокировкой 500 мс — цикл обязан продолжаться
      даже при полном отсутствии событий, иначе голодает keepalive
      (см. комментарий у ensure_future).
    ...
    poll_block_ms = int(_SSE_POLL_INTERVAL * 1000)
    ...
    try:
        while True:
            ...
            try:
                # Все три XREAD — только с КОНЕЧНЫМ block. Redis-семантика
                # BLOCK 0 = «блокировать вечно»: gather не завершается, пока
                # во встречах/фото нет событий, цикл замирает и keepalive
                # перестаёт отправляться — внешний reverse proxy (дефолтный
                # proxy_read_timeout 60с) рвёт «молчащий» SSE-стрим, браузеры
                # уходят в reconnect-цикл (баг 2026-08-18, прод).
