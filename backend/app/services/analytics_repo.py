...
# Использование модуля «Согласование 1С» (события появились в PR #232,
# 2026-09-07). bulk_approved несёт число проведённых документов в
# metadata->>'approved' — считаем документы, а не партии.
_APPROVALS_DOC_APPROVED = (
    "count(*) FILTER (WHERE event_type = 'approvals.approved')"
    " + COALESCE(SUM(CASE WHEN event_type = 'approvals.bulk_approved'"
    " THEN COALESCE((metadata->>'approved')::int, 0) ELSE 0 END), 0)"
)


async def fetch_approvals_usage(
    db: AsyncSession, *, cutoff: datetime
) -> tuple[Row[Any], Sequence[Row[Any]]]:
    """(итоги за период, дневной ряд) использования согласования 1С.

    Итоги: ``approved`` — документы (одиночные + из партий), ``rejected``,
    ``active_users`` — уникальные согласующие, ``last_event_at``. Ряд — по дням:
    ``approved``/``rejected``.
    """
    usage = await db.execute(
        text(
            f"""
            SELECT
                {_APPROVALS_DOC_APPROVED}  AS approved,
                count(*) FILTER (WHERE event_type = 'approvals.rejected') AS rejected,
                count(DISTINCT user_id) FILTER (WHERE user_id IS NOT NULL) AS active_users,
                MAX(created_at) AS last_event_at
            FROM audit_log
            WHERE created_at >= :cutoff
              AND event_type IN ('approvals.approved',
                                 'approvals.rejected',
                                 'approvals.bulk_approved')
            """
        ),  # nosec B608 — фрагмент из статических имён событий; данные в params.
        {"cutoff": cutoff},
    )
