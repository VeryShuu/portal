...
# PostgreSQL-коды, при которых транзакция терминирована сервером и клиенту
# осмысленно повторить попытку: 40P01 deadlock_detected, 40001 serialization_failure.
_RETRYABLE_TX_STATES = frozenset({"40P01", "40001"})
...
async def _flush_or_conflict(
    ...
    """Flush in a savepoint; map lock-конфликты БД в BookingConflict.

    IntegrityError — EXCLUDE GIST booking_rooms_no_overlap (пересечение слотов):
    транзакция жива, savepoint откатывается штатно.

    DBAPIError с sqlstate 40P01/40001 — сервер ТЕРМИНИРОВАЛ транзакцию
    (deadlock между конкурентными multi-room-брони с комнатами в разном
    порядке; audit-review 2026-08-22, P1). Savepoint управляется вручную,
    без ``async with``: неявный savepoint-rollback по мёртвой транзакции
    клинчит сессию (флаг rollback-in-progress → IllegalStateChangeError на
    любых последующих вызовах — снова 500). Клиенту отдаётся предсказуемый
    409 «конфликт, повторите»; сессию выбрасывает get_db.

    NB: ``db.begin_nested()`` сам делает autoflush ожидающих вставок —
    конфликт может подняться уже там, поэтому вызов тоже под try.
    """

    def _to_conflict(exc: Exception) -> BookingConflict:
        with contextlib.suppress(Exception):
            db.expunge(booking)
        return BookingConflict([])

    def _killed_by_server(exc: DBAPIError) -> bool:
        return _sqlstate_of(exc) in _RETRYABLE_TX_STATES

    try:
        sp = await db.begin_nested()
    except IntegrityError as exc:
        if log_conflict:
            logger.info("meetings.booking.conflict", error=str(exc), where="begin_nested")
        raise _to_conflict(exc) from exc
    except DBAPIError as exc:
        if _killed_by_server(exc):
            logger.warning(
                "meetings.booking.tx_killed_by_server",
                sqlstate=_sqlstate_of(exc),
                note="normalized to 409 BookingConflict; client may retry",
            )
            raise _to_conflict(exc) from exc
        raise
