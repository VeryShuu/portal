...
def invited_users_to_jsonb(invited: list[InvitedUser]) -> list[dict]:
    ...
    Поля ``absence`` и ``rsvp`` намеренно **выбрасываются**: статус отсутствия
    (отпуск/болезнь/командировка) и ответ на приглашение (Accept/Decline из
    Outlook) не персистятся в JSONB-слепок — оба пересчитываются на лету
    (absence — cron'ом из ERP, rsvp — из ``meeting_rsvp``) и обогащаются
    «на лету» в выдаче (``booking_to_out`` / live-поиск) и в письме-приглашении
    (см. :mod:`app.services.meetings.absence_enrichment`, :mod:`rsvp_ingest`).

    Без этого strip'а `model_dump()` оставил бы в dict ``AbsenceInfo`` с
    ``datetime.date``-полями, а SQLAlchemy при JSONB-сериализации падает:
    ``TypeError: Object of type date is not JSON serializable`` (прод 2026-08-07,
    участник с заполненным ``absence``). С RSVP-инджестом (прод 2026-09-03) та же
    история с ``RsvpInfo.updated_at: datetime``: GET возвращал участника с
    заполненным ``rsvp``, фронтенд эхо-отправлял его в PUT /bookings →
    ``TypeError: Object of type datetime is not JSON serializable`` → 500 на
    любое редактирование брони с ответившим участником.

    Args:
        invited: список приглашённых (входные данные из Pydantic-схемы).

    Returns:
        Список plain-dict'ов без ``absence``/``rsvp``, готовых к записи в JSONB.
    """
    return [u.model_dump(exclude={"absence", "rsvp"}) for u in invited]
