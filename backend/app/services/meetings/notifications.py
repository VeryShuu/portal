...
async def _enqueue_all_recipients(
    ...
    await _enqueue_many(
        session, booking, invited_emails, method, ical_bytes, absences_by_email, reply_to
    )
    ...
    await _enqueue_room_emails(
        session, booking, method, ical_bytes, already_notified, absences_by_email, reply_to
    )
...
async def _enqueue_organizer_and_rooms(
    ...
    await _enqueue_room_emails(
        session, booking, "REQUEST", req_bytes, already_notified, absences_by_email, reply_to
    )
...
async def _enqueue_series_relink(
    ...
    await _enqueue_many(
        session, booking, list(invited_emails), "CANCEL", cancel_old, absences_by_email, reply_to
    )
    await _enqueue_organizer(
        session,
        booking,
    ...
    await _enqueue_room_emails(
        session, booking, "CANCEL", cancel_old, set(), absences_by_email, reply_to
    )
    ...
    await _enqueue_many(
        session, booking, list(invited_emails), "REQUEST", req_bytes, absences_by_email, reply_to
    )
    await _enqueue_organizer(
        session,
        booking,
    ...
    await _enqueue_room_emails(
        session, booking, "REQUEST", req_bytes, req_notified, absences_by_email, reply_to
    )
...
async def _enqueue_updated_with_diff(
    ...
    if diff.added_users:
        ...
        await _enqueue_many(
            session, booking, added_emails, "REQUEST", ical_bytes, absences_by_email, reply_to
        )
    ...
    if diff.removed_users:
        ...
        await _enqueue_many(
            session, booking, removed_emails, "CANCEL", cancel_bytes, absences_by_email, reply_to
        )
    ...
    if diff.non_participant_changed and diff.unchanged_users:
        ...
        await _enqueue_many(
            session, booking, unchanged_emails, "REQUEST", req_bytes, absences_by_email, reply_to
        )

    await _enqueue_organizer_and_rooms(
        session, booking, organizer_user, ical, already_notified, absences_by_email, reply_to
    )
...
async def enqueue_meeting_emails(
    ...
    *,
    ...
    # ORGANIZER iCal и Reply-To письма — email реального создателя встречи:
    # кнопки Accept/Decline в Outlook уходят на ORGANIZER, текстовые ответы —
    # на Reply-To. Без создателя (удалён / нет email) — fallback на адрес
    # портала для iCal и без Reply-To (поведение до этой правки).
    #
    # RSVP-инджест (meetings.rsvp_ingest_enabled): ORGANIZER = адрес портала —
    # IMIP-ответы падают на общий ящик и принимаются поллером
    # (app.services.meetings.rsvp_ingest). Reply-To остаётся на создателе:
    # текстовые ответы по-прежнему идут ему лично.
    from app.core.modules_config import load_modules

    organizer_email = (
        ((getattr(organizer_user, "email", "") or "").strip()) if organizer_user else ""
    )
    if load_modules().meetings.rsvp_ingest_enabled:
        ical_organizer_email = from_email
        # CN ORGANIZER при адресе портала — display name из email-настроек
        # («Портал Маге <portal@mage.ru>»), иначе клиенты рендерят
        # «ФИО создателя <portal@mage.ru>» и участники отвечают на общий ящик.
        ical_organizer_cn = _get_from_name() or booking.organizer_name
    else:
        ical_organizer_email = organizer_email or from_email
        ical_organizer_cn = booking.organizer_name
    reply_to: str | None = organizer_email or None
    ...
    def _ical(method: Literal["REQUEST", "CANCEL"]) -> bytes:
        if method not in cache:
            cache[method] = build_ical(
                ...
                organizer_email=ical_organizer_email,
                organizer_cn=ical_organizer_cn,
            )
        return cache[method]
    ...
    if action == "created":
        ...
        await _enqueue_organizer_and_rooms(
            session, booking, organizer_user, _ical, already_notified, absences_by_email, reply_to
        )
...
async def _enqueue_organizer(
    ...
    await _enqueue(session, booking, payload, method, ical_bytes, absences_by_email, reply_to)
...
async def _enqueue_room_emails(
    ...
    for br in booking.rooms:
        ...
        await _enqueue(
            session, booking, {"email": email}, method, ical_bytes, absences_by_email, reply_to
        )


def _meeting_payload(method: str, ical_bytes: bytes, reply_to: str | None) -> dict[str, Any]:
    """Payload outbox-строки kind=meeting: method + iCal + опциональный reply_to.

    ``reply_to`` кладётся только при реальном создателе встречи; воркер ставит
    из него заголовок ``Reply-To`` MIME-письма.
    """
    from app.services.email_outbox import encode_ical_bytes

    payload: dict[str, Any] = {"method": method, "ical_b64": encode_ical_bytes(ical_bytes)}
    if reply_to:
...
async def _enqueue(
    ...
    if not to_email:
        return
    try:
        from app.services.email_outbox import KIND_MEETING, enqueue_outbox_email
        ...
        await enqueue_outbox_email(
            ...
            subject=subject,
            body_html=html_body,
            payload=_meeting_payload(method, ical_bytes, reply_to),
...
def _build_outbox_item(
    ...
    if not to_email:
        return None
    from app.services.email_outbox import KIND_MEETING
...
async def _enqueue_many(
    ...
    try:
        ...
        items = [
            item
            for item in (
                _build_outbox_item(booking, email, method, subject, html_body, ical_bytes, reply_to)
...
def _get_from_name() -> str:
    """Portal SMTP display name (вкладка «Почта»), пусто — если не задано.

    Тот же TTL-кеш-паттерн, что и :func:`_get_from_email`.
    """
    import time

    from app.services.email_settings import read_email_settings

    _cache = _get_from_name.__dict__
    now = time.monotonic()
    if _cache.get("fetched_at") and now - _cache.get("fetched_at", 0) < 60:
        return str(_cache.get("value") or "")

    settings = read_email_settings()
    result = ((settings.from_name if settings else "") or "").strip()
