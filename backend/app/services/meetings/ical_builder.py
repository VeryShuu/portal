...
def build_ical(
    ...
    # ORGANIZER по RFC 5545 — адрес, собирающий RSVP-ответы (Accept/Decline из
    # Outlook уходят именно сюда). Это email создателя встречи (см.
    # notifications.enqueue_meeting_emails), fallback — адрес портала.
    # CN при адресе портала (RSVP-инджест) — display name из email-настроек:
    # клиенты показывают отправителя по CN ORGANIZER, и «ФИО <portal@…>»
    # провоцировал личные ответы на общий ящик (см. notifications).
    organizer = vCalAddress(f"mailto:{organizer_email}")
    organizer.params["cn"] = vText(organizer_cn or booking.organizer_name)
