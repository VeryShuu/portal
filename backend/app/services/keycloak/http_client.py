...
def _new_kc_client() -> httpx.AsyncClient:
    """Create the shared Keycloak client (with outbound-request metrics)."""
    from app.services import keycloak as _kc

    return instrument_httpx_client(
        httpx.AsyncClient(
...
def _get_kc_http_client() -> httpx.AsyncClient:
    ...
    if _kc._KC_HTTP_CLIENT is None or _kc._KC_HTTP_CLIENT.is_closed:
        _kc._KC_HTTP_CLIENT = _new_kc_client()
