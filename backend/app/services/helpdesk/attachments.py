...
from app.core.constants import (
    ...
    HELPDESK_MAX_TOTAL_OUTBOUND_MB,
)
...
_MAX_TOTAL_OUTBOUND_BYTES = HELPDESK_MAX_TOTAL_OUTBOUND_MB * 1024 * 1024

_CHUNK_SIZE = 1024 * 1024  # 1 MiB

# ``libmagic`` не различает старые бинарные форматы Microsoft Office внутри
# Compound File Binary Format: корректный ``.doc`` обычно определяется как
# ``application/x-ole-storage``. Доверять одному имени файла нельзя, поэтому
# принимаем этот generic-тип только для входящих email-вложений, когда имя и
# объявленный MIME согласованно указывают на разрешённый legacy Office-формат.
_OLE_LEGACY_OFFICE_MIMES_BY_SUFFIX: dict[str, str] = {
        ...
        for file in files:
            ...
            if total_so_far > max_total_bytes:
                ...
                raise HTTPException(
                    ...
                    detail=f"Total attachments size exceeds {max_total_bytes} bytes",
                )
...
async def save_image_bytes(
    ...
    *,
    ...
    effective = _effective_ingress_mime(
    ...
    if effective not in HELPDESK_ATTACHMENT_ALLOWED_MIMES:
        logger.warning(
            "helpdesk.attachment.rejected_mime",
            original_name=original_name,
            detected_mime=detected,
            declared_content_type=declared_content_type,
        )
        _append_rejection(rejection_sink, original_name, "неподдерживаемый тип файла")
        return None

    if len(data) > _MAX_ATTACHMENT_BYTES:
        logger.warning(
            "helpdesk.attachment.too_large",
            original_name=original_name,
            size=len(data),
            max=_MAX_ATTACHMENT_BYTES,
        )
        _append_rejection(rejection_sink, original_name, "размер файла превышает 25 МБ")
        return None

    if total_tracker is not None and total_tracker.total + len(data) > _MAX_TOTAL_BYTES:
        logger.warning(
            "helpdesk.attachment.total_exceeded",
            original_name=original_name,
        )
        _append_rejection(
            rejection_sink, original_name, "превышен общий лимит вложений письма (50 МБ)"
        )
        return None
...
def _effective_ingress_mime(
    *,
    detected_mime: str | None,
    declared_content_type: str | None,
    original_name: str,
) -> str:
    """Нормализовать MIME вложения, полученного через email-ingress.

    Для большинства файлов первичным источником остаётся ``libmagic``. Старые
    Office-документы — исключение: все три формата CFBF (``.doc/.xls/.ppt``)
    библиотека определяет одним ``application/x-ole-storage``. Такой контейнер
    принимаем только при строгом совпадении расширения с MIME-заголовком письма;
    это не превращает generic OLE-файлы в разрешённые вложения.
    """
    detected = (detected_mime or "application/octet-stream").lower()
    if detected != "application/x-ole-storage":
        return detected

    declared = (declared_content_type or "").split(";", 1)[0].strip().lower()
    expected = _OLE_LEGACY_OFFICE_MIMES_BY_SUFFIX.get(Path(original_name).suffix.lower())
    return expected if expected is not None and declared == expected else detected


def _append_rejection(sink: list[str] | None, original_name: str, reason: str) -> None:
    """Добавить безопасную для агентского UI причину отказа вложения."""
    if sink is not None:
        sink.append(f"Вложение «{original_name}» не сохранено: {reason}.")
