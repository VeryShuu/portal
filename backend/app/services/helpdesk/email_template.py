...
def render_reply_email(
    *,
    ...
    Видимый reply-маркер ставится перед содержимым письма. Это контролируемый
    якорь: при ответе удаляется только наша процитированная история, а новый
    forward пользователя выше маркера сохраняется целиком.
    ...
    marker_plain = build_reply_marker_plain(ticket.number).strip()
    html_out = _wrap(
        marker_html + body_html,
...
def render_system_email(
    *,
    ...
    """Системное письмо с тем же reply-разделителем, что и публичный ответ."""
    marker_html = build_reply_marker_html(ticket.number)
    marker_plain = build_reply_marker_plain(ticket.number).strip()
    html_out = _wrap(
        marker_html + f'<div style="padding:0 0 8px;">{_absolutize_img_src(body_html)}</div>',
        ticket_number=ticket.number,
        subject=ticket.subject,
        portal_url=portal_url,
    )
