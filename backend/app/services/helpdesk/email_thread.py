...
def build_thread_history(
    ...
    *,
    ...
    Возвращает ``(plain, html)`` — два представления предшествующих
    ...
    Если предшествующих сообщений нет (первый ответ на заявку) — оба
    представления пустые (письмо = только ответ, без разделителя/истории).
    """
    prior = [m for m in messages if m.id != exclude_id]
