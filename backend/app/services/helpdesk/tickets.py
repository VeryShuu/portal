...
async def fetch_ticket_for_agent(
    db: AsyncSession, *, ticket_id: uuid.UUID, for_update: bool = False
) -> HelpdeskTicket | None:
    ...
    # ``take`` and a first agent reply must serialize on the ticket row. The
    # assignment check, message/outbox writes and commit then share one DB
    # transaction: a concurrent request wakes up only after it can see the
    # winning assignee, so it cannot overwrite it or emit duplicate side effects.
    if for_update:
...
async def take_ticket_tx(db: AsyncSession, *, ticket_id: uuid.UUID, agent: User) -> HelpdeskTicket:
    """Транзакционное ядро ``POST /tickets/{id}/take`` (audit-review 2026-08-22, P1).

    Строчная блокировка (``FOR UPDATE``) → проверка assignee → назначение →
    outbox-email (если mailbox сконфигурирован) → единый commit. Эту функцию
    зовут и роутер, и integration-тест конкурентного take — удаление
    блокировки или commit'а из production-пути валит тест, а не остаётся
    незамеченным.
    """
    ...
    ticket = await fetch_ticket_for_agent(db, ticket_id=ticket_id, for_update=True)
    if ticket is None:
    ...
    ticket = await assign_ticket(db, ticket=ticket, assignee_id=agent.id)
    # Email инициатору с ФИО ответственного (ТЗ §6) — только при
    # сконфигурированном mailbox, в той же транзакции (outbox-инвариант).
...
async def delete_ticket(db: AsyncSession, *, ticket: HelpdeskTicket) -> int:
    ...
    * Pending-записи outbox (``email_outbox``/``messenger_outbox``) НЕ
    ...
    audit/логирования. Сначала коммитится DB-удаление, затем best-effort
    очистка FS: orphan-папка безопаснее необратимой потери файлов, если DB
    транзакция не смогла закоммититься.
    """
...
async def link_guest_tickets(
    ...
    *,
    ...
    точки вызова нет (там логин без upsert'а). Синхронно привязывает и записи,
    уже перенесённые в физический архив: иначе старый email-тикет не появился
    бы в истории нового пользователя. Возвращает кол-во привязанных тикетов.
    """
    res = await db.execute(
    ...
    await db.execute(
        update(HelpdeskTicketArchive)
        .where(
            HelpdeskTicketArchive.requester_user_id.is_(None),
            func.lower(HelpdeskTicketArchive.requester_email) == email.lower(),
        )
        .values(requester_user_id=user_id)
    )
    # Сохраняем прежнюю семантику return-value: caller использует его как
    # количество обновлённых live-тикетов, архивная привязка — побочный эффект.
    return len(tickets)
