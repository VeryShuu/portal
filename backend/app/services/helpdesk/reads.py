...
from app.schemas.helpdesk import ACTIVE_STATUSES, HelpdeskDirection
...
async def enrich_with_unread(
    ...
    *,
    ...
    # обработки KeyError для прочитанных.
    return {tid: (tid in unread_ticket_ids) for tid in ticket_ids}


async def count_unread_assigned_tickets(
    db: AsyncSession,
    *,
    user_id: uuid.UUID,
) -> int:
    """Сколько активных тикетов, назначенных агенту, имеют непрочитанные
    ответы заявителя.

    Счётчик для красной подсветки цифры-бейджа в меню «Инбокс поддержки»
    (``GET /tickets/counts`` → ``unread``). Семантика зеркальна
    :func:`enrich_with_unread`: ``direction='inbound'``, «непрочитан» =
    есть сообщение новее ``last_seen_at`` (или агент тикет вообще не открывал).
    Ограничение ``ACTIVE_STATUSES`` — то же, что у ``active``-счётчика:
    непрочитанные ответы в закрытых тикетах бейдж не подкрашивают (закрытое —
    архивная история, иначе красный «залипал» бы навсегда).

    Один ``COUNT(DISTINCT ticket_id)`` c JOIN'ом на тикеты — без пагинации,
    дёшево на polling-каденции бейджа (60 c).
    """
    # Correlated-подзапрос ``last_seen_at`` этого агента на тикет сообщения —
    # тот же паттерн, что в :func:`enrich_with_unread` (LEFT-семантика через
    # COALESCE + EPOCH_SENTINEL: нет строки reads → любое сообщение новое).
    last_seen_subq = (
        select(HelpdeskTicketRead.last_seen_at)
        .where(HelpdeskTicketRead.user_id == user_id)
        .correlate(HelpdeskMessage)
    ...
    stmt = (
        select(func.count(func.distinct(HelpdeskMessage.ticket_id)))
        .select_from(HelpdeskMessage)
        .join(HelpdeskTicket, HelpdeskTicket.id == HelpdeskMessage.ticket_id)
        .where(
            HelpdeskTicket.assignee_user_id == user_id,
            HelpdeskTicket.status.in_(ACTIVE_STATUSES),
            HelpdeskMessage.direction == INBOUND_DIRECTION,
            HelpdeskMessage.created_at > func.coalesce(last_seen_subq, EPOCH_SENTINEL),
        )
    )
    res = await db.execute(stmt)
...
async def count_my_unread_tickets(
    db: AsyncSession,
    *,
    user_id: uuid.UUID,
) -> int:
    """Сколько активных тикетов заявителя имеют непрочитанные ответы агентов.

    Зеркало :func:`count_unread_assigned_tickets` для заявительской стороны:
    ``direction='outbound'`` (публичные ответы агентов), «непрочитан» = есть
    сообщение новее ``last_seen_at`` (или заявитель тикет не открывал).
    Счётчик для красной подсветки бейджа «Поддержка» в меню
    (``GET /tickets/my/counts`` → ``unread``).

    Ограничение активными статусами — по той же причине, что у агентского
    счётчика: непрочитанные ответы в закрытых тикетах бейдж не подкрашивают
    (закрытое — архивная история, иначе красный «залипал» бы навсегда).
    """
    last_seen_subq = (
        select(HelpdeskTicketRead.last_seen_at)
        .where(HelpdeskTicketRead.user_id == user_id)
    ...
    stmt = (
        ...
        .where(
            ...
            HelpdeskTicket.status.in_(ACTIVE_STATUSES),
            HelpdeskMessage.direction == OUTBOUND_DIRECTION,
            HelpdeskMessage.created_at > func.coalesce(last_seen_subq, EPOCH_SENTINEL),
        )
    )
    res = await db.execute(stmt)
