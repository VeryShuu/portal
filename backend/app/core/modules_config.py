...
class MeetingsModuleSettings(BaseModel):
    ...
    # RSVP-инджест: ORGANIZER iCal = адрес портала, ответы Accept/Decline
    # принимает IMAP-поллер общего ящика (IMAP-блок email-settings, тот же,
    # что у ERP-синка). False → ORGANIZER = email создателя (PR #79).
...
class DirectumModuleSettings(BaseModel):
    enabled: bool = False


class LearningModuleSettings(BaseModel):
    enabled: bool = False


class ApprovalsModuleSettings(BaseModel):
    enabled: bool = False
...
class AllModuleSettings(BaseModel):
    ...
    directum: DirectumModuleSettings = Field(default_factory=DirectumModuleSettings)
    learning: LearningModuleSettings = Field(default_factory=LearningModuleSettings)
    approvals: ApprovalsModuleSettings = Field(default_factory=ApprovalsModuleSettings)
