...
# Recognizable credentials in third-party messages / exception strings. Unknown
# unlabelled secrets cannot be identified reliably: never log arbitrary payloads.
...
_CREDENTIAL_ASSIGNMENT_RE = re.compile(
    r"(?<![\w.-])(?P<prefix>[\"']?(?P<key>[\w.-]+)[\"']?\s*[:=]\s*)"
)
_CREDENTIAL_VALUE_RE = re.compile(r"(?:\"(?:\\.|[^\"\\])*\"|'(?:\\.|[^'\\])*'|[^\s&,;\"'<>}\]]+)")


def _redact_string(value: str) -> str:
    value = _URL_USERINFO_RE.sub(r"\1" + REDACTED + "@", value)
    value = _HEADER_CREDENTIAL_RE.sub(REDACTED, value)
    value = _BEARER_RE.sub("Bearer " + REDACTED, value)
    # Scan keys once, without consuming a non-sensitive value (e.g. https:).
    # Avoid a sensitive-word alternation between two greedy identifier scans:
    # repeated "token" in a long message otherwise causes quadratic backtracking.
    parts: list[str] = []
    end = 0
    for match in _CREDENTIAL_ASSIGNMENT_RE.finditer(value):
        if match.start() < end or not _is_sensitive_key(match["key"]):
            continue
        credential = _CREDENTIAL_VALUE_RE.match(value, match.end())
...
def _redact_value(value: Any) -> Any:
    """Mask sensitive fields and recognizable credentials in nested strings."""
...
def configure_logging(
    ...
    # Materialize exceptions before sanitization; never collect frame locals.
    # Text mode also consumes exc_info here, so ConsoleRenderer cannot bypass
    # sanitizers through its optional rich/better-exceptions renderer.
    if use_json:
    ...
    shared_processors.extend(
        [redact_secrets_processor, mask_pii_processor, truncate_large_values_processor]
    )
