...
def _sanitize_html(value: str | None, *, url_schemes: set[str]) -> str:
    """Clean HTML with an explicit URL-scheme policy."""
...
def sanitize_html(value: str | None) -> str:
    ...
    sanitizer entirely.
    """
    return _sanitize_html(value, url_schemes=_URL_SCHEMES)


def sanitize_email_html_for_localization(value: str | None) -> str:
    """Sanitize inbound email HTML while temporarily preserving ``cid:`` images.

    This is an intermediate value only. Helpdesk ingress rewrites exact
    Content-ID references to authenticated local attachment URLs and calls
    :func:`sanitize_html` again before persisting the message.
    """

    return _sanitize_html(value, url_schemes=_URL_SCHEMES | {"cid"})
