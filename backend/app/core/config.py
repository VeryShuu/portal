...
class Settings(BaseSettings):
    ...
    # ── Модуль обучения (LMS): параметры brute-force защиты внешних учёток ──
    # Passwordless-вход (миграция 113): одноразовый код из письма. Значения
    # bootstrap (env), перевод в runtime-настройки Admin UI — при появлении
    # потребности менять без рестарта.
    learning_code_ttl_minutes: int = Field(default=10, ge=1, le=60)
    learning_code_max_attempts: int = Field(default=5, ge=1, le=10)
    # Напоминание о дедлайне курса (этап 2, §15): за сколько суток до
    # deadline_at ежедневный cron шлёт непрошедшим участникам одно письмо.
    learning_deadline_reminder_days: int = Field(default=3, ge=0, le=60)
    # Пароль DB-роли learning_app (§10.8 ТЗ learning.md): отдельный пул
    # learning-роутов с грантами только на learning_* + INSERT email_outbox
    # (миграция 102). Пусто → learning-роуты на общем движке (dev/тесты);
    # на проде переменная обязательна.
    learning_db_password: str = Field(default="", repr=False)
    ...
    def _validate_production_secrets(self) -> Settings:
        ...
        if not self.learning_db_password:
            # audit [PA-002]: fail-closed. Без пароля роли learning_app
            # learning-роуты молча уходят на общий привилегированный пул
            # (app/core/database.py::_learning_database_url) — изоляция грантов
            # learning_app (миграция 102) не действует, а ошибка конфигурации
            # не видна. На проде переменная обязательна; роль синхронизирует
            # scripts/ensure_learning_db_role.py (migrate.sh).
            raise ValueError(
                "LEARNING_DB_PASSWORD is required in production: without it learning "
                "routes silently use the privileged portal DB pool (no learning_app "
                "role isolation). Generate one: `openssl rand -hex 24`."
            )
        return self
