...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    from app.middleware.metrics import start_db_pool_updater, stop_db_pool_updater

    # Per-process DB-pool telemetry (MON-09): каждый uvicorn-воркер обновляет
    # свои liveall-ряды пула независимо от того, кто обслуживает /metrics.
    start_db_pool_updater()

    try:
        yield
    finally:
        logger.info("portal.shutdown")
        await stop_db_pool_updater()
        ...
        # Multiprocess Prometheus: пометить pid мёртвым, чтобы live-агрегации
        # MultiProcessCollector (напр. livesum у instrumentator) не учитывали
        # mmap-файл этого воркера после graceful-рестарта (uvicorn respawn).
        # См. app/core/metrics.py и Dockerfile (PROMETHEUS_MULTIPROC_DIR).
