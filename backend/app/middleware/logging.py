...
async def request_logging(
    ...
    - Необработанное исключение логируется через exception().
    - ContextVars очищаются в finally — ПОСЛЕ логирования http.request:
      финальная access-log строка сохраняет correlation-контекст
    ...
    try:
        ...
        elif request.url.path == NOTIFICATIONS_STREAM_PATH:
            # SSE-коннект живёт ~60 c до реконнекта — elapsed это время жизни
            # соединения, а не латентность обработки; на INFO каждый реконнект
            # классифицировался бы как slow-request warning и засорял Loki.
            log_method = logger.debug
        else:
            from app.core.system_config import load_system_settings

            _slow_ms = load_system_settings().log_slow_request_ms
            log_method = logger.warning if _slow_ms > 0 and elapsed_ms >= _slow_ms else logger.info

        # Пишем ДО finally: clear_request_context() ниже стирает contextvars,
        # и http.request, залогированный после очистки, терял request_id/
        # method/path/client_ip — корреляция финальной строки ломалась.
        log_method("http.request", status_code=sc, elapsed_ms=elapsed_ms)
