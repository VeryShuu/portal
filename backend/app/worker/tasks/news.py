...
async def sync_users_from_keycloak(ctx: dict) -> int:
    ...
    try:
        ...
        # Обязательные курсы «для всех сотрудников» (learning, миграция 111):
        # после синка дозачисляем сотрудников без явной строки — новые
        # сотрудники покрываются первым же прогоном после создания в Keycloak.
        # Письма не рассылаются; доступ и до этой вставки виртуальный.
        try:
            from app.services.learning.for_all_staff import enroll_missing_staff

            enrolled = await enroll_missing_staff(conn)
            if enrolled:
                logger.info("learning.for_all_staff_enrolled", count=int(enrolled))
        except Exception as exc:
            # не роняем сам sync: дозачисление повторится на следующем прогоне
            logger.warning(
                "learning.for_all_staff_enroll_failed",
                error=str(exc),
                error_type=type(exc).__name__,
            )
