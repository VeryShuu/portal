...
from app.worker.tasks.directum_sync import directum_watchdog, run_directum_sync
...
from app.worker.tasks.learning_cleanup import cleanup_expired_resets
...
from app.worker.tasks.meetings.rsvp_digest import send_rsvp_digest, send_rsvp_final_digest
...
from app.worker.tasks.runs_cleanup import cleanup_module_runs
...
# Cron-задачи, которые НЕ инструментируем (те же, что и в functions):
# сами сборщики метрик/heartbeat — их обёртка создавала бы рекурсивный шум
# и маскировала stale-детектор (PortalWorkerStale).
_CRON_UNINSTRUMENTED = frozenset(
    {
...
def tracked_cron(fqn: str, **kwargs: Any) -> CronJob:
    """Строковый cron() с инструментацией track_arq_job (PA-021).

    cron("<fqn>") резолвит raw-coroutine и регистрирует её под именем
    'cron:<fqn>': scheduled-прогоны шли мимо track_arq_job и не попадали в
    portal_arq_jobs_total — падения cron-задач не могли зажечь
    PortalArqJobFailures. Здесь coroutine оборачивается, а имя регистрации
    сохраняется ('cron:<fqn>' — уникальность/job_id/реестр ARQ не меняются);
    метрики пишутся под коротким именем функции, как у ручных enqueue.
    """
    if fqn in _CRON_UNINSTRUMENTED:
        return cron(fqn, **kwargs)
    return cron(track_arq_job(import_string(fqn)), name=f"cron:{fqn}", **kwargs)  # type: ignore[arg-type]
...
class WorkerSettings:
    ...
    functions = [
        ...
        track_arq_job(poll_meetings_rsvp),
        track_arq_job(send_rsvp_digest),
        track_arq_job(send_rsvp_final_digest),
        ...
        track_arq_job(run_directum_sync),
        track_arq_job(directum_watchdog),
        track_arq_job(cleanup_module_runs),
        ...
        track_arq_job(process_deadline_reminders),
        track_arq_job(cleanup_expired_resets),
    ]
    cron_jobs = [
        tracked_cron(
        ...
        tracked_cron(
            # Напоминания о дедлайне курса (этап 2): раз в сутки утром,
            # письма уходят через outbox-диспетчер.
            "app.worker.tasks.learning_deadlines.process_deadline_reminders",
            hour=7,
            minute=15,
            second=0,
        ),
        tracked_cron(
            # Ретеншен одноразовых токенов восстановления (learning): после
            # дедлайн-напоминаний, тот же суточный ритм.
            "app.worker.tasks.learning_cleanup.cleanup_expired_resets",
            hour=7,
            minute=45,
            second=0,
        ),
        ...
        tracked_cron(
            # Суточный пересчёт размера фотохранилища (дорогой rglob): в
            # refresh_custom_metrics каждые 30 секунд НЕ выполняется (аудит
            # 2026-08: сам мониторинг грузил диски). 04:35 — после
            # cleanup_deleted_photos (04:00), до утреннего пика.
            "app.worker.tasks.metrics.refresh_photo_storage",
            hour=4,
            minute=35,
            second=0,
            run_at_startup=True,
        ...
        # ── Meetings RSVP ───────────────────────────────────────────────────
        # Приём Accept/Decline из общего ящика портала (ORGANIZER = portal@).
        # Гейты внутри: meetings.enabled + rsvp_ingest_enabled + IMAP настроен;
        # interval-guard 60 c (RSVP_POLL_INTERVAL_SECONDS).
        tracked_cron(
            "app.worker.tasks.meetings.rsvp_poll.poll_meetings_rsvp",
            second={0, 30},
        ),
        # 30-минутная сводка ответов организаторам (окно — Redis-ключ).
        tracked_cron(
            "app.worker.tasks.meetings.rsvp_digest.send_rsvp_digest",
            minute={0, 30},
            second=10,
        ),
        # Финальная сводка за 15 минут до встречи (жёсткое минутное окно).
        tracked_cron(
            "app.worker.tasks.meetings.rsvp_digest.send_rsvp_final_digest",
            minute=None,
            second=45,
        ),
        tracked_cron(
        ...
        # ── Directum (СЭД): просроченные задачи → Matrix ─────────────────────
        # Cron тикает ежечасно в :17 UTC (= :17 MSK, минута не зависит от
        # сдвига на целые часы); реальное расписание — часы запуска из
        # directum_settings.overdue_run_hours (московское время, проверка в
        # самой задаче). Дедуп часа — Redis-ключ directum:last_run_hour.
        tracked_cron(
            "app.worker.tasks.directum_sync.run_directum_sync",
            minute=17,
            second=0,
        ),
        # Watchdog: раз в день; 09:10 — после erp-ватчдогов, к началу дня.
        tracked_cron(
            "app.worker.tasks.directum_sync.directum_watchdog",
            hour=9,
            minute=10,
            second=0,
        ),
        # ── Ретеншн историй/очередей (решение 2026-08-17: 7 дней, хардкод) ──
        # История прогонов (erp_sync/erp_absences/directum) старше 7 дней.
        tracked_cron(
            "app.worker.tasks.runs_cleanup.cleanup_module_runs",
            hour=4,
            minute=40,
            second=0,
        ),
