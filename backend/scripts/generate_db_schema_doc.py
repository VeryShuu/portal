...
import app.models  # noqa: F401  # импортирует ВСЕ model-модули (как migrations/env.py)
...
def _computed_str(c: Any) -> str:
    # repr(Computed) содержит адрес объекта → недетерминированный вывод.
    text = getattr(c.sqltext, "text", None)
    return f"`computed({text if text is not None else c.sqltext})`"


def _default_str(col: sa.Column) -> str:
    if col.server_default is not None:
        sd = col.server_default
        if isinstance(sd, sa.Computed):
            return _computed_str(sd)
        if hasattr(sd, "arg"):
            arg = sd.arg
            if hasattr(arg, "text"):
                return f"`{arg.text}`"
    ...
    if col.default is not None:
        d = col.default
        if isinstance(d, sa.Computed):
            return _computed_str(d)
        if hasattr(d, "arg"):
            arg = d.arg
            if callable(arg):
                return "<function>"
...
def _table_section(table: sa.Table, mapper_map: dict[str, Any]) -> str:
    ...
    # table.constraints — set: порядок итерации нестабилен между запусками.
    constraints = sorted(
        (
            c
        ...
        key=lambda c: ((c.name or ""), type(c).__name__),
    )
    if constraints:
        lines.append("\n### Constraints\n")
