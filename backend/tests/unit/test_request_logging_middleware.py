...
3. Выбирает уровень лога по status_code: 5xx → error, 4xx → warning,
   SSE-стрим уведомлений (2xx) → debug (время жизни соединения, не slow-request),
   иначе info (с slow-request branch: если elapsed >= log_slow_request_ms — warning).
...
def _make_app(handler: Callable[..., Awaitable[object]], path: str = "/probe") -> FastAPI:
...
# ---------------------------------------------------------------------------
# 3b. SSE-стрим уведомлений: длительность = время жизни соединения (~60 c),
# а не slow-request. 2xx → debug (не warning, не попадает в Loki при info-уровне);
# ошибки (429/5xx) логируются как обычно.
# ---------------------------------------------------------------------------


@pytest.mark.usefixtures("reset_contextvars")
def test_sse_2xx_logs_as_debug_even_when_slow() -> None:
    """SSE-коннект с elapsed > log_slow_request_ms → DEBUG, не warning."""

    async def handler():  # type: ignore[no-untyped-def]
        return {"ok": True}

    app = _make_app(handler, path="/api/v1/notifications/stream")
    mock_log, methods = _stub_logger()

    # Порог 10 ms — SSE-elapsed (~60 c) всегда «slow»; без SSE-ветки был бы warning.
    fake_settings = type("FakeSettings", (), {"log_slow_request_ms": 10})()
    with (
        patch.object(middleware_logging, "logger", mock_log),
        patch("app.core.system_config.load_system_settings", return_value=fake_settings),
        ...
        client.get("/api/v1/notifications/stream")

    methods["debug"].assert_called_once()
    methods["warning"].assert_not_called()
    methods["info"].assert_not_called()
    methods["error"].assert_not_called()
...
@pytest.mark.usefixtures("reset_contextvars")
def test_sse_4xx_still_logs_as_warning() -> None:
    """Ошибки стрима (лимит соединений 429) остаются видимыми — warning."""
    ...
    methods["warning"].assert_called_once()
    methods["debug"].assert_not_called()
    methods["info"].assert_not_called()
...
# ---------------------------------------------------------------------------
# 6. Correlation-контекст в момент логирования http.request
# (регрессия: http.request писался ПОСЛЕ clear_request_context() в finally —
# финальная строка теряла request_id/method/path/client_ip)
# ---------------------------------------------------------------------------


@pytest.mark.usefixtures("reset_contextvars")
def test_http_request_logged_with_bound_context() -> None:
    """В момент вызова logger.info('http.request') contextvars ещё забиндены:
    request_id/method/path/client_ip попадают в финальную access-log строку."""
    ...
    def _capture(*args: object, **kwargs: object) -> None:
        snapshots.append(dict(structlog.contextvars.get_contextvars()))
    ...
    app = _make_app(handler)
    mock_log, methods = _stub_logger()
    methods["info"].side_effect = _capture
    with patch.object(middleware_logging, "logger", mock_log), TestClient(app) as client:
        resp = client.get(
            "/probe",
            headers={"X-Request-Id": "ctx-probe-42", "X-Real-IP": "192.168.10.77"},
        )

    assert resp.status_code == 200
    methods["info"].assert_called_once()
    assert snapshots, "logger.info не вызывался — снимок контекста не сделан"
    ctx = snapshots[0]
    assert ctx.get("request_id") == "ctx-probe-42"
    assert ctx.get("method") == "GET"
    assert ctx.get("path") == "/probe"
    # client_ip маскируется (последний октет) до биндинга — см. audit [H9].
    assert ctx.get("client_ip") == "192.168.10.x"


@pytest.mark.usefixtures("reset_contextvars")
def test_http_request_failed_logged_with_bound_context() -> None:
    """Путь исключения: http.request_failed тоже логируется с живым контекстом."""
    ...
    app = _make_app(handler)
    mock_log, methods = _stub_logger()
    methods["exception"].side_effect = _capture
    with (
        patch.object(middleware_logging, "logger", mock_log),
        pytest.raises(RuntimeError, match="boom"),
        TestClient(app) as client,
    ):
        client.get("/probe", headers={"X-Request-Id": "exc-probe-7"})

    methods["exception"].assert_called_once()
    assert snapshots
    assert snapshots[0].get("request_id") == "exc-probe-7"
    assert snapshots[0].get("path") == "/probe"


@pytest.mark.usefixtures("reset_contextvars")
def test_context_cleared_after_request_completes() -> None:
    """После ответа contextvars очищены — следующий запрос не наследует контекст."""
    ...
    app = _make_app(handler)
    with TestClient(app) as client:
        client.get("/probe", headers={"X-Request-Id": "leak-probe"})
