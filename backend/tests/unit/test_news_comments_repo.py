...
Контракты:
* ``count_comments`` — ``scalar_one()`` (COUNT всегда возвращает одну строку),
  без фильтра deleted — согласовано с ``list_comments`` (PA-014);
...
@pytest.mark.asyncio
async def test_count_comments_returns_scalar_one():
    db = _make_db()
    db.execute.return_value = _result_scalar(42)

    assert await repo.count_comments(db, uuid.uuid4()) == 42
    db.execute.assert_awaited_once()


@pytest.mark.asyncio
async def test_count_comments_zero():
    db = _make_db()
    db.execute.return_value = _result_scalar(0)

    assert await repo.count_comments(db, uuid.uuid4()) == 0


@pytest.mark.asyncio
async def test_count_comments_includes_deleted_placeholders():
    """PA-014: count НЕ фильтрует deleted_at — total обязан соответствовать
    выдаче list_comments, которая возвращает placeholders. Регрессия старого
    поведения фиксируется по скомпилированному SQL: критерия deleted_at в
    SELECT COUNT нет."""
    from sqlalchemy import Select

    db = _make_db()
    db.execute.return_value = _result_scalar(5)

    await repo.count_comments(db, uuid.uuid4())
    (stmt,) = db.execute.await_args.args
    compiled = str(stmt.compile(compile_kwargs={"literal_binds": True}))
    assert isinstance(stmt, Select)
    assert "deleted_at" not in compiled, "count не должен фильтровать soft-deleted placeholders"
