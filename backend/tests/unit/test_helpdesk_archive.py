...
def _make_db(ticket_rows: list) -> MagicMock:
    ...
    Первый ``execute`` после count возвращает batch тикетов, затем — пустой
    batch. Сообщения/вложения уже eagerly-loaded в relationship тикета.
    ``add``/``delete``/``flush``/``commit`` — AsyncMock/MagicMock.
    """
    db = MagicMock()
    db.execute = AsyncMock(side_effect=[_make_result(ticket_rows), _make_result([])])
    db.scalar = AsyncMock(return_value=len(ticket_rows))
    db.add = MagicMock()
    db.delete = AsyncMock()
    db.flush = AsyncMock()
    db.commit = AsyncMock()
...
@pytest.mark.asyncio
async def test_archive_rejects_non_positive_batch_size() -> None:
    with pytest.raises(ValueError, match="batch_size must be positive"):
        await archive_closed_tickets(_make_db([]), batch_size=0)


@pytest.mark.asyncio
async def test_archive_commits_each_batch_and_uses_fixed_query_count() -> None:
    """Backlog is split into short transactions, not one 2N+1 transaction."""
    first = _make_ticket_row(number=101)
    second = _make_ticket_row(number=102)
    db = _make_db([])
    db.scalar = AsyncMock(return_value=2)
    db.execute = AsyncMock(
        side_effect=[_make_result([first]), _make_result([second]), _make_result([])]
    )

    archived = await archive_closed_tickets(db, batch_size=1)
