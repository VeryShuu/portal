...
class TestBuildIcal:
    ...
    def test_organizer_uses_organizer_email_not_portal_address(self):
        from app.services.meetings.ical_builder import build_ical

        room = _make_room()
        booking = _make_booking(rooms=[room])
        raw = build_ical(booking, "REQUEST", "portal.local", "alice@corp.local").decode("utf-8")
        text = raw.replace("\r\n ", "")  # unfold
        # ORGANIZER по RFC 5545 — адрес, собирающий RSVP: это email создателя
        # встречи, а не адрес портала (иначе Accept/Decline уходят «в никуда»).
        assert "ORGANIZER;CN=Alice:mailto:alice@corp.local" in text
        assert "noreply@portal.local" not in text
