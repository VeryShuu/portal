...
# LEARNING_DB_PASSWORD обязателен в production с audit [PA-002] — production-тесты
# задают его явно; отсутствие проверяет негативный тест ниже.
_VALID_PROD_LEARNING_PW = "learning-role-password"
...
def test_production_flag(monkeypatch):
    env = make_env(
        ...
        LEARNING_DB_PASSWORD=_VALID_PROD_LEARNING_PW,
    )
    for k, v in env.items():
        monkeypatch.setenv(k, v)
...
def test_production_requires_learning_db_password(monkeypatch):
    """audit [PA-002]: в production пустой LEARNING_DB_PASSWORD отклоняется —
    иначе learning-роуты молча уходят на общий привилегированный пул и
    изоляция грантов роли learning_app (миграция 102) не действует."""
    env = make_env(
        ENVIRONMENT="production",
        SECRET_KEY=_VALID_PROD_SECRET,
    ...
    for k, v in env.items():
        monkeypatch.setenv(k, v)
    monkeypatch.delenv("LEARNING_DB_PASSWORD", raising=False)

    from app.core.config import Settings

    with pytest.raises(ValidationError) as exc_info:
        Settings()
    assert "LEARNING_DB_PASSWORD is required in production" in str(exc_info.value)


def test_dev_accepts_empty_learning_db_password(monkeypatch):
    """Регресс: dev/staging/test работают без LEARNING_DB_PASSWORD (общий
    движок) — fail-closed валидатор не должен срабатывать вне production."""
    env = make_env(ENVIRONMENT="development")
    for k, v in env.items():
        monkeypatch.setenv(k, v)
    monkeypatch.delenv("LEARNING_DB_PASSWORD", raising=False)
