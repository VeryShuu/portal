...
def _make_scalar_row(value: int = 0):
    ...
    row.wau_7d = value
    row.mau_30d = value
    # approvals-блок дашборда (fetch_approvals_usage -> .one() того же мока)
    row.approved = value
    row.rejected = value
    row.active_users = value
...
async def test_dashboard_approvals_section_schema(app, user_factory):
    """Блок использования «Согласования 1С» (события approvals.* из audit_log)."""
    _authed_admin_app(app, user_factory)
    transport = ASGITransport(app=app)
    async with AsyncClient(
        transport=transport, base_url="http://test", headers={"Origin": "http://test"}
    ...
    assert approvals["rejected"] == 0
    assert approvals["active_users"] == 0
    assert approvals["last_event_at"] is None
    assert isinstance(approvals["daily"], list)
