...
class TestTicketAwareQuoteStripping:
    def test_plain_preserves_nested_forward_before_portal_quote(self) -> None:
        body = (
            "Прикладываю дополнительный контекст.\n\n"
            "-----Original Message-----\n"
            "From: vendor@example.test\n"
            "Ошибка внешней системы\n\n"
            "-----Original Message-----\n"
            "From: support@mage.ru\n"
            "Subject: [#TKT-42] Ответ поддержки\n"
            "Старая история тикета\n"
        )
        out = strip_quoted_reply(body, ticket_number=42)
        assert "Ошибка внешней системы" in out
        assert out.count("-----Original Message-----") == 1
        assert "Старая история тикета" not in out

    def test_plain_preserves_ambiguous_forward_without_ticket_fingerprint(self) -> None:
        body = (
            "Смотрите ниже.\n\n-----Original Message-----\n"
            "From: vendor@example.test\nquota exceeded\n"
        )
        assert strip_quoted_reply(body, ticket_number=42) == body

    def test_html_preserves_nested_forward_before_portal_quote(self) -> None:
        body = (
            "<p>Новый контекст</p>"
            "<div>-----Original Message-----</div>"
            "<p>Ошибка внешней системы</p>"
            "<div>-----Original Message-----</div>"
            "<div>Subject: [#TKT-42] Ответ поддержки</div>"
            "<p>Старая история тикета</p>"
        )
        out = strip_quoted_html(body, ticket_number=42)
        assert "Ошибка внешней системы" in out
        assert "Старая история тикета" not in out
