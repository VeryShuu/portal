...
async def test_create_folder_already_exists_405():
    ...
    # Контракт: один MKCOL по WebDAV-URL целевой папки (не «просто не упало»).
    mock_http_client.request.assert_awaited_once()
    args = mock_http_client.request.await_args.args
    assert args[0] == "MKCOL"
    assert args[1] == ("https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/Existing")
...
async def test_delete_success_204():
    ...
    assert args[1] == (
        "https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/file.xlsx"
    )
...
async def test_delete_not_found_404_silent():
    ...
    # 404 — молча, но запрос ровно один и по правильному пути.
    mock_http_client.request.assert_awaited_once()
    args = mock_http_client.request.await_args.args
    assert args[0] == "DELETE"
    assert args[1] == (
        "https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/already_deleted.xlsx"
    )
...
async def test_move_success_201():
    ...
    _assert_move_request(mock_http_client)
...
def _assert_move_request(mock_http_client: AsyncMock) -> None:
    """Контракт MOVE: метод, исходный URL и Destination/Overwrite-заголовки."""
    mock_http_client.request.assert_awaited_once()
    call = mock_http_client.request.await_args
    assert call.args[0] == "MOVE"
    assert call.args[1] == (
        "https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/old.xlsx"
    )
    headers = call.kwargs["headers"]
    assert headers["Destination"] == (
        "https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/new.xlsx"
    )
...
async def test_download_stream_success():
    ...
    with patch("app.services.nextcloud.webdav.httpx.AsyncClient", return_value=mock_http_client):
        r, _c = await client.download_stream("HR/doc.pdf")

    # Контракт стриминга: GET по DAV-URL (nc_path в БД хранится относительно
    # files_root, без префикса PortalFiles — см. folders.py::create_folder),
    # send со stream=True и собранным request.
    mock_http_client.build_request.assert_called_once()
    build_args = mock_http_client.build_request.call_args.args
    assert build_args[0] == "GET"
    assert build_args[1] == (
        "https://nc.example.com/remote.php/dav/files/portal/PortalFiles/HR/doc.pdf"
    )
    send_args = mock_http_client.send.await_args.args
    assert send_args[0] is mock_http_client.build_request.return_value
    assert mock_http_client.send.await_args.kwargs.get("stream") is True
