...
Пять зон проверки:
1. ``threading.extract_cc`` — парсинг заголовков ``To`` ∪ ``Cc`` входящего
   письма (фикс TKT-269: коллега в «Кому» рядом с ящиком поддержки раньше
   терялся, т.к. читался только ``Cc``).
1b. ``services.helpdesk.ingress._parse_inbound_headers`` — проводка
   отправителя/``support_reply_to`` в исключения участников.
...
class TestExtractCcToHeader:
    """``To`` ∪ ``Cc`` — фикс прод-тикета TKT-269 (2026-08-26).

    Заявительница в Outlook вписала ящик поддержки **и** коллегу оба в поле
    «Кому» (``To``), без ``Cc``. Старый ``extract_cc`` читал только ``Cc`` →
    коллега не попадал ни в ``cc`` сообщения, ни в ``participants`` карточки,
    и «Ответить всем» его не получал. Участники = ``To ∪ Cc`` независимо от
    того, куда отправитель вписал адресатов.
    """

    def test_to_only_recipient_captured(self) -> None:
        out = extract_cc(_msg({"To": "colleague@company.local"}))
        assert out == [CcRecipient(email="colleague@company.local", name=None)]

    def test_prod_ticket_269_outlook_folded_to(self) -> None:
        # Реальный заголовок из письма TKT-269 (Outlook 16, koi8-r): ``To``
        # свёрнут в две строки (continuation-строка с табом), первый адресат —
        # ящик поддержки без display-name, второй — с koi8-r encoded-word.
        # Заголовка ``Cc`` в письме не было.
        msg = _msg(
            {
                "From": "=?koi8-r?B?5M/Ny8nOwSD3wczFztTJzsEg98HMxdLYxdfOwQ==?= <domkina.vv@mage.ru>",
                "To": (
                    "<it@mage.ru>,\n"
                    "\t=?koi8-r?B?88HXxczYxdcg5MXOydMg4czFy9PBzsTSz9fJ3g==?= <savelev.da@mage.ru>"
                ),
            }
        )
        assert extract_cc(msg, exclude="it@mage.ru") == [
            CcRecipient(email="savelev.da@mage.ru", name="Савельев Денис Александрович"),
        ]

    def test_to_and_cc_merged_with_dedup(self) -> None:
        # Один адрес в To и Cc — один участник.
        msg = _msg({"To": "a@x.local, b@x.local", "Cc": "b@x.local, c@x.local"})
        assert [p.email for p in extract_cc(msg)] == ["a@x.local", "b@x.local", "c@x.local"]

    def test_order_to_first_then_cc(self) -> None:
        msg = _msg({"Cc": "cc-first@x.local", "To": "to-first@x.local"})
        assert [p.email for p in extract_cc(msg)] == ["to-first@x.local", "cc-first@x.local"]

    def test_sender_excluded(self) -> None:
        # Отправитель вписал себя в получатели — не дублируем: он уже заявитель.
        msg = _msg({"From": "f@x.local", "To": "f@x.local, other@x.local"})
        out = extract_cc(msg, exclude_sender="f@x.local")
        assert out == [CcRecipient(email="other@x.local", name=None)]

    def test_support_plus_alias_excluded(self) -> None:
        # it+TKT-123@mage.ru — plus-алиас ящика поддержки (RFC 5233,
        # marker из fallback-matching'а): не участник, иначе «Ответить всем»
        # уйдёт на marker-адрес и вернётся в IMAP мимо anti-loop.
        msg = _msg({"To": "it+TKT-123@mage.ru, man@x.local"})
        assert extract_cc(msg, exclude="it@mage.ru") == [
            CcRecipient(email="man@x.local", name=None)
        ]

    def test_exclude_iterable_support_and_reply_to(self) -> None:
        # support_address + support_reply_to — оба выкидываются (ответ клиента
        # адресуется на Reply-To алиас, это сам support-контур).
        msg = _msg({"To": "it@mage.ru, alias@mage.ru, man@x.local"})
        out = extract_cc(msg, exclude=["it@mage.ru", "alias@mage.ru"])
        assert out == [CcRecipient(email="man@x.local", name=None)]

    def test_exclude_string_still_supported(self) -> None:
        # Обратная совместимость: exclude как строка (старые вызовы/тесты).
        msg = _msg({"To": "it@mage.ru, man@x.local"})
        assert extract_cc(msg, exclude="it@mage.ru") == [
            CcRecipient(email="man@x.local", name=None)
        ]

    def test_no_address_headers(self) -> None:
        assert extract_cc(_msg({"Subject": "s"})) == []


# ──────────────────────────────────────────────────────────────────────────────
# 1b. ingress._parse_inbound_headers — участники To ∪ Cc
# ──────────────────────────────────────────────────────────────────────────────

from app.services.helpdesk.ingress import _parse_inbound_headers


class TestParseInboundHeadersParticipants:
    def test_prod_ticket_269_layout(self) -> None:
        # Письмо-заявка: From с koi8-r именем, To = ящик поддержки + коллега.
        msg = _msg(
            {
                "From": "=?koi8-r?B?5M/Ny8nOwSD3wczFztTJzsEg98HMxdLYxdfOwQ==?= <domkina.vv@mage.ru>",
                "To": (
        ...
        headers = _parse_inbound_headers(msg, support_address="it@mage.ru")
        assert headers["sender_email"] == "domkina.vv@mage.ru"
        assert headers["sender_name"] == "Домкина Валентина Валерьевна"
        assert headers["cc"] == [
            {"email": "savelev.da@mage.ru", "name": "Савельев Денис Александрович"}
        ]

    def test_reply_to_alias_excluded(self) -> None:
        # При настроенном support_reply_to ответы клиентов адресуются на алиас —
        # это сам support-контур, не участник переписки.
        msg = _msg({"From": "client@x.local", "To": "reply@mage.ru, man@x.local"})
        headers = _parse_inbound_headers(
            msg, support_address="it@mage.ru", reply_to_address="reply@mage.ru"
        )
        assert headers["cc"] == [{"email": "man@x.local", "name": None}]

    def test_sender_self_in_recipients_excluded(self) -> None:
        # Отправитель в To (отправил копию себе) — не попадает в cc.
        msg = _msg({"From": "client@x.local", "To": "it@mage.ru, client@x.local"})
        headers = _parse_inbound_headers(msg, support_address="it@mage.ru")
        assert headers["cc"] == []

    def test_no_recipients_besides_support(self) -> None:
        # Обычное письмо только на ящик поддержки — cc пуст (как раньше).
        msg = _msg({"From": "client@x.local", "To": "it@mage.ru"})
        headers = _parse_inbound_headers(msg, support_address="it@mage.ru")
        assert headers["cc"] == []
