...
class TestExtractInlineParts:
    ...
    def test_skips_inline_images_inside_attached_rfc822(self) -> None:
        raw = (
            "MIME-Version: 1.0\r\n"
...
class TestLocalizeImages:
    ...
    async def test_orphan_img_is_not_positionally_matched(self) -> None:
        """An unrelated inline part must not be injected into a src-less image."""
        att_id = uuid.uuid4()
        spy = _save_spy(att_id)
        inline_map = {
        ...
        assert spy.call_args.kwargs["is_inline"] is False
        assert spy.call_args.kwargs["content_id"] == "screenshot@outlook"
...
class TestInlineFlagMarking:
    ...
    async def test_unreferenced_inline_is_preserved_as_regular_attachment(self) -> None:
        """Ambiguous inline content remains downloadable without DOM guessing."""
        spy = _save_spy(uuid.uuid4())
        inline_map = {
        ...
        spy.assert_called_once()
        kwargs = spy.call_args.kwargs
        assert kwargs["is_inline"] is False
        assert kwargs["content_id"] == "shot@outlook"

    async def test_unreferenced_known_signature_logo_is_discarded(self) -> None:
        spy = _save_spy(uuid.uuid4())
        inline_map = {
            "sig": InlineImage(data=b"png", content_type="image/png", filename="Mage_Ru.png")
        }
...
class TestFetchRemoteSizeCap:
    ...
    async def test_oversize_content_length_rejected_without_streaming_body(self) -> None:
        ...
        resp.aiter_raw = _spy_aiter_raw  # type: ignore[assignment]
