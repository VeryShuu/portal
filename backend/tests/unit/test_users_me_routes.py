...
def _make_user(role: str = "reader", **kwargs) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        attributes={},
        preferences={},
        current_status=kwargs.get("current_status", "working"),
...
class TestDeleteAvatar:
    @pytest.mark.asyncio
    async def test_deletes_avatar_and_returns_user(self, authed_client_factory):
        user_id = uuid.uuid4()
        ac, _user = authed_client_factory(role="reader", id=user_id)
        ...
        with patch(
            "app.api.users.routes_me.users_me_service.delete_avatar",
            new=AsyncMock(return_value=updated_user),
        ):
            async with ac:
                resp = await ac.delete("/api/v1/users/me/avatar")
...
class TestPatchMyProfileFocal:
    """Фокал аватара через PATCH /me/profile: валидация диапазонов (422)."""

    @pytest.mark.asyncio
    async def test_valid_focal_accepted(self, authed_client_factory):
        user_id = uuid.uuid4()
        ac, _user = authed_client_factory(role="reader", id=user_id)
        updated_user = _make_user(role="reader", id=user_id)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("avatar_focal_x", 101),
            ("avatar_focal_y", -1),
            ("avatar_focal_zoom", 99),
            ("avatar_focal_zoom", 301),
        ],
    )
    async def test_out_of_range_rejected(self, authed_client_factory, field, value):
        ac, _user = authed_client_factory(role="reader")
        async with ac:
            resp = await ac.patch("/api/v1/users/me/profile", json={field: value})
...
class TestPatchMyPreferencesService:
    """Прямые тесты users_me_service.patch_my_preferences (без HTTP-слоя)."""

    @pytest.mark.asyncio
    async def test_persists_onboarding_completed_via(self):
        from app.api.users.users_me_service import patch_my_preferences
        from app.schemas.user import PatchPreferencesRequest
        ...
        saved_prefs = repo_update.call_args[0][2]["preferences"]
        assert saved_prefs["onboarding_completed"] is True
        assert saved_prefs["onboarding_completed_via"] == "finished"
        db.commit.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_via_kept_when_completed_not_resented(self):
        # PATCH без onboarding_completed не затирает ранее записанный маркер via
        from app.api.users.users_me_service import patch_my_preferences
        from app.schemas.user import PatchPreferencesRequest
        ...
        with patch(
            ...
            await patch_my_preferences(
                db, user, PatchPreferencesRequest(onboarding_seen_step_ids=["a"])
            )

        saved_prefs = repo_update.call_args[0][2]["preferences"]
        assert saved_prefs["onboarding_completed_via"] == "finished"
        assert saved_prefs["onboarding_seen_step_ids"] == ["a"]
