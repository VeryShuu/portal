...
class TestTryEnqueueOutbound:
    async def test_email_contains_reply_then_history(self) -> None:
        """Тело письма: separator → ответ агента → история."""
        prior = _msg()
        ...
        # Управляемый marker ставится до ответа и истории.
        assert body_text.index(REPLY_MARKER_TOKEN) < body_text.index("Ответ агентa.")
    ...
    async def test_no_history_for_first_reply(self) -> None:
        ...
        не добавляется. Separator всё равно нужен для следующего reply."""
        current = _current_message()
