...
def _extract_payloads(enqueue_mock, batch_mock) -> list[dict]:
    """Собирает payload outbox-строк из single- + batch-вызовов (audit M3)."""
    payloads = [c.kwargs.get("payload") or {} for c in enqueue_mock.call_args_list]
    for call in batch_mock.call_args_list:
        items = call.kwargs.get("items") or (call.args[1] if len(call.args) > 1 else [])
        payloads.extend(item.payload or {} for item in items)
    return payloads


class TestOrganizerReplyRouting:
    """ORGANIZER iCal и payload.reply_to указывают на email создателя встречи:
    кнопки Accept/Decline (IMIP REPLY уходит на ORGANIZER) и текстовые
    «Ответить» (Reply-To) доставляются организатору, а не на адрес портала.

    Без создателя (удалён / пустой email) — ORGANIZER = адрес портала и
    payload без reply_to (поведение до правки)."""
    ...
    async def test_organizer_email_in_ical_and_reply_to(self, enqueue_mocks, _patch_ical_builder):
        from app.services.meetings.notifications import enqueue_meeting_emails

        organizer = SimpleNamespace(email="boss@corp.local", full_name="Boss", notify_email=True)
        booking = _make_booking(room_email="room@x.com", invited_emails=["a@x.com"])
        with patch(
            "app.services.meetings.notifications._load_organizer",
            AsyncMock(return_value=organizer),
        ):
            await enqueue_meeting_emails(AsyncMock(), booking=booking, action="created")

        # Все iCal построены с ORGANIZER = email создателя встречи.
        for call in _patch_ical_builder.build_ical.call_args_list:
            assert call.kwargs.get("organizer_email") == "boss@corp.local"

        # Все письма (invited batch + organizer/rooms single) несут reply_to.
        single, batch = enqueue_mocks
        payloads = _extract_payloads(single, batch)
        assert payloads
        assert all(p.get("reply_to") == "boss@corp.local" for p in payloads)

    async def test_no_organizer_falls_back_to_portal_from(self, enqueue_mocks, _patch_ical_builder):
        from app.services.meetings.notifications import enqueue_meeting_emails
        ...
        for call in _patch_ical_builder.build_ical.call_args_list:
            assert call.kwargs.get("organizer_email") == "portal@c.local"

        single, batch = enqueue_mocks
        payloads = _extract_payloads(single, batch)
        assert payloads
        assert all("reply_to" not in p for p in payloads)

    async def test_rsvp_ingest_switches_organizer_to_portal(
        self, enqueue_mocks, _patch_ical_builder
    ):
        """rsvp_ingest_enabled=True → ORGANIZER = адрес портала (IMIP-ответы
        принимает поллер), Reply-To остаётся на создателе (текстовые ответы —
        ему лично)."""
        from app.services.meetings.notifications import enqueue_meeting_emails

        organizer = SimpleNamespace(email="boss@corp.local", full_name="Boss", notify_email=True)
        booking = _make_booking(room_email="room@x.com", invited_emails=["a@x.com"])
        mods = SimpleNamespace(meetings=SimpleNamespace(enabled=True, rsvp_ingest_enabled=True))
        with (
            patch(
                "app.services.meetings.notifications._load_organizer",
                AsyncMock(return_value=organizer),
            ),
            patch("app.core.modules_config.load_modules", return_value=mods),
        ):
            await enqueue_meeting_emails(AsyncMock(), booking=booking, action="created")
    ...
    async def test_rsvp_ingest_organizer_cn_uses_from_name(
        self, enqueue_mocks, _patch_ical_builder
    ):
        """rsvp_ingest + заданный display name → CN ORGANIZER = имя портала,
        а не ФИО создателя (клиенты рендерят отправителя по CN; «ФИО <portal@…>»
        провоцировал личные ответы на общий ящик)."""
        from app.services.meetings.notifications import enqueue_meeting_emails
        ...
        with (
            ...
            patch("app.core.modules_config.load_modules", return_value=mods),
            patch(
                "app.services.meetings.notifications._get_from_name",
                return_value="Портал Маге",
            ),
        ):
            await enqueue_meeting_emails(AsyncMock(), booking=booking, action="created")

        for call in _patch_ical_builder.build_ical.call_args_list:
            assert call.kwargs.get("organizer_cn") == "Портал Маге"

    async def test_rsvp_ingest_empty_from_name_falls_back_to_organizer_name(
        self, enqueue_mocks, _patch_ical_builder
    ):
        """from_name не задан → CN ORGANIZER = ФИО создателя (прежнее поведение)."""
        from app.services.meetings.notifications import enqueue_meeting_emails
        ...
        with (
            ...
            patch("app.core.modules_config.load_modules", return_value=mods),
            patch("app.services.meetings.notifications._get_from_name", return_value=""),
        ):
            await enqueue_meeting_emails(AsyncMock(), booking=booking, action="created")

        for call in _patch_ical_builder.build_ical.call_args_list:
            assert call.kwargs.get("organizer_cn") == booking.organizer_name

    async def test_empty_organizer_email_falls_back_too(self, enqueue_mocks, _patch_ical_builder):
        from app.services.meetings.notifications import enqueue_meeting_emails

        organizer = SimpleNamespace(email="", full_name="Boss", notify_email=True)
        booking = _make_booking(room_email="room@x.com", invited_emails=["a@x.com"])
        with patch(
            "app.services.meetings.notifications._load_organizer",
            AsyncMock(return_value=organizer),
