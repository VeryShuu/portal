...
def _make_db_user(
    ...
    u.avatar_url = None
    # Фокал аватара (миграция 096): явные значения обязательны — MagicMock
    # авто-создаёт отсутствующие атрибуты child-моками, на которых падает
    # pydantic-валидация UserPublic (int | None).
    u.avatar_focal_x = None
    u.avatar_focal_y = None
    u.avatar_focal_zoom = None
...
class TestPatchUserProfile:
    ...
    async def test_sso_user_focal_only_is_allowed(self, app, authed_client_factory):
        """Фокал аватара — настройка портала, для SSO-пользователей тоже доступна."""
        from app.api.deps import get_db
        ...
        try:
            r = await ac.patch(
                f"/api/v1/users/admin/{sso_user.id}/profile",
                json={"avatar_focal_x": 30, "avatar_focal_y": 70, "avatar_focal_zoom": 150},
            )
        finally:
            app.dependency_overrides.pop(get_db, None)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("avatar_focal_x", 101),
            ("avatar_focal_y", -1),
            ("avatar_focal_zoom", 301),
        ],
    )
    async def test_focal_out_of_range_rejected(self, authed_client_factory, field, value):
        ac, _ = authed_client_factory(role="admin")
        r = await ac.patch(
            f"/api/v1/users/admin/{uuid.uuid4()}/profile",
            json={field: value},
...
class TestAdminAvatarEndpoints:
    """POST/DELETE /users/admin/{id}/avatar — admin-управление аватарами."""

    async def test_non_admin_upload_gets_403(self, authed_client_factory):
        user_id = uuid.uuid4()
        for role in ("reader", "editor"):
            ac, _ = authed_client_factory(role=role)
            r = await ac.post(
                f"/api/v1/users/admin/{user_id}/avatar",
                files={"file": ("avatar.png", b"data", "image/png")},
            )
            assert r.status_code == 403, f"role={role} should be 403"

    async def test_non_admin_delete_gets_403(self, authed_client_factory):
        user_id = uuid.uuid4()
        for role in ("reader", "editor"):
            ac, _ = authed_client_factory(role=role)
            r = await ac.delete(f"/api/v1/users/admin/{user_id}/avatar")
            assert r.status_code == 403, f"role={role} should be 403"

    async def test_upload_user_not_found_returns_404(self, app, authed_client_factory):
        from app.api.deps import get_db
        ...
        try:
            r = await ac.post(
                f"/api/v1/users/admin/{uuid.uuid4()}/avatar",
                files={"file": ("avatar.png", b"data", "image/png")},
            )
        finally:
            app.dependency_overrides.pop(get_db, None)
    ...
    async def test_delete_user_not_found_returns_404(self, app, authed_client_factory):
        from app.api.deps import get_db

        app.dependency_overrides[get_db] = _make_fake_db(target_user=None)
        ac, _ = authed_client_factory(role="admin")
        try:
            r = await ac.delete(f"/api/v1/users/admin/{uuid.uuid4()}/avatar")
        finally:
            app.dependency_overrides.pop(get_db, None)
        assert r.status_code == 404

    async def test_upload_calls_service_and_emits_audit(self, app, authed_client_factory):
        target = _make_db_user(auth_source="keycloak")
        with (
            patch(
                "app.api.users.users_admin_service.avatar_service.save_avatar",
            ...
            patch(
                "app.api.users.users_admin_service._emit_audit",
                new=AsyncMock(),
            ) as audit_mock,
        ):
            ac, _ = authed_client_factory(role="admin")
            r = await ac.post(
                f"/api/v1/users/admin/{target.id}/avatar",
                files={"file": ("avatar.png", b"data", "image/png")},
            )
        assert r.status_code == 200
    ...
    async def test_delete_calls_service_and_emits_audit(self, app, authed_client_factory):
        target = _make_db_user(auth_source="keycloak")
        with (
            patch(
                "app.api.users.users_admin_service.avatar_service.remove_avatar",
                new=AsyncMock(return_value=target),
            ) as remove_mock,
            patch(
                "app.api.users.users_admin_service.users_repo.fetch_active_user",
            ...
            ac, _ = authed_client_factory(role="admin")
            r = await ac.delete(f"/api/v1/users/admin/{target.id}/avatar")
        assert r.status_code == 200
