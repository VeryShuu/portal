...
def _make_db_with_booking(booking=None):
    ...
    # _flush_or_conflict управляет savepoint вручную (без async with —
    # см. deadlock-фикс, audit-review 2026-08-22): begin_nested() ожидается
    # и возвращает объект с commit/rollback
    savepoint = AsyncMock()
...
class TestCreateBooking:
    ...
    async def test_create_booking_integrity_error_raises_conflict(self):
        ...
        # первый flush (строка брони) — ок; конфликт поднимается на втором,
        # внутри savepoint (_flush_or_conflict)
        db.flush = AsyncMock(side_effect=[None, IntegrityError("stmt", {}, Exception("unique"))])
    ...
    async def test_create_booking_deadlock_normalized_to_conflict(self):
        """40P01 на flush внутри savepoint → BookingConflict (аудит-review P1)."""
        import asyncpg
        ...
        db = _make_db_with_booking(booking=booking)
        deadlock = DBAPIError(
            "INSERT ...", [], asyncpg.exceptions.DeadlockDetectedError("deadlock detected")
        )
        db.flush = AsyncMock(side_effect=[None, deadlock])
        ...
        with (
            patch("app.services.meetings.bookings_service._crud._verify_rooms_active", verify_mock),
            patch(
                "app.services.meetings.bookings_service._crud._get_conflict_details",
                conflict_mock,
            ),
            pytest.raises(BookingConflict),
        ):
    ...
    async def test_create_booking_deadlock_on_begin_nested_normalized(self):
        """40P01 на autoflush внутри begin_nested() → BookingConflict."""
        import asyncpg
        from sqlalchemy.exc import DBAPIError
        ...
        db.begin_nested = AsyncMock(side_effect=deadlock)
    ...
    async def test_create_booking_nonretryable_dbapierror_propagates(self):
        """DBAPIError без retryable-sqlstate не нормализуется — пробрасывается."""
        from sqlalchemy.exc import DBAPIError
        ...
        db = _make_db_with_booking(booking=booking)
        other = DBAPIError("INSERT ...", [], RuntimeError("connection reset"))
        db.flush = AsyncMock(side_effect=[None, other])
