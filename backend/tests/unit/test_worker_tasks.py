...
class TestFlushAuditQueue:
    ...
    async def test_poison_record_quarantined_valid_still_flushed(self):
        """audit [PA-018]: malformed-запись не блокирует валидные — она
        изолируется в quarantine, валидные вставляются ровно один раз,
        processing-list очищается, следующий flush проходит."""
        from app.worker.tasks.audit import QUARANTINE_KEY, flush_audit_queue

        good = json.dumps({"event_type": "news.created", "user_id": "u1", "metadata": {}})
        poison_item = "{not-valid-json"
        ...
        async def mock_lrange(key, start, end):
            calls[0] += 1
            return [poison_item, good] if calls[0] == 1 else []
        ...
        assert result == 1, "валидная запись обязана дойти до PostgreSQL"
        redis.lrem.assert_awaited_once_with("audit_processing", 1, poison_item)
        redis.lpush.assert_awaited_once_with(QUARANTINE_KEY, poison_item)
        redis.ltrim.assert_awaited_once()
        rows = conn.executemany.await_args.args[1]
        assert len(rows) == 1
        assert rows[0][0] == "news.created"
        redis.delete.assert_awaited()  # processing-list очищен

    async def test_all_poison_batch_flushes_zero_and_clears_processing(self):
        from app.worker.tasks.audit import flush_audit_queue
        ...
        async def mock_lrange(key, start, end):
            calls[0] += 1
            return ["[1, 2]", "null"] if calls[0] == 1 else []  # валидный JSON, но не dict
...
class TestRefreshCustomMetrics:
    async def test_saves_snapshot_to_redis(self, tmp_path):
        ...
        # hgetall обязателен: без него await возвращает AsyncMock, чей .items()
        # даёт корутину → "not iterable" в коллекторах и не-awaited-корутины.
    ...
    async def test_photo_storage_from_daily_key(self):
        """Размер фотохранилища — из Redis-ключа, который пишет суточный cron
        refresh_photo_storage (rglob каждые 30 секунд убран — аудит 2026-08);
        сам refresh_custom_metrics каталог НЕ сканирует."""
        from pathlib import Path

        from app.worker.tasks import metrics as metrics_mod
        from app.worker.tasks.metrics import PHOTO_STORAGE_KEY, refresh_custom_metrics
        ...
        # даёт корутину → "not iterable" в коллекторах и не-awaited-корутины.
        redis.hgetall = AsyncMock(return_value={})
        redis.get = AsyncMock(return_value=b"1024")
        redis.set = AsyncMock()

        # Несуществующий каталог: если бы refresh сканировал сам,
        # значение было бы 0, а не 1024 из ключа.
        with patch.object(metrics_mod, "PHOTOS_ORIGINALS_DIR", Path("/nonexistent")):
            result = await refresh_custom_metrics({"redis": redis})

        assert result.get("photo_storage_bytes") == 1024
        redis.get.assert_any_call(PHOTO_STORAGE_KEY)
