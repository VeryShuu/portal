...
def test_invited_users_to_jsonb_strips_rsvp():
    """`rsvp` не должен попасть в JSONB-слепок.

    Regression (прод 2026-09-03): GET /bookings обогащает участников `rsvp`
    (ответ на приглашение из Outlook, `RsvpInfo.updated_at: datetime`).
    Фронтенд эхо-отправлял его обратно в PUT /bookings, `model_dump()` тащил
    `RsvpInfo` с `datetime`-полем → SQLAlchemy падал на JSONB-сериализации с
    `TypeError: Object of type datetime is not JSON serializable` → 500 на
    любое редактирование брони, где хотя бы один участник ответил.
    """
    import json

    from app.schemas.meetings import InvitedUser, RsvpInfo
    from app.services.meetings.bookings_service import invited_users_to_jsonb
    ...
    result = invited_users_to_jsonb(
        [
            InvitedUser(
                ...
                full_name="A",
                email="a@x.com",
                rsvp=RsvpInfo(status="accepted", updated_at=datetime(2026, 9, 3, 7, 0, tzinfo=UTC)),
            ),
            InvitedUser(user_id="u2", full_name="B", email="b@x.com"),
        ]
    )
    assert len(result) == 2
    assert "rsvp" not in result[0]
    assert "rsvp" not in result[1]
    assert result[0]["user_id"] == "u1"
    # Главный регрессионный ассерт: результат сериализуем в JSON (как делает
    # SQLAlchemy для JSONB). До фикса json.dumps падал на `datetime`.
    json.dumps(result)


def test_invited_users_to_jsonb_strips_absence_and_rsvp_together():
    """Обогащённый участник (absence + rsvp одновременно) → чистый слепок.

    GET-выдача может заполнить оба runtime-поля; в JSONB остаются только
    персистируемые user_id/full_name/email/source.
    """
    import json

    from app.schemas.meetings import AbsenceInfo, InvitedUser, RsvpInfo
    from app.services.meetings.bookings_service import invited_users_to_jsonb
    ...
    result = invited_users_to_jsonb(
        [
            InvitedUser(
                ...
                rsvp=RsvpInfo(
                    status="tentative", updated_at=datetime(2026, 9, 3, 9, 30, tzinfo=UTC)
                ),
            ),
        ]
    )
    assert result == [{"user_id": "u1", "full_name": "A", "email": "a@x.com", "source": "keycloak"}]
    json.dumps(result)
