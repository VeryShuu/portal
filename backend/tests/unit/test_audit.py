...
@pytest.mark.asyncio
async def test_push_audit_event_increments_cross_process_counter():
    """Метрика — HINCRBY в Redis-хэш (кросс-процессный паттерн, прод --workers 2):
    локальный Counter процесс-специфичен, /metrics попадает на случайный воркер."""
    redis = AsyncMock()

    await push_audit_event(redis, event_type="news.created", user_id="u")

    redis.hincrby.assert_awaited_once_with(AUDIT_METRICS_KEY, "news.created", 1)


@pytest.mark.asyncio
async def test_push_audit_event_metric_failure_does_not_break_push():
    """hincrby упал (Redis отвалился между rpush и счётчиком) — событие уже в
    очереди, исключение проглатывается, обработчик не падает."""
    redis = AsyncMock()
    redis.hincrby = AsyncMock(side_effect=ConnectionError("metrics redis down"))
