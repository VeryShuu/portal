...
class TestLocalAuthIsolation:
    """Изоляция local vs keycloak auth_source на production-объектах.

    Аудит тестирования 2026-08-23 (P3): прежние три теста были таутологиями
    (проверяли локальные SimpleNamespace/boolean, а не код приложения) —
    сломать production-логику можно было без единого упавшего теста. Ниже —
    инварианты, которые реально удерживает production-код.
    """

    def test_auth_source_check_constraint_allows_only_local_and_keycloak(self):
        """БД-уровень: users.auth_source ограничен констрейнтом ck_users_auth_source.

        Это фундамент изоляции: невозможность «третьего» источника auth,
        из-за которого ветки login-логики расходились бы непредсказуемо.
        """
        from sqlalchemy import CheckConstraint

        from app.models.user import User

        constraints = [c for c in User.__table__.constraints if isinstance(c, CheckConstraint)]
        ck = next((c for c in constraints if c.name == "ck_users_auth_source"), None)
        assert ck is not None, "ck_users_auth_source исчез из модели User"
        sqltext = str(ck.sqltext)
        assert "'keycloak'" in sqltext and "'local'" in sqltext, sqltext

    def test_dummy_hash_never_verifies_any_password(self):
        """Timing-защита local login: keycloak-юзер получает DUMMY_HASH.

        Если DUMMY_HASH случайно окажется валидным хэшем какого-то пароля,
        verify пройдет у пользователя, которого нет в local-контуре.
        """
        from app.core.security import DUMMY_HASH

        assert verify_password("any-password", DUMMY_HASH) is False
        assert verify_password("", DUMMY_HASH) is False
        assert verify_password("admin12345", DUMMY_HASH) is False

    # Третья бывшая таутология (bootstrap_idempotency) удалена: реальная
    # идемпотентность bootstrap-admin покрыта tests/unit/test_bootstrap_admin.py.
