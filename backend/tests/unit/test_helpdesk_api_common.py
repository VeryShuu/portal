...
class TestMessageMapper:
    ...
    def test_attachment_warnings_only_in_agent_view(self) -> None:
        msg = _msg()
        msg.attachment_warnings = [
            "Вложение «unsafe.exe» не сохранено: неподдерживаемый тип файла."
        ]
        ticket = _ticket([msg])
        assert ticket_to_out(ticket).messages[0].attachment_warnings == []
        assert (
            ticket_to_agent_out(ticket).messages[0].attachment_warnings == msg.attachment_warnings
        )
