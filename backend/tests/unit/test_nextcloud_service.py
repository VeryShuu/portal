...
async def test_create_folder_ok():
    ...
    args = mock_client.request.await_args.args
    assert args[0] == "MKCOL"
    assert args[1] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/NewFolder"
    )
...
async def test_create_folder_already_exists_ok():
    ...
    assert args[0] == "MKCOL"
    assert args[1] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/Existing"
    )
...
async def test_delete_ok():
    ...
    assert args[1] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/old.pdf"
    )
...
async def test_delete_404_ok():
    ...
    assert args[0] == "DELETE"
    assert args[1] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/gone.pdf"
    )
...
async def test_move_ok():
    ...
    assert call.args[1] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/old"
    )
    headers = call.kwargs["headers"]
    assert headers["Destination"] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/new"
    )
...
async def test_upload_stream_ok():
    ...
    assert call.args[0] == (
        "https://nc.company.local/remote.php/dav/files/portal-svc/PortalFiles/HR/file.txt"
    )
    # стриминг: content — именно переданный async-генератор, не буферизованный bytes
    assert not isinstance(call.kwargs.get("content"), (bytes, bytearray))
