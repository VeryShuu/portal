...
class TestMeetingsModuleOut:
    # async-тест (а не get_event_loop().run_until_complete): ручной event loop
    # падал order-dependent — если тест шёл раньше любого async-теста, loop ещё
    # не создан («There is no current event loop»). Поймано nightly-flakes.
    async def test_meetings_field_in_modules_endpoint(self, authed_client_factory):
