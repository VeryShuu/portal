...
@pytest.mark.asyncio
async def test_add_agent_reply_limits_total_outbound_attachments() -> None:
    db = _make_db()
    ticket = _ticket(status="new")
    agent = _user()

    with unittest.mock.patch(
        "app.services.helpdesk.attachments.upload_attachments", new=AsyncMock()
    ) as uploaded:
        await add_agent_reply(
            db,
            ticket=ticket,
            agent=agent,
            payload=MessageCreateIn(body_text="ответ"),
            files=[SimpleNamespace(filename="report.pdf")],
            support_domain="example.com",
        )

    assert uploaded.await_args is not None
    assert uploaded.await_args.kwargs["max_total_bytes"] == 15 * 1024 * 1024
