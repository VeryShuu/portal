...
# Дефолт списка iframe-origin'ов (system.json → video_iframe_origins).
DEFAULT_ORIGINS_FIXTURE = [
    "https://video.mage.ru",
...
def test_build_branding_includes_mage_video_embed_origin():
    from app.api.bootstrap import _build_branding
    from app.schemas.branding import BrandingSettings

    with (
        patch("app.api.bootstrap.branding_assets.load_settings", return_value=BrandingSettings()),
        patch(
            "app.api.bootstrap.load_system_settings",
            return_value=MagicMock(
                video_gallery_url=None, video_iframe_origins=DEFAULT_ORIGINS_FIXTURE
            ),
        ),
        patch("app.api.bootstrap.branding_assets.find_file", return_value=None),
    ):
        branding = _build_branding()

    assert branding.allowed_iframe_origins == [
        "https://video.mage.ru",
        "https://www.youtube-nocookie.com",
        "https://rutube.ru",
        "https://vk.com",
