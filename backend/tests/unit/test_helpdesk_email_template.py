...
class TestRenderReplyEmail:
    ...
    def test_reply_marker_is_in_both_email_alternatives(self) -> None:
        """Управляемый separator — основной надёжный якорь очистки ответа."""
        ...
        assert REPLY_MARKER_TOKEN in html_out
        assert REPLY_MARKER_TOKEN in plain_out
        # Ответ и история всё равно присутствуют (история под заголовком).
        assert "ОТВЕТ" in html_out
        assert "ИСТОРИЯ" in html_out

    def test_no_history_keeps_reply_marker_but_omits_history_section(self) -> None:
        """Первый ответ тоже должен нести separator для последующего reply."""
        html_out, _ = render_reply_email(
            ticket=_ticket(),
            agent_body_html="<p>Ответ</p>",
            agent_body_text="Ответ",
...
class TestRenderSystemEmail:
    def test_wraps_content_with_marker_and_without_history(self) -> None:
        html_out, _ = render_system_email(
        ...
        # На системное письмо тоже можно ответить в тот же тикет.
        assert REPLY_MARKER_TOKEN in html_out
        assert "Предыдущие сообщения" not in html_out
