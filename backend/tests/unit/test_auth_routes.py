...
def _fake_request(xfp: str | None = None, scheme: str = "http"):
...
class TestBuildSessionCookieResponse:
    def test_returns_redirect_with_cookie(self):
        ...
        resp = _build_session_cookie_response(_fake_request(), "/dashboard", "my-session-id")
    ...
    def test_sets_last_auth_method_keycloak_cookie(self):
        ...
        resp = _build_session_cookie_response(_fake_request(), "/", "sid")
    ...
    def test_session_cookie_secure_follows_forwarded_proto(self):
        """ADR-021: Secure по фактическому протоколу, не по ENVIRONMENT.

        HTTP-запрос (даже в production-окружении) → кука без Secure, иначе
        браузер её молча отбросит; HTTPS (X-Forwarded-Proto: https) → Secure.
        """
        from app.api.auth import _build_session_cookie_response

        http_resp = _build_session_cookie_response(_fake_request(), "/", "sid")
        session_cookie = next(
            h for h in http_resp.headers.getlist("set-cookie") if h.startswith("portal_session=")
        )
        assert "secure" not in session_cookie.lower()

        https_resp = _build_session_cookie_response(_fake_request(xfp="https"), "/", "sid")
        session_cookie = next(
            h for h in https_resp.headers.getlist("set-cookie") if h.startswith("portal_session=")
        )
