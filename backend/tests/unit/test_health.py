...
class TestReadyEndpoint:
    ...
    def test_ready_503_when_nextcloud_enabled_without_url(self, client):
        """audit [PA-009]: включённый Nextcloud без URL — misconfiguration,
        /ready обязан отвечать 503 (Compose не должен считать backend healthy),
        а не 200 с checks["nextcloud"]="unconfigured"."""
        from types import SimpleNamespace

        from app.core.modules_config import AllModuleSettings

        modules = AllModuleSettings()
        modules.nextcloud.enabled = True
        ...
        with (
            patch("app.api.health.AsyncSessionLocal", return_value=mock_session),
            patch("app.api.health.get_redis", return_value=mock_redis),
            patch("app.api.modules.load_modules", return_value=modules),
            patch(
                "app.core.system_config.load_system_settings",
                return_value=SimpleNamespace(nextcloud_url=""),
            ),
        ):
            response = client.get("/ready")
        ...
        assert body["checks"]["nextcloud"] == "unconfigured"
        assert body["checks"]["postgres"] == "ok"

    def test_ready_200_when_nextcloud_disabled(self, client):
        """Выключенный модуль NC не влияет на readiness (дефолт dev/CI)."""
        from app.core.modules_config import AllModuleSettings
    ...
    def test_ready_sets_backend_ready_gauge_ok(self, client):
        """PA-023: /ready 200 → portal_backend_ready == 1."""
        from app.core.metrics import backend_ready
        from app.core.modules_config import AllModuleSettings
        ...
        with (
            ...
            response = client.get("/ready")
        assert response.status_code == 200
        assert backend_ready._value.get() == 1

    def test_ready_sets_backend_ready_gauge_fail(self, client):
        """PA-023: /ready 503 (DB down) → portal_backend_ready == 0."""
        from app.core.metrics import backend_ready
        ...
        with (
            ...
            response = client.get("/ready")
        assert response.status_code == 503
        assert backend_ready._value.get() == 0
