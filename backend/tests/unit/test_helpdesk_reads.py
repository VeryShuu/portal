...
@pytest.mark.asyncio
class TestCountUnreadAssignedTickets:
    """Счётчик непрочитанных назначенных тикетов для красного бейджа в меню.

    На unit-уровне — control-flow (один COUNT-запрос, int из ``scalar_one``,
    без commit); SQL-семантика (inbound-фильтр, ACTIVE_STATUSES, COALESCE +
    EPOCH-джентлен) проверяется integration-тестами в
    ``test_helpdesk_tickets.py::TestUnreadAssignedCount``.
    """

    async def test_returns_int_from_scalar(self) -> None:
        """COUNT(DISTINCT) → int (количество тикетов, не сообщений)."""
        db = _make_db([_result(scalar=3)])
        result = await reads_svc.count_unread_assigned_tickets(db, user_id=USER_ID)
        assert result == 3

    async def test_single_query_no_commit(self) -> None:
        """Один execute (join messages×tickets в одном COUNT), commit не нужен."""
        db = _make_db([_result(scalar=0)])
        await reads_svc.count_unread_assigned_tickets(db, user_id=USER_ID)
        assert db.execute.await_count == 1
        db.commit.assert_not_awaited()

    async def test_zero_when_nothing_unread(self) -> None:
        """Пустой результат COUNT → 0 (int, не None)."""
        db = _make_db([_result(scalar=0)])
        result = await reads_svc.count_unread_assigned_tickets(db, user_id=USER_ID)
        assert result == 0
...
@pytest.mark.asyncio
class TestCountMyUnreadTickets:
    """Заявительское зеркало счётчика — для красного бейджа «Поддержка».

    Unit-уровень: control-flow (один COUNT-запрос, int, без commit);
    SQL-семантика (outbound-фильтр, свои тикеты, ACTIVE_STATUSES) —
    в ``test_helpdesk_tickets.py::TestMyUnreadCount``.
    """

    async def test_returns_int_from_scalar(self) -> None:
        db = _make_db([_result(scalar=2)])
        result = await reads_svc.count_my_unread_tickets(db, user_id=USER_ID)
        assert result == 2

    async def test_single_query_no_commit(self) -> None:
        db = _make_db([_result(scalar=0)])
        await reads_svc.count_my_unread_tickets(db, user_id=USER_ID)
        assert db.execute.await_count == 1
        db.commit.assert_not_awaited()
