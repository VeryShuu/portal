...
def _xread_stream_offsets(redis, user_id):
    """Собрать per-stream offsets из фактических вызовов xread.

    Это и есть контракт Last-Event-ID: после реконнекта клиент обязан
    продолжить каждый из трёх потоков с распарсенного offset, а не с '$'.
    Возвращает (personal, meetings, photos) — по одному значению на поток.
    """
    ...
    offsets = {}
    for call in redis.xread.call_args_list:
        for key, offset in call.args[0].items():
            offsets[key] = offset

    missing = {stream_key, MEETINGS_STREAM_KEY, PHOTOS_STREAM_KEY} - set(offsets)
    assert not missing, f"xread не опросил потоки: {missing}"
    return (
        offsets[stream_key],
        offsets[MEETINGS_STREAM_KEY],
        offsets[PHOTOS_STREAM_KEY],
    )
...
async def test_sse_generator_last_event_id_plain_dollar():
    ...
    assert _xread_stream_offsets(redis, user_id) == ("$", "$", "$")
...
async def test_sse_generator_last_event_id_composite_triple():
    ...
    assert _xread_stream_offsets(redis, user_id) == ("111-0", "222-0", "333-0")
...
async def test_sse_generator_last_event_id_two_parts():
    ...
    assert _xread_stream_offsets(redis, user_id) == ("111-0", "222-0", "$")
...
# ── Regression: конечный block у всех XREAD (инцидент прода 2026-08-18) ────────
#
# meetings/photos XREAD ходили с block=0 — в Redis это «блокировать вечно».
# asyncio.gather не завершался, пока во встречах/фото нет событий, цикл
# замирал и keepalive не отправлялся → внешний proxy (proxy_read_timeout 60с)
# рвал SSE-стрим → ERR_INCOMPLETE_CHUNKED_ENCODING + reconnect-цикл у всех
# клиентов. Юнит-тесты этого не видели: мок xread отвечает мгновенно,
# семантика BLOCK 0 в моке не воспроизводится — поэтому фиксируем контракт
# прямой проверкой kwargs.


@pytest.mark.asyncio
async def test_sse_generator_all_xreads_use_finite_block():
    """Каждый из трёх XREAD обязан получать конечный block (никогда 0)."""
    from app.api.notifications import _sse_generator
    from app.services.notifications_sse import _SSE_POLL_INTERVAL
    ...
    assert redis.xread.await_count >= 3  # все три потока опрошены
    expected_ms = int(_SSE_POLL_INTERVAL * 1000)
    for call in redis.xread.call_args_list:
        block = call.kwargs.get("block")
        assert block == expected_ms, (
            f"xread block={block!r}, ожидался конечный {expected_ms} мс (BLOCK 0 = вечная блокировка)"
        )


@pytest.mark.asyncio
async def test_sse_generator_keepalive_emitted_with_no_events_at_all():
    """При полном отсутствии событий во всех потоках keepalive всё равно
    уходит: цикл не имеет права замирать (иначе прокси рвут стрим)."""
    from app.api.notifications import _sse_generator
    ...
    with (
        patch("app.services.notifications_sse._SSE_KEEPALIVE_SEC", 0.2),
        patch("app.services.notifications_sse._SSE_POLL_INTERVAL", 0.1),
    ):
        combined = await _collect(_sse_generator(request, redis, user_id, "c1", None))
