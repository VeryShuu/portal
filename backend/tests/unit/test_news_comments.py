...
class TestListComments:
    ...
    async def test_empty(self):
        ...
        with (
            ...
            patch(f"{_MOD}.comments_repo.count_comments", AsyncMock(return_value=0)),
    ...
    async def test_with_items_and_deleted(self):
        ...
        with (
            patch(f"{_MOD}._get_news_or_404", AsyncMock(return_value=news)),
            patch(f"{_MOD}.require_news_read_access"),
            patch(f"{_MOD}.comments_repo.count_comments", AsyncMock(return_value=2)),
            patch(
        ...
        assert r.status_code == 200
        data = r.json()
        # total согласован с выдачей: 2 строки (1 active + 1 deleted placeholder) — PA-014
        assert data["total"] == 2
        items = data["items"]
