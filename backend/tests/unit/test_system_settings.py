...
class TestRenderNginxConfigsScript:
    ...
    def _skip_if_missing_tools(cls) -> None:
        # Аудит 2026-08-21 (P0.5): в nightly-контуре отсутствие инструментов —
        # ошибка окружения, а не тихий skip (6 вечных skip'ов в каждом прогоне).
        nightly = os.environ.get("NIGHTLY", "false").lower() in ("1", "true", "yes")
        if not cls._have_sh():
            (pytest.fail if nightly else pytest.skip)("POSIX sh not available")
        if not cls._have_jq():
            (pytest.fail if nightly else pytest.skip)("jq not available")
        if not cls._have_envsubst():
            (pytest.fail if nightly else pytest.skip)("envsubst not available")
    ...
    def test_ssl_conf_http_only_when_no_certs(self, tmp_path):
        ...
        # Контейнеры бегут без root: nginx слушает непривилегированные 8080/8443,
        # хост-маппинг 80/443 делает docker-compose (см. 0431cdc).
        assert "listen 8080" in ssl
        assert "listen 8443" not in ssl
        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com"
            " https://nc.company.local" in ssl
        )
    ...
    def test_ssl_conf_https_when_certs_present(self, tmp_path):
        ...
        ssl = out["ssl_server.conf"]
        # 8443, не 443: непривилегированный порт (запуск без root); тест застал
        # drift из 0431cdc — до аудита 2026-08-21 этот тест в nightly всегда
        # skip'ался (нет envsubst) и изменение порта прошло незамеченным.
        assert "listen 8443 ssl" in ssl
        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com"
            " https://nc.company.local" in ssl
    ...
    def test_ssl_conf_allows_trusted_mage_video_without_runtime_settings(self, tmp_path):
        """The nginx sidecar itself keeps the editor's trusted video origin."""
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {}, certs=False)

        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com;"
            in out["ssl_server.conf"]
        )
    ...
    # ── learn-контур (ADR-051): fail-closed рендер learn_server.conf ─────────

    def test_learn_conf_disabled_without_base_url(self, tmp_path):
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {}, certs=True)
        learn = out["learn_server.conf"]
        assert "server {" not in learn
        assert "ADR-051" in learn

    def test_learn_conf_disabled_without_certs(self, tmp_path):
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {"learning_base_url": "https://learn.mage.ru"}, certs=False)
        assert "server {" not in out["learn_server.conf"]

    def test_learn_conf_rendered_with_portal_wildcard_cert(self, tmp_path):
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {"learning_base_url": "https://learn.mage.ru"}, certs=True)
        learn = out["learn_server.conf"]
        assert "server_name learn.mage.ru" in learn
        assert "listen 8443 ssl" in learn
        # allowlist API публичного контура + личная rate-зона
        assert "location ~ ^/api/v1/auth/learning/" in learn
        assert "limit_req zone=learning_auth" in learn
        # PA-003: learner-подмножество — точно; admin/methodist — 404 на границе
        assert "location = /api/v1/learning/meta" in learn
        assert "location /api/v1/learning/me/" in learn
        assert "location /api/v1/learning/ {" in learn
        assert "return 404;" in learn
        # статика — отдельная сборка learn (frontend:81), не портал-руут
        assert "frontend:81" in learn
        # публичный контур БЕЗ CIDR-гейта (комментарий в шапке не в счёт)
        assert "if ($allowed_network" not in learn
        # внешние iframe — только доверенные видеохостинги (материалы курсов)
        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com;"
            in learn
    ...
    def test_learn_conf_allowlist_order_me_before_catchall(self, tmp_path):
        """PA-003: proxy-локации learner-путей объявлены ДО 404-catchall
        (nginx выбирает длиннейший префикс независимо от порядка, но порядок
        фиксируем, чтобы регрессия «catchall первым» была видна глазами)."""
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {"learning_base_url": "https://learn.mage.ru"}, certs=True)
        learn = out["learn_server.conf"]
        me_pos = learn.index("location /api/v1/learning/me/")
        meta_pos = learn.index("location = /api/v1/learning/meta")
        catchall_pos = learn.index("location /api/v1/learning/ {")
        assert meta_pos < me_pos < catchall_pos

    def test_learn_conf_prefers_dedicated_cert(self, tmp_path):
        self._skip_if_missing_tools()
        certs_dir = tmp_path / "certs"
        certs_dir.mkdir(exist_ok=True)
        (certs_dir / "learn.crt").write_text("crt")
        (certs_dir / "learn.key").write_text("key")
        out = self._render(tmp_path, {"learning_base_url": "https://learn.mage.ru"}, certs=False)
        assert "learn.crt" in out["learn_server.conf"]
        assert "portal.crt" not in out["learn_server.conf"]

    def test_learn_conf_disabled_for_http_base_url(self, tmp_path):
        """PA-025: легаси http:// origin не рендерится — публичный контур
        существует только как TLS (reset-токен в query не должен уходить по
        HTTP). Хранимое значение остаётся в system.json (Admin API отвергает
        новые не-HTTPS), но контур fail-closed отключён."""
        self._skip_if_missing_tools()
        out = self._render(tmp_path, {"learning_base_url": "http://learn.mage.ru"}, certs=True)
        learn = out["learn_server.conf"]
        assert "server {" not in learn
        assert "ADR-051" in learn
...
class TestBuildNginxCsp:
    def test_includes_nextcloud_origin(self):
        ...
        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com"
            " https://nextcloud.company.local" in csp
        )
    ...
    def test_self_only_without_nextcloud(self):
        ...
        assert (
            "frame-src 'self' https://video.mage.ru https://www.youtube-nocookie.com"
            " https://rutube.ru https://vk.com https://vkvideo.ru https://player.vimeo.com;" in csp
        )
        frame_src = csp.split("frame-src ", 1)[1].split(";", 1)[0].split()
        assert "https:" not in frame_src
...
class TestOnboardingSettings:
    ...
    async def test_public_endpoint_requires_auth(self, client, tmp_settings_dir):
        # Настройки экскурса (включая админские тексты шагов) не должны
        # быть доступны без входа.
        r = await client.get("/api/v1/portal/onboarding")
