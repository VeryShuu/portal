...
def tmp_settings_dir(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(ss, "_NGINX_RELOAD_DIR", nginx_reload_dir)
    monkeypatch.setattr(ss, "_NGINX_RELOAD_TRIGGER", nginx_reload_dir / "reload-trigger")
    monkeypatch.setattr(ss, "_CERTS_DIR", certs_dir)
    monkeypatch.setattr(ss, "_settings_cache", {})
