...
class TestBuildMime:
    ...
    def test_meeting_reply_to_from_payload(self):
        """Reply-To = email создателя встречи (payload.reply_to от notifications):
        текстовые ответы участников идут организатору, а не на адрес портала."""
        from app.worker.tasks import email_outbox as eo

        row = _mk_row(kind=eo.KIND_MEETING, payload={"reply_to": "boss@corp.local"})
        msg = eo._build_mime(row, {"from_address": "noreply@x.com"})
        assert msg["Reply-To"] == "boss@corp.local"
        # From не меняется — отправителем остаётся адрес портала.
        assert msg["From"] == "noreply@x.com"

    def test_meeting_without_reply_to_omits_header(self):
        """Старые PENDING-строки / встречи без создателя: заголовка нет (как до правки)."""
        from app.worker.tasks import email_outbox as eo
...
class TestHeaderInjection:
    ...
    def test_reply_to_crlf_stripped_meeting(self):
        """payload.reply_to приходит из JSONB — санитизируется как Subject/To/From."""
        from app.worker.tasks import email_outbox as eo

        row = _mk_row(
            kind=eo.KIND_MEETING,
            payload={"reply_to": "boss@corp.local\r\nBcc: victim@evil.com"},
        )
        msg = eo._build_mime(row, {"from_address": "noreply@x.com"})
        assert msg["Bcc"] is None
        assert "\r" not in msg["Reply-To"]
        assert "\n" not in msg["Reply-To"]
