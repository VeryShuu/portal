...
async def test_admin_endpoint_requires_admin(security_authed_client_factory):
    """reader → 403 на admin-эндпоинте создания локального пользователя.

    Роут зарегистрирован как /api/v1/users/admin/local
    (app/api/users/routes_admin.py). 404 не допускается: он означал бы, что
    тест бьёт в несуществующий маршрут и защиту не проверяет вовсе.
    """
    ac, _ = security_authed_client_factory(role="reader")
    r = await ac.post(
        "/api/v1/users/admin/local",
        json={"email": "x@y.local", "full_name": "X", "password": "Pass1234!", "role": "reader"},
    )
    assert r.status_code == 403, f"/api/v1/users/admin/local → {r.status_code}"


async def test_editor_can_access_editor_routes(security_authed_client_factory):
    """editor имеет доступ к /news создание."""
    ac, _ = security_authed_client_factory(role="editor")
    r = await ac.get("/api/v1/news")
    assert r.status_code == 200, f"GET /api/v1/news (editor) → {r.status_code}"
