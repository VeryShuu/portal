...
async def test_nginx_csp_frame_src_narrowed_with_nextcloud_url():
    ...
    assert frame_src == ["'self'", *DEFAULT_VIDEO_IFRAME_ORIGINS, "https://nextcloud.company.local"]
    assert "https:" not in frame_src


async def test_nginx_csp_frame_src_includes_trusted_mage_video_origin():
    """CSP permits the same fixed video origins as the editor and sanitizer."""
    from app.core.constants import DEFAULT_VIDEO_IFRAME_ORIGINS
    from app.services.nginx_config import _build_nginx_csp

    csp = _build_nginx_csp("", "")

    assert "frame-src 'self' " + " ".join(DEFAULT_VIDEO_IFRAME_ORIGINS) + ";" in csp


async def test_nginx_csp_frame_src_fallback_self_only_without_nextcloud():
    """Без Nextcloud остаётся только фиксированный набор разрешённых video-origin."""
    from app.core.constants import DEFAULT_VIDEO_IFRAME_ORIGINS
    from app.services.nginx_config import _build_nginx_csp
    ...
    frame_src = csp.split("frame-src ", 1)[1].split(";", 1)[0].split()
    assert frame_src == ["'self'", *DEFAULT_VIDEO_IFRAME_ORIGINS]
    assert "https:" not in frame_src
