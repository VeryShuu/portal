...
# Образ переопределяется env: CI передаёт свежесобранный артефакт
# (portal-postgres:ci-<run>) — иначе успех зависит от устаревшего
# portal-postgres:16, случайно оставшегося на общем docker-daemon runner'ов.
...
def pytest_generate_tests(metafunc):
    """Параметризация test_migration_revision_round_trip.
    ...
    По умолчанию round-trip проверяет только последние MIGRATIONS_ROUNDTRIP_DEPTH
    ревизий (обычно 3) — каждая новая миграция в PR попадает в диапазон, а полный
    пошаговый прогон всех ревизий остаётся nightly (test_migrations_stepwise_down_up,
    @pytest.mark.nightly). Причина: round-trip с возвратом к head после каждой
    ревизии даёт O(N²) миграционных шагов — на 95+ ревизиях это ~90 секунд
    каждого PR-прогона. MIGRATIONS_ROUNDTRIP_DEPTH=all возвращает полный охват.
    """
    if "revision" in metafunc.fixturenames:
        ...
        depth_raw = os.environ.get("MIGRATIONS_ROUNDTRIP_DEPTH", "3")
        depth = len(revs) if depth_raw.lower() == "all" else max(1, int(depth_raw))
        metafunc.parametrize("revision", revs[:depth], ids=revs[:depth])
...
def test_migrations_stepwise_down_up(migration_env):
    ...
    nightly-only: линейный проход по всем ревизиям дублирует PR-проверки
    (full_lifecycle + round-trip head-ревизий), а в PR добавлял ~40 секунд;
    запускается в nightly-flakes (NIGHTLY=true).
