...
async def test_news_list_with_filters(app):
    ...
    async def _fake_get_list(
        db, *, user, status_filter, page, page_size, category=None, is_pinned=None, q=None, **kwargs
    ):
...
async def test_csrf_blocks_post_without_origin(app):
    ...
    /auth/local/login is CSRF-exempt (pre-session, cookie not yet issued).
    Use a non-exempt endpoint to verify the middleware blocks the request.
    """
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        r = await ac.post("/api/v1/news", json={})
    assert r.status_code == 403
