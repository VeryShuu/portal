...
# ---------------------------------------------------------------------------
# Unread-счётчик назначенных тикетов (красный бейдж в меню, /tickets/counts)
# ---------------------------------------------------------------------------


class TestUnreadAssignedCount:
    """``count_unread_assigned_tickets`` — SQL-семантика счётчика для красного
    бейджа «Инбокс поддержки».

    Считаются только активные (new/open/pending) тикеты, назначенные лично
    агенту, имеющие непрочитанные ответы заявителя (``direction='inbound'``
    новее ``last_seen_at``). Неназначенные и закрытые НЕ считаются.
    """

    async def test_assigned_with_unread_counts(self, real_db_session, ticket, real_admin):
        """Назначенный агенту тикет с непрочитанным первым сообщением → 1."""
        from app.services.helpdesk import reads as reads_svc

        ticket.assignee_user_id = real_admin.id
        await real_db_session.commit()

        count = await reads_svc.count_unread_assigned_tickets(
            real_db_session, user_id=real_admin.id
        )
        assert count == 1

    async def test_unassigned_not_counted(self, real_db_session, ticket, real_admin):
        """Неназначенный тикет (даже непрочитанный) в счётчик не попадает:
        бейдж — «в работе у меня», новые неназначенные — отдельный блок инбокса."""
        from app.services.helpdesk import reads as reads_svc
    ...
    async def test_mark_seen_clears_counter(self, real_db_session, ticket, real_admin):
        """Агент открыл карточку → прочитан → счётчик 0."""
        from app.services.helpdesk import reads as reads_svc
    ...
    async def test_closed_ticket_not_counted(self, real_db_session, ticket, real_admin):
        """Непрочитанный, но закрытый тикет не подкрашивает бейдж — иначе
        красный «залипал» бы навсегда на архивной истории."""
        from app.services.helpdesk import reads as reads_svc
    ...
    async def test_new_requester_reply_recolors_badge(
        self, real_db_session, ticket, real_admin, real_user
    ):
        """Полный цикл бейджа: прочитано (0) → новый ответ заявителя → снова 1."""
        from app.services.helpdesk import reads as reads_svc
        ...
        await real_db_session.commit()
        assert (
            await reads_svc.count_unread_assigned_tickets(real_db_session, user_id=real_admin.id)
            == 0
        )
...
# ---------------------------------------------------------------------------
# My-unread-счётчик (красный бейдж «Поддержка», /tickets/my/counts)
# ---------------------------------------------------------------------------


class TestMyUnreadCount:
    """``count_my_unread_tickets`` — заявительское зеркало агентского счётчика.

    Считаются только активные свои тикеты с непрочитанными ответами агентов
    (``direction='outbound'``). Ответы самого заявителя и закрытые тикеты
    НЕ считаются.
    """

    async def _add_agent_reply(self, real_db_session, ticket, real_admin) -> None:
        """Прямой INSERT outbound-сообщения (агентский ответ без outbox-цепочки
        ``add_agent_reply`` — нам нужен только факт нового сообщения)."""
        ...
        real_db_session.add(
            HelpdeskMessage(
                ...
                direction="outbound",
                body_text="ответ поддержки",
                source="web",
            )
        )
        await real_db_session.commit()

    async def test_agent_reply_counts_for_requester(
        self, real_db_session, ticket, real_admin, real_user
    ):
        """Ответ агента на активный тикет заявителя → счётчик 1."""
        from app.services.helpdesk import reads as reads_svc

        await self._add_agent_reply(real_db_session, ticket, real_admin)

        count = await reads_svc.count_my_unread_tickets(real_db_session, user_id=real_user.id)
        assert count == 1

    async def test_mark_seen_clears_counter(self, real_db_session, ticket, real_admin, real_user):
        """Заявитель открыл карточку → прочитано → счётчик 0."""
        from app.services.helpdesk import reads as reads_svc

        await self._add_agent_reply(real_db_session, ticket, real_admin)
        await reads_svc.mark_ticket_seen(real_db_session, ticket_id=ticket.id, user_id=real_user.id)
        await real_db_session.commit()

        count = await reads_svc.count_my_unread_tickets(real_db_session, user_id=real_user.id)
        assert count == 0

    async def test_own_requester_reply_not_counted(self, real_db_session, ticket, real_user):
        """Ответ самого заявителя (inbound) свой счётчик не подкрашивает."""
        from app.services.helpdesk import reads as reads_svc
        ...
        await messages_service.add_requester_reply(
            ...
            ticket=ticket,
            user=real_user,
            payload=MessageCreateIn(body_text="уточнение от меня"),
            files=[],
        )
        await real_db_session.commit()
    ...
    async def test_closed_ticket_not_counted(self, real_db_session, ticket, real_admin, real_user):
        """Непрочитанный ответ в закрытом тикете бейдж не подкрашивает."""
        from app.services.helpdesk import reads as reads_svc
