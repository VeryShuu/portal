...
class TestCreateBooking:
    ...
    async def test_update_room_change_with_rsvp_in_invited_persists(
        self, real_db_session, real_user, room, room2
    ):
        """Regression (прод 2026-09-03): смена комнаты при ответившем участнике.

        Точный прод-сценарий 500: GET /bookings обогащает участника `rsvp`
        (ответ из Outlook, `RsvpInfo.updated_at: datetime`), фронтенд при
        редактировании эхо-отправляет его в PUT /bookings. До фикса
        `model_dump()` тащил `RsvpInfo` с `datetime` в JSONB-слепок →
        `TypeError: Object of type datetime is not JSON serializable` → 500.
        """
        from app.schemas.meetings import (
            BookingCreate,
            BookingUpdate,
        ...
        booking, _diff = await update_booking(
            real_db_session,
            booking_id=b.id,
            payload=BookingUpdate(
                room_ids=[room2.id],  # смена комнаты, как на проде
                invited_users=[
                    InvitedUser(
                        user_id="u1",
                        full_name="A",
                        ...
                        rsvp=RsvpInfo(
                            status="accepted",
                            updated_at=datetime(2026, 9, 3, 7, 0, tzinfo=UTC),
                        ),
                    ),
                ],
            ),
        ...
        assert booking.invited_users is not None
        assert "rsvp" not in booking.invited_users[0]
        assert [br.room_id for br in booking.rooms] == [room2.id]

        # Перезагрузка из БД подтверждает, что строка действительно сохранена.
        reloaded = await get_booking(real_db_session, b.id)
        assert reloaded is not None
        assert reloaded.invited_users is not None
        assert "rsvp" not in reloaded.invited_users[0]

    async def test_create_with_rsvp_in_invited_persists(self, real_db_session, real_user, room):
        """Regression: `rsvp` (с `datetime`-полем) не должен попасть в JSONB.

        Тот же инвариант, что и в absence-тесте выше, но для POST /bookings:
        `create_booking` пишет `payload.invited_users` в JSONB.
        """
        from app.schemas.meetings import BookingCreate, InvitedUser, RsvpInfo
        from app.services.meetings.bookings_service import create_booking, get_booking
        ...
        assert b.invited_users is not None
        assert "rsvp" not in b.invited_users[0]
...
class TestUpdateRoomChange:
    ...
    async def test_update_room_change_reflected_in_rooms_and_ical(
        ...
        ical_text = build_ical(
            updated, method="REQUEST", company_domain="test", organizer_email="p@x"
