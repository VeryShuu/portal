...
async def test_all_rate_limited_endpoints_return_429(limiter, app):
    ...
    skip_path_fragments = (
        ...
        "/helpdesk/",
        # learning: auth/learning/* закрыт require_learning_module (ревью
        # 2026-08-28) — при выключенном модуле (дефолт test env) gate отдаёт
        # 404 раньше RateLimiter. Паттерн helpdesk; целевые 429-тесты —
        # вместе с публичным контуром (инкремент 6, ТЗ §13).
        "/auth/learning/",
    )
