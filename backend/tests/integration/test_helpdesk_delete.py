...
class TestDeleteTicketService:
    ...
    async def test_fs_cleanup_called(self, real_db_session, ticket, monkeypatch):
        """``delete_ticket_dir`` вызывается после коммита DB-удаления.

        Это best-effort очистка ``/data/helpdesk/TKT-{number}``: orphan-файл
        безопаснее потери вложений у тикета при неуспешном DB commit.
        """
        calls: list[int] = []
