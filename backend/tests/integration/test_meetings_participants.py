...
- нерешённый email → внешний участник (``source=external``);
- неоднозначное ФИО → ``ambiguous`` с кандидатами;
- нераспознанное ФИО → ``unresolved``;
...
- ``POST /meetings/participants/absences`` — отсутствие на дату встречи.
...
from app.api.meetings.participants import participant_absences, resolve_participants
from app.models.erp_sync import ErpAbsence
from app.models.user import User
from app.schemas.meetings import ParticipantAbsencesRequest, ResolveParticipantsRequest
...
# ── отсутствие на дату встречи (POST /participants/absences) ─────────────────
...
class TestParticipantAbsences:
    """``POST /meetings/participants/absences`` — отсутствие на дату встречи.

    Сценарий жалобы, из-за которой endpoint появился: live-поиск обогащает
    бейдж «на сегодня», а сводка/письма — на дату встречи; бронирование на
    будущую дату показывало в форме «работает», а после сохранения — «отпуск».
    """

    async def test_absence_on_future_meeting_date(self, real_db_session, real_user):
        """Отсутствие, действующее на будущую дату встречи (сегодня — working)."""
        marker = uuid.uuid4().hex[:6]
        u = await _create(
            real_db_session,
            email=f"vac-{marker}@example.com",
            full_name=f"Отпускник {marker}",
            current_status="working",  # сегодня на работе — live-поиск бейдж не даст
        )
        ...
        result = await participant_absences(
            data=ParticipantAbsencesRequest(emails=[u.email], on_date=meeting_date),
            db=real_db_session,
            user=real_user,
        )

        assert set(result.keys()) == {u.email.lower()}
        info = result[u.email.lower()]
        assert info.category == "vacation"
        assert info.start_date == meeting_date - timedelta(days=1)
        assert info.end_date == meeting_date + timedelta(days=2)

    async def test_absence_not_covering_meeting_date_excluded(self, real_db_session, real_user):
        """Отсутствие закончилось до даты встречи → пустой ответ."""
        marker = uuid.uuid4().hex[:6]
        u = await _create(real_db_session, email=f"past-{marker}@example.com")
        meeting_date = date.today() + timedelta(days=30)
        await _create_absence(
            real_db_session,
            user_id=u.id,
            kind="sick",
            start=meeting_date - timedelta(days=10),
            end=meeting_date - timedelta(days=2),
        )
        await real_db_session.flush()
    ...
    async def test_mixed_case_email_keyed_lowercase(self, real_db_session, real_user):
        """Ключ ответа — lower(email) при любом регистре запроса."""
        marker = uuid.uuid4().hex[:6]
        u = await _create(real_db_session, email=f"case-{marker}@example.com")
        ...
        result = await participant_absences(
            data=ParticipantAbsencesRequest(emails=[u.email.upper()], on_date=meeting_date),
            db=real_db_session,
            user=real_user,
        )
        assert set(result.keys()) == {u.email.lower()}
        assert result[u.email.lower()].category == "business_trip"

    async def test_unknown_email_omitted(self, real_db_session, real_user):
        """Email без аккаунта в ``users`` просто не попадает в ответ."""
        result = await participant_absences(
            data=ParticipantAbsencesRequest(
                emails=[f"nobody-{uuid.uuid4().hex[:6]}@example.com"],
                on_date=date.today(),
            ),
            db=real_db_session,
            user=real_user,
    ...
    async def test_empty_emails_rejected_by_schema(self):
        """Пустой список email → ValidationError (min_length=1)."""
        with pytest.raises(ValidationError):
            ParticipantAbsencesRequest(emails=[], on_date=date.today())
