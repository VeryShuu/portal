...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("parent_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("photo_folders.id", ondelete="CASCADE"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("folder_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("photo_folders.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.CheckConstraint("subject_type IN ('user', 'group')", name="ck_photo_folder_perm_subject_type"),
        sa.CheckConstraint("permission IN ('viewer', 'uploader', 'manager')", name="ck_photo_folder_perm_permission"),
    ...
    op.create_table(
        ...
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("inherit_permissions", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column("processed", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("uploaded_by", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
    )
    op.create_index("idx_photos_folder_created", "photos", ["folder_id", sa.text("created_at DESC")])
    op.create_index("idx_photos_taken_at", "photos", [sa.text("taken_at DESC NULLS LAST")])

    op.create_foreign_key(
        "fk_photo_folders_cover",
        "photo_folders", "photos",
        ["cover_photo_id"], ["id"],
        ondelete="SET NULL",
    )
