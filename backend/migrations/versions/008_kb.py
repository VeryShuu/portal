...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("parent_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("kb_sections.id", ondelete="RESTRICT"), nullable=True),
    ...
    op.create_table(
        "kb_articles",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("section_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("kb_sections.id", ondelete="SET NULL"), nullable=True),
        ...
        sa.CheckConstraint("status IN ('draft', 'published', 'archived')", name="ck_kb_articles_status"),
    )
    op.execute("""
    ...
    op.create_index("idx_kb_articles_trgm", "kb_articles", ["title"], postgresql_using="gin",
                    postgresql_ops={"title": "gin_trgm_ops"})
    op.create_index("idx_kb_articles_section", "kb_articles", ["section_id"])
    op.create_index("idx_kb_articles_active", "kb_articles", ["section_id", "deleted_at"],
    ...
    op.create_table(
        "kb_article_tags",
        sa.Column("article_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("kb_articles.id", ondelete="CASCADE"), nullable=False),
        sa.Column("tag_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("kb_tags.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.CheckConstraint("status IN ('pending', 'approved', 'rejected')", name="ck_kb_suggestions_status"),
    )
