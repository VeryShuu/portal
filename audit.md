...
| H7  | 🟠 High | Code Smell | Primitive Obsession (консолидация первой) | M | 3 | [x] 2026-08-09 |
...
- [x] ~Первый подход: `user: "999:1000"` в compose (применяется к entrypoint).~ **Откат**: CI smoke упал — bind-mount `base_data/redis` с хоста приходит root-owned (`mkdir -p` в CI от root), redis-юзер не может создать `appendonlydir` в `/data` → `Permission denied` → unhealthy → каскадный fail migrations/backend.
- [x] **Финальный подход**: entrypoint стартует от root, делает `chown -R 999:1000 /data /tmp/redis.acl` (как `docker-entrypoint.sh` у postgres), затем дропает права через `su redis -s /bin/sh -c 'exec redis-server ...'` (gosu/su-exec в redis-alpine нет — только busybox `su`). `user:` убран из compose (drop-privileges в entrypoint).
- [x] Локальная верификация CI-сценария (root-owned volume): redis-server PID 1 работает от uid=999 (`/proc/<pid>/status` подтверждает Uid/Gid 999/1000), healthcheck → healthy, ACL/AOF создаются, `redis-cli ping` → PONG.

#### DoD
- [x] redis-server работает от uid=999 (не root) — проверено через `/proc/<pid>/status`
- [x] ACL-файл создаётся и читается redis-owned (`-rw------- redis redis`)
- [x] `/data` доступен на запись (AOF `appendonlydir` создаётся) — даже когда volume root-owned с хоста
- [x] Healthcheck → healthy, `redis-cli AUTH ... ping` → PONG
- [x] CI compose-smoke (воспроизведён локально) — проходит
- [ ] **Прод-верификация (оператор):** `docker compose exec redis sh -c 'cat /proc/1/status | grep Uid'` → `999`, healthcheck зелёный, приложение стартует, сессии работают

- **Сложность:** S → M (entrypoint-chown оказался нужен из-за root-owned volume)
- **Риск регрессии:** Низкий. `su redis` в busybox/alpine корректно передаёт сигналы (PID 1 = redis-server, SIGTERM обрабатывается).
- **Ожидаемый эффект:** blast-radius компрометации Redis: root → unprivileged (uid=999).
- **Статус:** [x] 2026-07-28 — выполнено. Первая итерация (`user: "999:1000"`) сломала CI (root-owned volume → AOF permission denied); вторая итерация (entrypoint chown + `su redis`) работает локально и совместима с CI-сценарием. ⚠️ прод-верификация после деплоя: см. runbook.
...
- **Статус:** [x] 2026-08-09 — выполнено. **Этап 0 (baseline):** `test_helpdesk_lifecycle.py` зелёный (26 тестов), mypy/drift snapshot зафиксирован. **Этап 1 (консолидация):** в `schemas/helpdesk.py` добавлены производные множества от enum'ов — `ACTIVE_STATUSES`, `ASSIGNED_ACTIVE_STATUSES`, `UNASSIGNED_ACTIVE_STATUSES`, `AGENT_SETTABLE_STATUSES`, `REQUESTER_REOPEN_STATUSES`, `ALL_STATUSES`, `ALL_SOURCES` (единый источник истины). Удалены дубликаты: `_ACTIVE_STATUSES` (tickets.py:185 → алиас), `ASSIGNED/UNASSIGNED_ACTIVE_STATUSES` (digest.py:52,54), `_REQUESTER_REOPEN_STATUSES` (messages.py:31), inline `("new","open","pending")` (tickets.py:426), `_TICKET_STATUSES`/`_TICKET_SOURCES` (api/hd/tickets.py:73,74), `INBOUND/OUTBOUND_DIRECTION` (reads.py:35,36 — теперь `HelpdeskDirection` enum). `TicketStatusIn.status` Literal сохранён (статическая OpenAPI-схема), но связан с enum комментарием. **Этап 2 (UserRole):** `UserRole(StrEnum)` с `reader/editor/admin` (НЕ `user`) в `schemas/user.py` + `PRIVILEGED_ROLES = (UserRole.editor, UserRole.admin)`. validator (`schemas/user.py`) и service-guard (`users_admin_service.py`) ссылаются на enum. `PRIVILEGED_ROLES` в `news/poll.py` и `news/poll/_helpers.py` заменены на импорт (дубликат убран). **Этап 3 (замена литералов):** lifecycle.py, tickets.py (service+api), messages.py, ingress.py, archive.py, reads.py, notifications.py — все status/source/direction литералы заменены на enum-значения. **Этап 4 (visibility): ОТКЛОНЁН** — feature удалён миграцией 084 (`ck_helpdesk_messages_visibility` дропнут, колонки нет в модели); создавать enum для несуществующей колонки бессмысленно. **Что НЕ тронуто (осознанно):** `email_template.py:269` `msg.direction == "outbound"` (модуль рендеринга писем, enum под TYPE_CHECKING — риск регрессии на письмах выше ценности); ACL-permission литералы `"editor"/"viewer"/"manager"` в `files_acl/photos_acl/kb` (другой домен — permission level, не role); raw SQL в миграциях и partial-index `sql_text`. **Проверки:** ci_lint ✓ (757 файлов), 4324 unit ✓ (1548 helpdesk+user+news+poll зелёные), check-drift ✓ — **OpenAPI не дрейфит** (StrEnum сериализуется в строки, как и было; DoD выполнен). 15 файлов, +169/−68 LOC.
...
| 2026-08-09 | Reydan (ZCode) | **Реализация H7 (ветка `feat/audit-h7`).** Разведка через subagent дала полную карту enum-landscape + 3 ключевые находки, изменившие план: (1) **Этап 4 (visibility) отпал** — feature удалён миграцией 084 (`ck_helpdesk_messages_visibility` дропнут, колонки нет в модели); создавать enum для несуществующей колонки бессмысленно. (2) **`"editor"` перегружен** — роль (`UserRole`) vs ACL-permission (`viewer/editor/manager` в files_acl/photos_acl/kb); трогать только role-context, ACL оставить. (3) **Доп. дубликаты вне карточки** — `_TICKET_STATUSES`/`_TICKET_SOURCES` (api/hd/tickets.py), `INBOUND/OUTBOUND_DIRECTION` (reads.py), `PRIVILEGED_ROLES` × 2 (news/poll). **Этап 1 (консолидация):** в `schemas/helpdesk.py` добавлены 7 производных множеств от enum'ов (ACTIVE/ASSIGNED_ACTIVE/UNASSIGNED_ACTIVE/AGENT_SETTABLE/REQUESTER_REOPEN/ALL_STATUSES/ALL_SOURCES). Удалены дубликаты: `_ACTIVE_STATUSES` (tickets.py:185 → алиас), `ASSIGNED/UNASSIGNED_ACTIVE_STATUSES` (digest.py), `_REQUESTER_REOPEN_STATUSES` (messages.py), inline `("new","open","pending")` (tickets.py:426), `_TICKET_STATUSES`/`_TICKET_SOURCES` (api/hd/tickets.py), `INBOUND/OUTBOUND_DIRECTION` (reads.py → `HelpdeskDirection` enum). `TicketStatusIn.status` Literal сохранён (OpenAPI), связан с enum комментарием. **Этап 2 (UserRole):** `UserRole(StrEnum)` с `reader/editor/admin` (НЕ `user` — баг карточки исправлен) в `schemas/user.py` + `PRIVILEGED_ROLES = (UserRole.editor, UserRole.admin)`. validator + service-guard ссылаются на enum. `PRIVILEGED_ROLES` в news/poll.py и _helpers.py — импорт из schemas (дубликат убран). **Этап 3 (литералы):** lifecycle/tickets(service+api)/messages/ingress/archive/reads/notifications — все status/source/direction литералы → enum-значения. **НЕ тронуто (осознанно):** `email_template.py:269` (модуль рендеринга писем, риск регрессии), ACL-permission литералы (другой домен), raw SQL в миграциях/DDL. **Проверки:** ci_lint ✓ (757 файлов), 4324 unit ✓ (1548 helpdesk+user+news+poll), **check-drift ✓ — OpenAPI НЕ дрейфит** (StrEnum → строки, DoD выполнен). 15 файлов, +169/−68. Карточка H7 → [x]. Все три High (H4/H6/H7) закрыты. |
