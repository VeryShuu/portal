...
const ru = JSON.parse(fs.readFileSync(path.join(__dirname, 'frontend/src/i18n/ru.json'), 'utf8'));
const en = JSON.parse(fs.readFileSync(path.join(__dirname, 'frontend/src/i18n/en.json'), 'utf8'));
...
miss.slice(0, 60).forEach(function(k) { console.log('  ' + k); });
var extra = Object.keys(flat(en)).filter(function(k) { return !new Set(rk).has(k); });
console.log('\nExtra in en (not in ru): ' + extra.length);
