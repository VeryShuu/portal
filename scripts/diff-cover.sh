...
# Frontend (LCOV): vitest пишет SF:-пути относительно frontend/ ('src/...'),
# а diff-cover внутри relative_path() превращает их в '../src/...' — пути никогда
# не совпадают с git-diff ('frontend/src/...' → 'src/...') и гейт молча проходит
# впустую («No lines with coverage information»). Фикс: переписать SF: на
# 'frontend/src/...' во временном файле — тогда обе стороны сходятся в 'src/...'.
if [[ "$CONTOUR" == "frontend" ]]; then
  FIXED_LCOV="$WORK_DIR/coverage/lcov.diff-cover.info"
  sed "s|^SF:src/|SF:frontend/src/|" "$COV_FILE" > "$FIXED_LCOV"
  COV_FILE="$FIXED_LCOV"
fi
