...
#
#   tests/integration/test_local_auth.py — ПЕРЕВЕДЁН на DSN-стек (аудит 2026-08-21):
#   реальный HTTP + PostgreSQL + Redis через scripts/test-integration.sh;
#   в ephemeral-контейнере этого скрипта нет env (SECRET_KEY/DATABASE_URL),
#   поэтому из дефолтного набора файл убран.
#
# Usage:
...
if [[ $# -eq 0 ]]; then
  PYTEST_ARGS=(tests/integration/test_migrations.py)
else
  PYTEST_ARGS=("$@")
fi
...
# GID владельца docker.sock на хосте: контейнерный пользователь (portal, uid 1001)
# не совпадает с хостовым, поэтому группу сокета нужно добавить в контейнер явно
# (иначе testcontainers ловит PermissionError(13) на Docker API).
