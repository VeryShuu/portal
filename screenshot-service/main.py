...
_SYNTHETIC_FLOW = "login_and_load"
_SYNTHETIC_RESULT_TTL = 900


class _ProbeMetrics:
    """Process-local metrics owned by the process that executes the browser flow."""

    def __init__(self) -> None:
        self.registry = CollectorRegistry()
        labels = ("flow",)
        self.expected = Gauge(
            "portal_synthetic_probe_expected",
            "Whether the synthetic flow is configured and expected to run.",
            labelnames=labels,
            registry=self.registry,
        )
        self.result_available = Gauge(
            "portal_synthetic_probe_result_available",
            "Whether a fresh completed synthetic result is available.",
            labelnames=labels,
            registry=self.registry,
        )
        self.last_attempt = Gauge(
            "portal_synthetic_probe_last_attempt_timestamp_seconds",
            "Unix timestamp when the synthetic flow was last started.",
            labelnames=labels,
            registry=self.registry,
        )
        self.last_completed = Gauge(
            "portal_synthetic_probe_last_completed_timestamp_seconds",
            "Unix timestamp when the synthetic flow last completed.",
            labelnames=labels,
            registry=self.registry,
        )
        self.up = Gauge(
            "portal_synthetic_probe_up",
            "Synthetic user-flow result (1 = succeeded, 0 = failed).",
            labelnames=labels,
            registry=self.registry,
        )
        self.duration = Gauge(
            "portal_synthetic_probe_duration_seconds",
            "Synthetic user-flow wall-clock duration in seconds.",
            labelnames=labels,
            registry=self.registry,
        )
    ...
    @staticmethod
    def configured() -> bool:
        return bool(os.environ.get("PROBE_ADMIN_EMAIL")) and bool(
            os.environ.get("PROBE_ADMIN_PASSWORD")
        )

    @staticmethod
    def _remove(gauge: Gauge) -> None:
        try:
            gauge.remove(_SYNTHETIC_FLOW)
        except KeyError:
            pass

    def mark_attempt(self) -> None:
        self.attempted_at = time.time()
        self.last_attempt.labels(_SYNTHETIC_FLOW).set(self.attempted_at)

    def clear_result(self) -> None:
        self.completed_at = None
        self.last_ok = None
        self.last_duration_seconds = None
        self._remove(self.last_completed)
        self._remove(self.up)
        self._remove(self.duration)

    def mark_completed(self, *, ok: bool, elapsed_ms: int) -> None:
        self.completed_at = time.time()
        self.last_ok = ok
        self.last_duration_seconds = elapsed_ms / 1000.0
        self.last_completed.labels(_SYNTHETIC_FLOW).set(self.completed_at)

    def render(self) -> bytes:
        configured = self.configured()
        self.expected.labels(_SYNTHETIC_FLOW).set(1 if configured else 0)
        if not configured:
            self.clear_result()
        ...
        fresh = (
            ...
            and time.time() - self.completed_at <= _SYNTHETIC_RESULT_TTL
            and self.last_ok is not None
            and self.last_duration_seconds is not None
        )
        self.result_available.labels(_SYNTHETIC_FLOW).set(1 if fresh else 0)
        if fresh:
            self.up.labels(_SYNTHETIC_FLOW).set(1 if self.last_ok else 0)
            self.duration.labels(_SYNTHETIC_FLOW).set(self.last_duration_seconds)
        else:
            self._remove(self.up)
            self._remove(self.duration)
...
async def metrics(request: web.Request) -> web.Response:
    """Expose synthetic state inside the private Docker network."""

    probe_metrics: _ProbeMetrics = request.app["probe_metrics"]
    return web.Response(
        body=probe_metrics.render(),
...
async def ready(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(
            {
                "status": "not_ready",
                "reason": f"browser_check_failed: {type(exc).__name__}",
            },
            status=503,
        )
...
async def run_probe(request: web.Request) -> web.Response:
    ...
    via local-auth (POST /api/v1/auth/local/login), open the home page, verify
    its stable monitoring marker and an authenticated bootstrap response for
    the same local user, then fail on any browser ``pageerror``. Credentials come from
    the ``PROBE_ADMIN_EMAIL`` / ``PROBE_ADMIN_PASSWORD`` env vars (configured on
    screenshot-service, defaults empty → probe returns ``configured: false``).
    ...
    if flow != _SYNTHETIC_FLOW:
        return _error(400, f"unknown flow: {flow!r} (supported: {_SYNTHETIC_FLOW})")
    ...
    try:
        ...
        try:
            page = await context.new_page()
            page_error_seen = False

            def _record_page_error(_error: object) -> None:
                # Error text can contain URLs or user data. Keep only the fact.
                nonlocal page_error_seen
                page_error_seen = True
            ...
            # 3. Open the authenticated home route. ``#app`` alone is not
            # sufficient: it exists on error/login shells as well.
            try:
                await page.goto(
                    frontend_url, wait_until="domcontentloaded", timeout=PAGE_TIMEOUT_MS
                )
            except Exception:
                step_failed = "navigate_home"
                raise
            if "/login" in page.url:
                step_failed = "still_on_login"
                raise RuntimeError("probe remained on the login route")
            try:
                await page.wait_for_selector(
                    '[data-monitoring-ready="home"]', timeout=PAGE_TIMEOUT_MS
                )
            except Exception:
                step_failed = "authorized_content_missing"
                raise RuntimeError("authenticated home marker is missing") from None

            # 4. Verify that the same browser context can read authenticated
            # application data for the configured local probe account. Never
            # include returned identity values in logs or the probe response.
            try:
                bootstrap = await page.request.get(
                    f"{frontend_url}/api/v1/bootstrap", timeout=PAGE_TIMEOUT_MS
                )
                if bootstrap.status != 200:
                    step_failed = f"bootstrap_status_{bootstrap.status}"
                    raise RuntimeError("bootstrap returned a non-200 status")
                try:
                    payload = await bootstrap.json()
                except Exception:
                    step_failed = "bootstrap_invalid"
                    raise RuntimeError("bootstrap returned invalid JSON") from None
                user = payload.get("user") if isinstance(payload, dict) else None
                identity_valid = (
                    isinstance(user, dict)
                    and bool(user.get("id"))
                    and isinstance(user.get("email"), str)
                    and user["email"].strip().casefold()
                    == admin_email.strip().casefold()
                    and user.get("auth_source") == "local"
                )
                if not identity_valid:
                    step_failed = "bootstrap_invalid"
                    raise RuntimeError("bootstrap identity validation failed")
            except Exception:
                if step_failed is None:
                    step_failed = "bootstrap_request"
                raise

            # Give initial async work a bounded chance to surface page errors.
            # networkidle is unsuitable because the portal has long-lived SSE.
            await page.wait_for_timeout(300)
            if page_error_seen:
                step_failed = "page_error"
                raise RuntimeError("browser page error observed")
        finally:
            await context.close()
        elapsed = round((time.time() - t0) * 1000)
        probe_metrics.mark_completed(ok=True, elapsed_ms=elapsed)
        ...
        elapsed = round((time.time() - t0) * 1000)
        probe_metrics.mark_completed(ok=False, elapsed_ms=elapsed)
...
def build_app() -> web.Application:
    ...
    app["probe_metrics"] = _ProbeMetrics()
