...
| `payload` | jsonb | Произвольные данные (для встреч: `ical_b64`, `method`, опционально `reply_to` — email создателя встречи для заголовка `Reply-To`) |
...
### MIME-сборка и iCal
- `KIND_MEETING` → собирается как `multipart/mixed` с заголовком `Content-Class: urn:content-classes:calendarmessage`. Внутрь вкладывается `multipart/alternative`, содержащий HTML-тело и inline-календарь `text/calendar; method=REQUEST\|CANCEL`, декодированный из Base64 (`payload.ical_b64`). Если в `payload.reply_to` передан email создателя встречи, ставится заголовок `Reply-To` (санитизируется как `Subject`/`To`/`From`) — текстовые ответы участников уходят организатору, а не на адрес портала; без поля заголовок не ставится (старые PENDING-строки, встречи без создателя). `From` всегда остаётся адресом портала.
...
Помимо SMTP-блока, файл содержит общий IMAP-блок (миграция `089`) — параметры опроса общего ящика портала (portal@). Его потребители: **ERP-синк** (отчёты, фильтр по темам писём) и **meetings RSVP-поллер** (Accept/Decline-ответы на приглашения встреч, фильтр по календарному вложению `text/calendar; method=REPLY` — ADR-050, см. `docs/meetings.md` §6.2). Фильтры не пересекаются: каждый поллер забирает своё и не трогает чужие письма.
- `imap_host` / `imap_port` / `imap_use_ssl` — адрес и TLS IMAP-сервера.
- `imap_username` / `imap_password` — учётная запись ящика. IMAP-пароль хранится на диске **Fernet-шифром** (`imap_password_enc`, ключ из `SECRET_KEY`), а не открытым текстом.
- `imap_folder` — имя папки для опроса.

> Чтение настроек кешируется в процессе на 60 секунд (`_CACHE_TTL` в `./backend/app/services/email_settings.py`) — частые перечитывания файла не выполняются.
...
| **Helpdesk** (тяжёлый producer) | `./backend/app/services/helpdesk/outbound.py` (`kind=KIND_HELPDESK`, 3 точки: отправка ответа агента, уведомление заявителя о новом сообщении, авто-ответ при создании заявки), `./backend/app/services/helpdesk/digest.py` (`kind=KIND_GENERIC` — сводка), `./backend/app/services/helpdesk/notifications.py` (`kind=KIND_GENERIC` — уведомления) | Двусторонний email-thread тикетов (`[#TKT-{number}]`, заголовки `Message-ID`/`In-Reply-To`/`References` строятся в `./backend/app/services/helpdesk/outbound.py` + `worker/tasks/email_outbox.py`). Inline-картинки из ответов агента встраиваются как `cid:`-attach (`multipart/related`). **Собственный SMTP-контур (миграция 086):** при настроенном `smtp_host` в `helpdesk_mailbox_settings` вся helpdesk-почта (`kind=helpdesk` + `kind=generic` с маркером `payload.smtp_source="helpdesk"`) уходит через собственный SMTP под учёткой support-ящика, а не через общий порталный SMTP — `From:`/envelope/SMTP-auth консистентны с адресом приёма. Маршрутизация — в `process_email_outbox` (`_cfg_for_row` + `load_helpdesk_smtp_config`). При пустом `smtp_host` → fallback на общий SMTP портала. Подробности — `./docs/helpdesk.md` §«Outbound». |
