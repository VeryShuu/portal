...
| Frontend Widget | `./frontend/src/components/widgets/QuickServicesWidget.vue` | Виджет «Сервисы» на главной (до 8 ярлыков, сетка 4×2) |
...
Главная страница содержит виджет `./frontend/src/components/widgets/QuickServicesWidget.vue`, отображающий до **8** корпоративных ярлыков (сетка 4×2). Какие именно ярлыки попадают в виджет, настраивается флагом `show_on_home` на самом ярлыке (миграция `070`).
- **Логика выборки** (`topLinks` в `./frontend/src/pages/composables/useHomeLinksPreview.ts`): берутся ярлыки с `show_on_home=true`, отсортированные по `sort_order`, и обрезаются до первых 8. **Fallback:** если не отмечен ни один ярлык — показываются первые 8 ярлыков из общего списка (как до появления флага), чтобы виджет не оставался пустым.
