...
| **POST** | `/meetings/participants/resolve` | Bulk-resolve списка ФИО/email в участников: `{resolved, external, unresolved, ambiguous}` (используется модалом «Вставить список» при bulk-приглашении) | Любой сотрудник |
| **POST** | `/meetings/participants/absences` | Действующие отсутствия участников **на дату встречи** (`{emails, on_date}` → `{lower(email): AbsenceInfo}`) — форма обновляет бейджи при смене даты/состава участников | Любой сотрудник |
...
- **Служба iCal:** `ical_builder.build_ical` генерирует байты iCal-файла.
  ...
  - **Маршрутизация ответов (ORGANIZER + Reply-To):** поле `ORGANIZER` iCal и заголовок `Reply-To` письма содержат **email создателя встречи** (`users.email` через `creator_id`), а не адрес портала. По RFC 5545 IMIP-ответы Accept/Decline из почтовых клиентов (Outlook, Apple Calendar, Thunderbird) отправляются на адрес `ORGANIZER`, текстовые «Ответить» — на `Reply-To`, поэтому организатор видит реакции участников в своей почте. `From` письма остаётся адресом портала (SMTP-контур и подлинность писем не меняются). Fallback: создатель удалён (`creator_id = NULL`) или без email → `ORGANIZER` = адрес портала, `Reply-To` не ставится (поведение до правки). Реализация: `notifications.py` кладёт email организатора в `payload.reply_to` outbox-строки и в `build_ical(organizer_email=...)`; воркер `email_outbox.py` ставит заголовок из payload (с `_sanitize_header`).
  - ⚠️ Для встреч, созданных **до** этого изменения, организатор в календарях участников уже записан как адрес портала; первое редактирование такой встречи после деплоя разошлёт iCal с новым `ORGANIZER` — Outlook может отреагировать как на обновление/пересоздание встречи в календаре (разовый эффект на старые встречи).
...
### 6.2. RSVP: приём ответов на приглашения из почты портала

**Когда включено** (`modules.json` → `meetings.rsvp_ingest_enabled`, переключатель в Admin UI → Переговорные; см. ADR-050):

- **ORGANIZER iCal = адрес портала** (`from_address` из `/admin?tab=email`). IMIP-ответы Accept/Decline/Tentative из Outlook/Apple Calendar/Thunderbird (штатные кнопки писем) уходят на ORGANIZER — то есть на общий ящик портала. `Reply-To` остаётся на email создателя встречи — текстовые «Ответить» по-прежнему идут организатору лично.
- **IMAP-поллер** `poll_meetings_rsvp` (cron каждые 30 с, interval-guard 60 с, distributed lock) опрашивает общий ящик (IMAP-блок email-settings — тот же, что у ERP-синка). Паттерн «общего ящика»: `SEARCH ALL`, `\Seen` не ставится, чужие письма (ERP-отчёты, переписка, отскоки) не трогаются, дедуп — по `Message-ID` в `meeting_rsvp_email_log`. Обработанные REPLY удаляются (`STORE \Deleted` + `EXPUNGE`).
- **Парсер** (`services/meetings/rsvp_parser.py`): MIME → `text/calendar; method=REPLY` → UID/ATTENDEE/PARTSTAT/RECURRENCE-ID. Отбрасываются: не-REPLY (включая COUNTER — «предложить другое время»), From ≠ ATTENDEE (подделка/пересылка), чужие UID, неподдерживаемые PARTSTAT.
- **Матчинг**: UID `{booking.id}@` → одиночная встреча; `series-{id}@` + RECURRENCE-ID → конкретный экземпляр серии (по `start_time` UTC); серия без RECURRENCE-ID («ответил на всю серию») → все будущие экземпляры. Участник валидируется по `invited_users` (CI); протухший `user_id` в JSONB → `participant_user_id = NULL` (FK-защита).
- **Хранение**: таблица `meeting_rsvp` (миграция 100), upsert по `(booking_id, participant_email)` — «передумал» перезаписывает статус; повторное письмо (тот же Message-ID) — duplicate.
- **Отражение**: `BookingOut.invited_users[].rsvp` (enrich в `_mappers.py`, bulk-запрос) → бейджи ✓ Принял / ✕ Отклонил / ± Возможно на странице встречи. SSE-событие `meeting_changed` (action=`rsvp`) инвалидирует кэш открытых страниц.
- **Уведомления организатору** (worker `rsvp_digest.py`):
  - колокольчик сразу — in-app при изменении статуса (одно уведомление на письмо, для серии — «и ещё N встреч»);
  - email-сводка каждые 30 минут (окно — Redis-ключ, двигается после commit outbox): «кто ответил» по будущим встречам;
  - финальная сводка за 15 минут до встречи: полный список приглашённых со статусами (`meeting_bookings.rsvp_final_digest_sent_at` — идемпотентность, метка в той же транзакции, что и outbox-строка).
  - Не шлётся при `creator.notify_email=false` / удалённом создателе (для email-канала; колокольчик — только живому создателю).

**Выключено (по умолчанию)** → ORGANIZER = email создателя встречи, ответы идут организатору на почту (поведение PR #79), поллер не работает.

**Переходный период не нужен:** вся история встреч рассылалась с ORGANIZER = portal@, поэтому после включения поллер начнёт ловить ответы и по старым встречам.

### 6.3. Серии и повторения встречи
...
### 6.6. Настройки модуля и Feature Flag
...
class MeetingsModuleSettings(BaseModel):
    ...
    rsvp_ingest_enabled: bool = False   # §6.2: ORGANIZER=portal@ + IMAP-поллер
```
...
### Участники
- Список приглашаемых участников запрашивается из Keycloak Directory; дополнительно можно вручную добавить **внешнего участника по email** (например, контрагента).
...
- **Внешние участники.** В схеме `InvitedUser` есть дискриминатор `source ∈ {keycloak, external}` (default `keycloak` — обратная совместимость со старыми JSONB-строками). Внешний контакт хранится тем же объектом `InvitedUser` со стабильным синтетическим `user_id = "ext:<email>"` (`full_name = email`, `source = "external"`). Это переиспользует весь существующий конвейер: diff (`_compute_diff` ключует по `user_id`), iCal (`_add_invited_attendees` шлёт по email) и outbox-рассылку — внешний участник получает полноценное iCal-приглашение (REQUEST/CANCEL), как сотрудник. Лимит `max_length=100` на `invited_users` распространяется и на внешних. Email валидируется `EmailStr` (реальные домены; `.local` не принимается).
- **UX добавления (`ParticipantPicker.vue`).** Единое поле: по мере ввода идёт remote-поиск сотрудников в Keycloak Directory. Если введён валидный email и сотрудник не найден (нет совпадения по email среди результатов и среди уже выбранных), в выпадающем списке появляется дополнительная опция с бейджем «Внешний» — её выбор добавляет внешнего участника (`user_id = "ext:<email>"`). Дедуп — по email (case-insensitive). Bulk-приглашение: модал «Вставить список» → `POST /meetings/participants/resolve` (матчинг по словам в любом порядке).
- **Статус отсутствия участников.** `InvitedUser` обогащается полем `absence: AbsenceInfo | None` (отпуск/больничный/командировка) «на лету» — в `booking_to_out`, search/resolve участников и HTML писем; в JSONB не хранится. Источник — дневной cron `erp_absences_sync` (`./backend/app/worker/tasks/erp_absences_sync.py`, см. `./docs/erp-sync.md`).
  Две точки расчёта: live-поиск/resolve дают статус «на сегодня» (из `users.current_status` — подсказка в момент набора), а сводка (`booking_to_out`) и письма — на дату встречи (`enrich_absences_for_invited`, range-запрос к `erp_absences`). Чтобы бейдж в **форме** создания/редактирования совпадал со сводкой, фронтенд дополнительно вызывает `POST /meetings/participants/absences` (`useParticipantAbsenceSync`: при открытии формы, смене даты и изменении списка участников; опоздавшие ответы отбрасываются, сетевые ошибки — молча, бейдж не блокирует бронирование).
...
Классы: `RoomCreate`, `RoomUpdate`, `RoomOut`, `InvitedUser` (`user_id`, `full_name`, `email`, `absence` — runtime-обогащение, не хранится), `RecurrenceRule` (`freq` DAILY/WEEKDAYS/WEEKLY/BIWEEKLY/MONTHLY, `until_date`), `BookingCreate` (`invited_users` ограничен `max_length=100`), `BookingUpdate`, `BookingDelete`, `BookingOut`, `SeriesUpdate`, `SeriesCountOut`. Валидация таймзоны по IANA справочнику. Валидация `end_time > start_time`.
