...
> Последнее обновление: май 2026 (ADR-035 — silent refresh; ADR-036 — auto-SSO; ADR-037 — bootstrap env / runtime system.json; ADR-038 — виджет «Время в городах» + Open-Meteo)
...
- настраиваемый список городов (CRUD + drag-and-drop порядок) через `/settings`;
...
2. **UI:**
   ...
   - `pages/admin/tabs/WorldClockTab.vue` — вкладка в `/settings` с сортируемым списком (sortablejs, drag-handle), модалкой add/edit, сбросом к дефолтам.
