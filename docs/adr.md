...
7. **Удаление Sentry** — полностью вычищен (см. коммит сессии 1): `sentry-sdk` убран из зависимостей, `core/sentry.py` удалён, `sentry_dsn` убран из `SystemSettings`, секция из Admin UI и i18n удалена. Ошибки теперь идут через structlog `logger.exception(...)` → Loki.
