...
7. **Истечение сессии учитывает тип входа (июнь 2026).** `_handle401` в `api/index.ts` редиректит по последнему известному `auth_source`: локальная сессия (`local`) уходит на форму `/auth/local?redirect=...`, а не на `/api/v1/auth/login` (Keycloak SSO), который для local-пользователя только зациклил бы вход (у него нет Keycloak-учётки). Стор сообщает тип сессии через `setSessionAuthSource(...)` при каждой загрузке пользователя (`loadUser`/`loadBootstrap`/`setUser`); на клир значение НЕ сбрасывается, чтобы на момент 401 знать тип истёкшей сессии. По умолчанию — `keycloak`. Симметрично п.5 (logout): local-вход всегда возвращается на `/auth/local`.
