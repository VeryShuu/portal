...
> Последнее обновление: август 2026 — итерация 21 (**audit-2 review-2/3 remediation**): idempotency-атомарность доказана поведенческим takeover-барьером (перехват клиента подменяет владельца лока между GET и EXEC; ловит и WatchError-мутации) + TTL-контракт литералом 120с; e2e error-гейт — точечный allowlist (4xx у /api/ для негатив-кейсов, точные внешние сервисы; 5xx и не-API ресурсы всегда ошибки), полный teardown слушателей; ZAP nightly — `zap-api-scan.py -f openapi -S` (официальный импорт спеки: 335 путей → 449 URL, coverage-гейт ≥100); publish-Trivy: исправимые CRITICAL блокируют публикацию (jq-гейт), CVE-2025-68121 сужен package+path (gosu в postgres:16) с датой истечения; ARQ-контракт: полный литеральный снапшот расписаний всех 40 cron (second/minute/hour/day/run_at_startup) + анти-дубликаты; bookmark: spy + детерминированная проверка advisory-лок на pg_stat_activity; `tests.generated.md` включает e2e-кейсы (setup+дубли) и screenshot-методы (async). Замеры 2026-08-23: Backend — ~4820 unit + security (6 skip), integration — ~627 DSN; merged coverage 84%. Frontend — ~2493 Vitest / 261 файл. Playwright e2e — 14 spec-файлов (13 + gate-мета), required-прогон chromium production-preview, 47 тестов (45 + 2 gate). Предыдущие итерации — см. git-историю.
...
Хелперы `./backend/scripts/run_pytest_unit.sh` и `./backend/scripts/run_pytest_integration.sh` гарантируют единообразие флагов (`--no-cov -p no:cacheprovider`) между локалкой и CI. `run_pytest_integration.sh` покрывает **только DSN-based** integration: он запускает `pytest tests/integration` с `--ignore=` на трёх testcontainers-файлах (`test_migrations*.py`, `test_helpdesk_ingress.py` — аудит 2026-08-23, P1.3: прежний `pytest tests/` смешивал все контуры и падал на отсутствующем docker.sock); для testcontainers-based используйте отдельный `./scripts/run-testcontainers-tests.sh`.
...
> | **testcontainers-based** (`test_migrations.py`, `test_local_auth.py`, `test_migrations_nightly.py`) | Сам поднимает **свежую** Postgres/Redis через `PostgresContainer(...)` — нужен доступ к Docker API (docker.sock), которого **намеренно нет** в dev-контейнере (проброс сокета = root на хосте). Образ Postgres — env `PORTAL_POSTGRES_IMAGE` (default `portal-postgres:16`): CI передаёт свежесобранный артефакт `portal-postgres:ci-<run>`, чтобы успех не зависел от устаревших образов на общем daemon | ✅ через `./scripts/run-testcontainers-tests.sh` — запускает ephemeral-контейнер в host-сети с проброшенным сокетом (см. ниже). `test_helpdesk_ingress.py` — гибридный (testcontainers-IMAP + portal-БД), требует доп. env. |
>
> **`./scripts/run-testcontainers-tests.sh`** — локальный прогон testcontainers-тестов (миграции, local_auth, DB-роль learning). Запускает ephemeral-контейнер `portal-backend:dev` с `--network host` и проброшенным `docker.sock` (GID сокета добавляется в контейнер через `--group-add` — контейнерный пользователь uid 1001 иначе получает PermissionError(13) на Docker API), где testcontainers корректно поднимает sibling-Postgres. Не трогает живой portal-backend (host-сеть ломает DNS имён сервисов, поэтому не применимо к контейнеру из compose). Не требует локального python/deps.
>
> ```bash
...
> ```
>
> Если `docker compose exec backend run_pytest_integration.sh` падает на testcontainers-файлах с `docker.errors.DockerException: FileNotFoundError('/var/run/docker.sock)` — это устаревшее поведение: с аудита 2026-08-23 (P1.3) скрипт **исключает** testcontainers-файлы (`--ignore=` на миграции/helpdesk-ingress) и запускает ровно один контур — DSN-integration. Для testcontainers используйте отдельный скрипт выше.
...
#### Typecheck юнит-тестов (ratchet)

`tests/unit` не входит в `tsconfig.json` (там только `src/**`) и в `tsconfig.e2e.json`
(там `tests/e2e/**`) — отдельный `tsconfig.tests.json` + скрипт:

```bash
npm run typecheck:tests   # vue-tsc по src + tests/unit, строгий гейт
```

История: 283 ошибки типов (аудит-3 2026-08-24 — дыра прятала дрейф типов и
делала `expectTypeOf`-тесты api-types.spec.ts no-op) сожжены ratchet'ом
двумя сериями (PR #128: 283→130, PR #130: 130→0). С PR #130 гейт строгий:
любая ошибка типов в `tests/unit` роняет CI (джоба
`frontend / eslint + tsc + i18n + build`); baseline-файл удалён.
...
Проекты: `setup` (единственный admin-логин прогона → storageState `playwright/.auth/admin.json`, см. `auth.setup.ts`), `chromium` (PR + локально), `firefox` / `webkit` (без `*.mobile.spec.ts`, локально — в CI-ночном прогоне не запускаются), `mobile` (iPhone 13, только `*.mobile.spec.ts`). Браузерные проекты зависят от `setup` через `dependencies` — админ входит один раз, спеки создают контексты через `adminContextOptions()` (иначе email-лимит `/auth/local/login` 10/15 мин исчерпывается на 4-м спеке). Спеки `kb-acl`, `kb-media`, `photos` помечены `test.describe.configure({ mode: 'serial' })`, потому что используют shared state между тестами внутри describe. Спеки, тестирующие сам UI логина (`admin-login`, `local-login`), логинятся сами свежими контекстами. NB: на авторизованных страницах `waitForLoadState('networkidle')` не наступает из-за открытого SSE-потока `/notifications/stream` — ждать нужно конкретный элемент.
...
- `./frontend/tests/e2e/fixtures/api.ts` — `getCsrfToken(page)` читает реальный `XSRF-TOKEN` cookie (через `context.cookies()`, независимо от URL страницы) для CSRF double-submit, `e2eContextOptions()` переносит test-only trusted-proxy IP в контексты, созданные вручную через `browser.newContext()`, `adminContextOptions()` даёт контекст с готовой admin-сессией из setup-проекта, `newAdminPage(browser)` — готовую к API-вызовам admin-страницу (с навигацией на origin: на `about:blank` относительный `fetch` и cookie недоступны), `apiRequest(page, method, path, body?)` выполняет API-запросы и `CleanupRegistry` удаляет созданные сущности в `afterAll` (LIFO, best-effort).
- `./frontend/tests/e2e/fixtures/zip.ts` — `makeStoredZip(entries)` собирает валидный ZIP (stored, без сжатия) для теста импорта Obsidian-vault; заменяет динамический import JSZip с CDN, который в CI молча skip'ался.
...
### Skip-бюджет (какие skip'ы допустимы и где)

Аудит тестирования 2026-08-21: тихий skip в контуре, обещающем полный охват, —
ложнозелёный результат. Правила:

| Контур | Поведение при skip |
|--------|--------------------|
| **nightly** (`NIGHTLY=true`, workflow `nightly-flakes.yml`) | **Любой skip проваливает прогон.** Guard в корневом `tests/conftest.py` (`pytest_sessionfinish`) печатает список и выставляет exit 1 |
| daily CI (`backend / pytest unit + security`, `backend / pytest integration`) | skips допустимы только из бюджета ниже |
| локальные прогоны | без ограничений |

Бюджет допустимых skip-причин (daily):

1. `nightly marker: set NIGHTLY=true to run` — `@pytest.mark.nightly`-тесты
   (nginx render-config.sh, stepwise-миграции) сознательно не гоняются ежедневно.
2. `INTEGRATION_DB=true required` / `INTEGRATION_REDIS=true required` —
   DB/Redis-фикстуры в unit-прогоне без стека.
3. `NIGHTLY=true required` — nightly-only файлы (`test_migrations_nightly.py`).

Бюджет исполняется машиной (audit-review 2026-08-22 + follow-up²): таблица
`_DAILY_SKIP_CAPS` в корневом `tests/conftest.py` задаёт **(контур, причина) →
потолок** — потолки независимы по контурам (unit/integration), рост сверх
потолка проваливает прогон. Collection-skip'и (module-level `importorskip`)
вне reason-бюджета: разрешены только точными путями из
`_COLLECT_SKIP_ALLOWED_MODULES` (сейчас пуст). INTEGRATION_*-skip в
unit-контуре допустим только тестам с `@pytest.mark.unit_with_db` —
принадлежность маркеру проверяется кодом. Regression-тесты гейта:
`tests/unit/test_skip_gate.py` (subprocess-прогоны, включая xdist).
Probe-файлы уникальны для каждого test/worker; проверки требуют не только
ожидаемый exit code, но и диагностический маркер гейта. Marker metadata
`nightly`/`unit_with_db` передаётся от xdist-worker контроллеру через
`TestReport.user_properties`, поэтому controller применяет те же правила, что
последовательный прогон. Причины и контуры сравниваются точно по отдельным
полям report, а не substring-поиском по объединённому longrepr.

Маркеры `pytest -m security` / `-m integration` выбирают только свои каталоги
(`tests/security/`, `tests/integration/`): хуки `pytest_collection_modifyitems`
фильтруют по пути conftest'а (audit-review 2026-08-22, P1 — раньше маркировалась
вся коллекция, и `-m integration` выбирал заодно unit-набор).
...
| `test_migrations.py` | Alembic upgrade/downgrade idempotency + параметризованный `test_migration_revision_round_trip(revision)` по последним ревизиям (`head → downgrade {rev}-1 → upgrade {rev} → upgrade head`; глубина — `MIGRATIONS_ROUNDTRIP_DEPTH`, default 3, `all` — полный охват); полный пошаговый `test_migrations_stepwise_down_up` — `@pytest.mark.nightly` (запускается в nightly-flakes с `NIGHTLY=true`). Round-trip с возвратом к head даёт O(N²) шагов — полный охват на каждом PR стоил ~90 сек. Список ревизий формируется через `pytest_generate_tests` из `ScriptDirectory.walk_revisions` (без поднятия БД на этапе collection) |
...
Workflow определён в `./.forgejo/workflows/ci.yml`. Реализованные job'ы:
...
| `diff-coverage` | PR only (после `backend-coverage` + `frontend-unit`) | **diff-coverage гейт ≥80%**: новый/изменённый код в PR обязан быть покрыт. В отличие от абсолютного покрытия (меряет всю базу), этот job смотрит только строки из `git diff origin/main` — останавливает регресс. `diff-cover` (Python) на backend `coverage.xml` (Cobertura) + frontend `lcov.info`; запуск из директории контура (важно для путь-мэтчинга). Локально: `./scripts/diff-cover.sh backend\|frontend`. Branch protection: required check `coverage / diff-coverage gate (≥80% new code)` |
| `drift-checks` | push/PR | три слитые drift-проверки в одном job'е (экономия чекаутов/установок зависимостей): перегенерирует `openapi.json` (`./backend/scripts/export_openapi.py`), из него `frontend/src/api/types.gen.d.ts` (`npm run gen:types`) и `docs/db-schema.generated.md` (`generate_db_schema_doc`, PA-007); падает с подсказками команд по каждому файлу, если результат отличается от закоммиченного |
...
| `frontend-e2e` | push/PR (после `backend-integration`; lint jobs идут независимо, timeout 20 мин) | повторно использует подготовленный `portal-postgres:ci` + redis, `alembic upgrade head`, поднимает uvicorn в фоне, `playwright install --with-deps chromium`, прогоняет `chromium`-проект в `E2E_MODE=preview` (production-сборка `npm run build && vite preview`, аудит 2026-08-23 P1.7; `VITE_API_TARGET=http://localhost:8000` — preview проксирует `/api`); артефакт `playwright-report/` (retention 14 дней). После прогона: flaky-бюджет (≤2) и skip-гейт (любой `<skipped>` в junit = провал), авто-фикстура pageerror/console.error в `tests/e2e/lib/test.ts` |
...
|----------|---------|-----------|
| `./.forgejo/workflows/nightly-flakes.yml` (`flake-run`, matrix 1..5) | cron `17 2 * * *` + `workflow_dispatch` | поднимает PG+Redis, `alembic upgrade head`, прогоняет полный suite (`tests/unit tests/security tests/integration`) по одному разу в каждой из 5 matrix-job (случайный порядок тестов сохранён — разные сиды ловят order-dependent flakes; runner убивает job через 30 минут, поэтому цикл из 5 прогонов в одной job был невозможен); каждая job печатает `FAILED `-строки в `$GITHUB_STEP_SUMMARY`; артефакты `flake-reports-run-N/` (junit XML + лог, retention 30 дней). Красная часть матрицы = flake. Образ дополнительно тегируется `portal-postgres:16` — testcontainers-тесты миграций находят его локально без pull из registry |
| `./.forgejo/workflows/nightly-security.yml` (`zap-baseline`) | cron `37 3 * * *` + `workflow_dispatch` | поднимает PG+redis+uvicorn (изолированные контейнеры со случайными портами), запускает `ghcr.io/zaproxy/zaproxy:stable zap-baseline.py -t http://localhost:8000 -c zap-baseline.conf` с конфигом `./security/zap-baseline.conf`; конфиг и отчёты прокидываются через `docker create → docker cp → docker start` (bind-mount workspace недоступен host-docker-daemon из-под act-runner); парсит `zap-report.json` (High/Medium/Low/Info) в `$GITHUB_STEP_SUMMARY`; артефакт `zap-report/` (HTML+JSON, retention 30 дней); фейлит job при наличии High-risk alerts |
...
| `publish-images` | push в `main` **и** тег `v*` (НЕ на PR) | gate `needs` сохраняет все blocking quality/contract/e2e проверки. Matrix публикует 6 образов в Forgejo Registry (`portal-backend`, `-frontend`, `-nginx`, `-nginx-config`, `-screenshot`, `-postgres`); теги: на main — `sha-<7>` + `latest`, на `v1.2.3` — также `v1.2.3`, `v1.2`, `v1`. После push выполняется один authenticated Trivy image-scan: найденные HIGH/CRITICAL информационные, но 401/недоступный образ/битый JSON проваливают job. JSON валидируется и конвертируется в table без повторного registry scan. `~8–12 мин` (с кэшем ~3–5 мин) |
...
- ✅ OWASP ZAP api-scan (passive) по импорту OpenAPI-спеки — нет High-алертов + coverage-гейт ≥100 URL (см. `docs/test-report.md`)
...
| Frontend E2E (Playwright) | переменные `E2E_ADMIN_EMAIL`/`E2E_ADMIN_PASSWORD` | локально задать в env запуска; обязательный CI передаёт test-only bootstrap credentials и падает при их отсутствии, чтобы authenticated suites не превращались в skip |
| Migrations stepwise round-trip | nightly-flakes (`NIGHTLY=true`); локально: `INTEGRATION_DB=true NIGHTLY=true` |

> Локально это удобно — но в CI не должно оборачиваться «зелёным» прогоном с нулевым
> покрытием. В текущем `.forgejo/workflows/ci.yml` integration и authenticated E2E
> входят в обязательный PR/push pipeline; CI передаёт им нужные test-only credentials
> и infrastructure env. Nightly дополнительно запускает repeated/seeded проверки.
> Для аудита фактически прогоняемых тестов используйте `pytest --co -q | tail` и
> `pytest -rs` (вывод reasons для пропусков).
...
3. **Playwright E2E** в CI: `chromium`-проект на каждом PR/push (job `frontend-e2e` поднимает backend через uvicorn + PG/Redis); `firefox`/`webkit` — локально (в nightly-workflows браузерная матрица не запускается). `fullyParallel: true`, `workers: 4` в CI; спеки с межтестовыми зависимостями (`kb-acl`, `kb-media`, `photos`) маркированы `test.describe.configure({ mode: 'serial' })`.
