...
- **Отображение в профиле + ручная загрузка — ГОТОВО** (добор в PR #76):
  `UserAbsencesCard.vue` (блок «Дата — Причина» над коллегами), READ API
  `GET /users/{user_id}/absences`, кнопка загрузки файла отсутствий в
...
## Чеклист (DoD) — отображение (частично выполнено в PR #76, добор)

- [ ] виджет «кого нет на неделе» (главная / отдел) — будущая задача
- [x] отсутствия в карточке сотрудника — `UserAbsencesCard.vue` (блок «Дата —
      Причина» над коллегами, только актуальные/будущие, i18n-метки 7 kinds)
- [x] ручная загрузка файла отсутствий — блок `absencesActions` в `ErpSyncTab.vue`
      (клон дней рождения: запуск + multipart-upload)
- [x] история запусков отсутствий в admin UI — частично (история общая через
      `ErpSyncRuns`, отдельная таблица отсутствий в следующей задаче)
- [x] READ API для профиля: `GET /users/{user_id}/absences` (CurrentUser,
      `end_date >= today`, ORDER BY start_date)
- [ ] READ API для виджета: `GET /absences?from=&to=` (диапазон для «кого нет
      на неделе») — будущая задача
