...
- **2026-08-07 (правка v2 после разбора концепта):** Hero stats card ВОЗВРАЩЕНА.
  Числа из реальных источников через useHeroStats: news total + встречи-сегодня
  (client-side) + active helpdesk-тикеты (если helpdesk включён). Без новых API.
- **2026-08-07 (правка v2):** DOM-архитектура строго по референсу — НЕ единый grid.
  Page → Hero → Content(flex: main flex:1 + aside 360px flex-shrink:0) → Bottom(3 grid).
  Правая колонка — отдельный <aside>, НЕ часть сетки новостей. NewsGrid внутри main —
  единственный grid (3×N). (Урок: LLM склонен сворачивать всю страницу в один grid,
  надо явно задавать DOM-иерархию, а не описывать «внешний вид».)
- **2026-08-07 (правка v2):** Типографика по концепту — hero 42px, section 28px,
  card 20px. Корп. сервисы 4×2=8 плиток. Декоративные SVG (судно в QuickLinks,
  горы в Meetings empty).
