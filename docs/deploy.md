...
> **ADR:** 032, 036, 037, 038, 045, 046, 047, 048.

> ⚠️ **Миграция GitHub → Forgejo в процессе (ADR-048).** Эта страница описывает
> **target-state**: образы — из Forgejo Container Registry (`forgejo.mage.ru/mage/*`),
> deploy-bundle — из Forgejo Release. **Функциональный prod-переключатель (`setup.sh`:
> `download_bundle`/`fetch_latest_release_tag`, `IMAGE_PREFIX`-дефолт) пока указывает на
> GitHub — будет переведён отдельной задачей (prod-switch).** До этого момента
> команды ниже (`curl .../releases/download/...`, `IMAGE_PREFIX=forgejo.mage.ru/mage/`)
> описывают цель, а не текущее поведение `setup.sh`.
...
- **Встраиваемое видео (CSP `frame-src`)** — список origin'ов, iframe с которых разрешён, задаётся в Admin UI → System → «Видео в iframe (CSP)» (`system.json → video_iframe_origins`, ADR-052) и применяется sidecar'ом без рестарта. Один список на портал и learn-домен: он же — sanitize-гейт iframe'ов в новостях/БЗ и гейт видео-плеера материалов обучения. Дефолт: `video.mage.ru`, `youtube-nocookie`, `rutube.ru`, `vk.com`, `vkvideo.ru`, `player.vimeo.com` (зеркало `backend/app/core/constants.py::DEFAULT_VIDEO_IFRAME_ORIGINS` — для старых `system.json` без поля).
...
> - **Prod (без клона репо):** скачать deploy-bundle из Forgejo Release — `curl -fL -H "Authorization: token \$FORGEJO_TOKEN" -o portal-deploy-bundle-v1.2.3.tar.gz https://forgejo.mage.ru/mage/portal/releases/download/v1.2.3/portal-deploy-bundle-v1.2.3.tar.gz`, распаковать в каталог запуска. В bundle уже есть `.env.example`, `docker-compose.yml`, `setup.sh`, `monitoring/`, `system_data/nginx/` (nginx.conf + entrypoint.sh — bind-mount в контейнер, amendment-2 к ADR-046).
> - **Dev/staging (полный клон):** `git clone https://forgejo.mage.ru/mage/portal.git` — все исходники для локальной сборки.
...
- `IMAGE_PREFIX` — режим доставки образов (см. ADR-045, ADR-046): пусто = локальная сборка (dev/staging); `forgejo.mage.ru/mage/` = pull готовых CI-образов (prod). `setup.sh` выставляет автоматически по выбранному профилю контура.
...
- **`IMAGE_PREFIX=forgejo.mage.ru/mage/`** (прод) — pull готовых CI-образов из Forgejo Container Registry. Сервер не компилирует, откат = сменить `IMAGE_TAG` на short-SHA.
- **`IMAGE_PREFIX=` пусто** (dev/staging) — локальная сборка образов из исходников.

### 6.1 Запуск с готовыми образами из registry (прод)

```bash
# .env: IMAGE_PREFIX=forgejo.mage.ru/mage/  и IMAGE_TAG=latest (или sha-XXXXXXX)
docker compose pull
docker compose up -d
...
Порядок старта:
1. `data-permissions-init` (одноразовый root-only job) назначает владельца `1001:1001` только bind-mount'ам приложения и завершается.
2. `postgres` (healthy) → `redis` (healthy)
3. `screenshot-service` (healthy) + `nginx-config` (healthy) — запускаются параллельно
4. `migrations` (одноразовый init-job, `alembic upgrade head`)
5. `backend` + `worker` (после `migrations` exit 0)
6. `frontend`
7. `nginx` (после `backend`, `frontend`, `nginx-config`)

`backend`, `worker`, `nginx-config` и `nginx` работают от непривилегированного UID 1001. Nginx слушает порты 8080/8443 внутри контейнера; Compose сохраняет внешние порты из `HTTP_PORT`/`HTTPS_PORT` (обычно 80/443). Не запускайте ручной `chown` перед обновлением: это делает `data-permissions-init` и его успешное завершение нужно проверить в `docker compose ps`/логах.
...
| `upload_data/` | пользовательские файлы: фото, KB-вложения, news media, аватары, branding, бессрочный архив helpdesk (переписка и вложения) |
...
| Docker volumes observability-overlay | `portal_grafana-data` (правки дашбордов в UI, пользователи) и `portal_alertmanager-data` (silences) — если observability-стек поднят; `prometheus-data`/`loki-data` не критичны (retention 30d, пересоздаются) |
...
Каждый push в `main` собирает и публикует образы в Forgejo Container Registry: теги `sha-<7симв>` (точная
...
deploy-bundle из Forgejo Release тега `IMAGE_TAG` — ручные `curl`/`tar xzf`
больше не нужны. Тег можно сменить в `.env` вручную **либо** позволить
...
#      → распаковать поверх (docker-compose.yml/setup.sh/monitoring/
#        system_data/nginx/ — nginx.conf обновится вместе с bundle, nginx
#        перезапустится на down/up; .env НЕ трогается)
...
`.env`, runtime-`system_data/` (settings/secrets/certs/nginx_conf/nginx_reload),
`base_data/`, `.portal-*` в bundle не входят и не перетираются.
...
> ```bash
> curl -fL -H "Authorization: token \$FORGEJO_TOKEN" -o portal-deploy-bundle-v1.2.3.tar.gz https://forgejo.mage.ru/mage/portal/releases/download/v1.2.3/portal-deploy-bundle-v1.2.3.tar.gz
> tar xzf portal-deploy-bundle-v1.2.3.tar.gz
> cp -a portal-deploy/. . && rm -rf portal-deploy   # merge-copy: содержимое, включая dotfiles (.env.example)
> ./setup.sh   # → п.6
> ```
...
Полная процедура для прод-сервера, который **не держит клон репозитория**. Всё необходимое приезжает двумя путями: образы из Forgejo Container Registry (ADR-045) + deploy-bundle из Release (ADR-046).
...
├── system_data/
│   ├── nginx/                  # nginx.conf + entrypoint.sh — из deploy-bundle (статика)
│   ├── settings/, secrets/, certs/, nginx_conf/, nginx_reload/   # runtime, не в bundle
├── base_data/                  # данные PostgreSQL/Redis (runtime)
├── upload_data/                # пользовательские файлы (runtime)
└── backups/                    # pg_dump (runtime)
```

**Чего тут НЕТ (и не нужно):** `backend/`, `frontend/`, `nginx/Dockerfile`, `postgres/Dockerfile`, `screenshot-service/`, тесты, `*.md` (кроме README bundle), `.git`. Всё это живёт внутри образов из Forgejo Container Registry.
...
|---|---|---|
| `prod` | Продакшен | Образы из Forgejo Container Registry (`IMAGE_PREFIX=forgejo.mage.ru/mage/`); local-build заблокирован; пункты «Разработка»/«Стейджинг» скрыты. |
...
**Проверка:** `https://forgejo.mage.ru/mage/portal/releases` должен показать tarball.
...
docker image inspect forgejo.mage.ru/mage/portal-backend:latest --format '{{.Id}}' \
...
# Прод: обновление
cd <каталог-прода>
curl -fL -H "Authorization: token \$FORGEJO_TOKEN" -o portal-deploy-bundle-v1.1.0.tar.gz https://forgejo.mage.ru/mage/portal/releases/download/v1.1.0/portal-deploy-bundle-v1.1.0.tar.gz   # если на bundle
