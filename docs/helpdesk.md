...
| `POST` | `/tickets/{id}/messages` | Ответ (`Form`: `body_text`, `body_html?`, `visibility`, `cc[]?`, `files[]`). `public` → `pending` + outbound email. `cc` (опц., повторяющееся Form-поле) — адресаты в копии для «Ответить всем» (миграция `083`); бэк нормализует (lowercase, дедуп, отсечение `support_address`/агента/requester), лимит 20 (422 свыше). 201. |
