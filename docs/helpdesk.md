...
- Гонка взятия устранена строковой блокировкой PostgreSQL: `take` и первый ответ агента читают тикет с `FOR UPDATE`; проверка исполнителя, изменение, сообщение/outbox и commit — одна транзакция. Проигравший `take` получает `409`, второй ответ — обычный assignee-lock `403`; лишние audit/уведомления не создаются.
...
- **Email Cc — «ответить всем»** (миграция `083`): заявитель добавляет сотрудников в получатели при подаче заявки по почте → поддержка видит всех участников (`participants` в карточке агента) и может ответить всем (чекбокс «Ответить всем» + редактируемый список Cc в форме ответа). Участники парсятся из заголовков **`To` ∪ `Cc`** (фикс 2026-08-26, прод-тикет TKT-269: Outlook ставит коллегу вторым адресом в `To` рядом с ящиком поддержки — при чтении только `Cc` такой получатель «хаотично» пропадал из карточки в зависимости от привычки отправителя). Cc хранится на уровне сообщения; участники тикета агрегируются в рантайме. В веб-версии создания заявки Cc нет.
- Гостевые заявители (email без аккаунта) создаются без `requester_user_id`; при появлении аккаунта с тем же email — авто-линкование live **и уже архивированных** заявок (`link_guest_tickets` в OIDC-callback).
...
- Архивирование закрытых тикетов в партиционированную таблицу `helpdesk_tickets_archive` (полный jsonb-снимок); файлы хранятся бессрочно в защищённом `upload_data` и входят в backup.
...
| Router | `./backend/app/api/helpdesk/drafts.py` | Черновики ответов: `POST /draft-attachments` (загрузка вложений черновика до отправки ответа). |
| Router | `./backend/app/api/helpdesk/users.py` | `GET /users/search` — typeahead по пользователям для подстановки заявителя/агентов. |
...
| Service | `./backend/app/services/helpdesk/email_quote.py` | Preservation-first отсечение цитат: reply-separator в исходящих + ticket-aware legacy fallback по fingerprint конкретного `TKT-N`; произвольные forward сохраняются. |
...
| Service | `./backend/app/services/helpdesk/email_template.py` | Единый шаблон исходящих helpdesk-писем. Публичные ответы и системные письма содержат видимый reply-separator перед содержимым — точный якорь очистки следующего ответа; история остаётся в письме таймлайном. |
| Service | `./backend/app/services/helpdesk/email_images.py` | Локализация картинок входящего письма при ingress: точные `cid:` и внешние `http(s)://` сохраняются как `HelpdeskAttachment`, `src` переписывается на `/api/v1/helpdesk/attachments/{id}`. Неоднозначные unreferenced inline-части сохраняются обычными вложениями вместо позиционной вставки в чужой `<img>`; inline из приложенных `message/rfc822` не смешиваются с основным телом. SSRF-guard, таймауты и лимиты — fail-closed/best-effort. |
| Service | `./backend/app/services/helpdesk/ingress.py` | Оркестрация IMAP-poll и транзакционный ingest: anti-loop, matching, идемпотентность через `helpdesk_email_log`. |
| Service | `./backend/app/services/helpdesk/ingress_imap.py` | Изолированный IMAP transport: подключение/probe, `SEARCH`, `FETCH` RFC822, `Seen`/`Deleted` flags. Не знает о БД тикетов. |
...
| Service | `./backend/app/services/helpdesk/notifications.py` | In-app уведомления по событиям (через `create_notification` + Redis SSE) + email-уведомления агентам через outbox `kind=generic`: о новой заявке (`notify_ticket_created_email`) и о новом сообщении от заявителя (`notify_requester_reply_email` — несёт **историю переписки** после нового ответа, симметрично `enqueue_reply_outbound` для инициатора; грузит сообщения через `_load_ticket_messages_with_attachments`, собирает историю `build_thread_history`) + тела письма о назначении (`build_assigned_email_*`). |
...
| Service | `./backend/app/services/helpdesk/reads.py` | Per-agent read-state (миграция 080): `mark_ticket_seen` (UPSERT `last_seen_at`, без commit), `has_unread_requester_messages` (EXISTS публичного inbound-сообщения новее `last_seen_at`), `enrich_with_unread` (один запрос для всего списка инбокса → map `{ticket_id: bool}`, защита от N+1), `count_unread_assigned_tickets` (COUNT(DISTINCT) активных назначенных с непрочитанными ответами — `unread` в `GET /tickets/counts`, красный бейдж меню), `count_my_unread_tickets` (заявительское зеркало, `direction='outbound'` — `unread` в `GET /tickets/my/counts`). |
| Model | `./backend/app/models/helpdesk.py` | 12 моделей: `HelpdeskTicket`, `HelpdeskMessage`, `HelpdeskAttachment`, `HelpdeskAgent`, `HelpdeskEmailLog`, `HelpdeskMailboxSettings`, `HelpdeskDigestSettings`, `HelpdeskTicketArchive`, `HelpdeskTicketRead`, `HelpdeskDraftAttachment`, `HelpdeskMaxBotSettings`, `MessengerOutbox`. |
| Schema | `./backend/app/schemas/helpdesk.py` | Pydantic-схемы + StrEnum-наборы (`HelpdeskStatus`/`Source`/`Direction`; enum `Visibility` удалён миграцией `084` вместе с колонкой `helpdesk_messages.visibility`). |
...
| `cc` | `JSONB` NULL | Участники-получатели письма (миграция `083`): `[{"email":"a@x","name":"Иван"}]`. Для inbound — из заголовков `To` ∪ `Cc` входящего письма (`threading.extract_cc`, фикс 2026-08-26; раньше только `Cc`); для outbound — из формы ответа агента (чекбокс «Ответить всем»). `None` для старых сообщений и ответов без копии. Участники тикета «в сборе» НЕ хранятся — агрегируются в рантайме в сериализаторе карточки. |
| `attachment_warnings` | `JSONB NOT NULL DEFAULT '[]'` | Диагностика отклонённых ingress-вложений (миграция `095`): список причин, почему вложение письма не попало в `helpdesk_attachments` (не прошёл MIME/размер и т.п.). Сериализуется агентской карточкой через `message_to_out(include_attachment_warnings=True)`; заявителю не показывается. |
...
| `content_type` | `String(255)` NOT NULL | MIME через `python-magic`; legacy `.doc/.xls/.ppt` из email нормализуются по согласованным MIME-заголовку и расширению, так как libmagic возвращает generic OLE-тип |
...
`PARTITION BY RANGE (closed_at)`, composite PK `(id, closed_at)`, партиции помесячно (`helpdesk_tickets_archive_YYYY_mm`, создаёт `ensure_helpdesk_archive_partitions`). Хранит полный jsonb-снимок (`ticket`, сообщения, Cc, авторы и метаданные вложений); сами байты вложений и inline-медиа остаются в `/data/helpdesk/TKT-{number}` **бессрочно** и входят в backup `upload_data`. Read-only, без FK. При `GET /tickets/my?status=closed` виден только своему заявителю; при агентском `GET /tickets?status=closed` и detail — агентам/админу. Архивные карточки возвращаются с `archived=true` и не допускают мутаций.
...
| `GET` | `/tickets/my` | Свои заявки (`?status`, `?unassigned`, `?assigned`, `?limit`, `?offset`). При `status=closed` объединяет ещё не перенесённые live-заявки и физический JSONB-архив, поэтому история не исчезает после retention. Каждая строка содержит `unread: bool` — есть ли публичные ответы агентов новее `last_seen_at` (заявительский контракт, `direction='outbound'` в `enrich_with_unread`). `unassigned=true` — только тикеты без агента (блок «ожидают принятия»); `assigned=true` — только назначенные (блок «в работе у специалиста»). Взаимоисключающие. |
| `GET` | `/tickets/my/counts` | Лёгкий count: `{active: N, unread: M}` — свои тикеты в статусах new/open/pending (для бейджа в меню «Поддержка»). `unread` — сколько из активных имеют непрочитанные ответы агентов (`reads.count_my_unread_tickets`, `direction='outbound'`; закрытые исключены). |
| `GET` | `/tickets/my/{id}` | Своя заявка с **публичными** сообщениями; для отсутствующей live-строки читает архивный снимок. Архивная карточка read-only (`archived=true`). |
...
| `GET` | `/tickets/{id}/archive-attachments/{filename}` | Скачать файл из архивного снимка. ACL: свой заявитель либо агент/админ; `filename` сверяется с сохранёнными метаданными, поэтому нельзя получить файл подбором имени. |
...
| Метод | Путь | Назначение |
|---|---|---|
| `GET` | `/tickets` | Инбокс: фильтры `status`, `assignee`, `unassigned`, `source`, `active_only` (new/open/pending — для двухблочного вида), `q` (полнотекстовый — см. ниже), пагинация. При `status=closed` объединяет live и физический архив; в архиве поиск совместимый по тексту снимка. Каждая строка содержит `unread: bool` (есть ли непрочитанные ответы заявителя для этого агента — см. §3 `helpdesk_ticket_reads`); считается одним запросом на весь список (`enrich_with_unread`). |
| `GET` | `/tickets/counts` | Лёгкий count: `{active: N, unread: M}` — тикеты, назначенные этому агенту, в статусах new/open/pending (для бейджа в меню «Инбокс поддержки»). «Моя нагрузка», не объём очереди: неназначенные не считаются. `unread` — сколько из активных назначенных имеют непрочитанные ответы заявителя (`reads.count_unread_assigned_tickets`, та же семантика непрочитанности, что у `enrich_with_unread`; закрытые исключены — иначе красный бейдж «залипал» бы на архивной истории). |
...
| `GET` | `/tickets/{id}` | Карточка (`TicketAgentOut`, **все** сообщения + служебные поля + `participants` — requester ∪ Cc всех сообщений ∪ авторы сообщений, агрегация в рантайме, миграция `083`). Для отсутствующей live-строки возвращает архивный снимок с `archived=true`; он доступен только для просмотра. |
...
| `DELETE` | `/tickets/{id}` | **Полное (hard) удаление заявки — только администратор** (`AdminDep`, не `HelpdeskAgentDep`): 403 для агента/заявителя/прочих ролей. Hard-delete строки `helpdesk_tickets` → каскадно (БД `ON DELETE CASCADE`) чистит сообщения, вложения-записи, marker-reads; файлы вложений/inline-картинок на диске удаляет `delete_ticket_dir` (best-effort). Необратимая операция (спам-очистка / GDPR-удаление) — в отличие от `close`/`reopen`. Архив (`helpdesk_tickets_archive`) **не** трогается (отдельный read-only контур; для живого тикета архивной копии обычно ещё нет). Уведомления (email/in-app) **не** шлются — тихое удаление; единственный след — запись аудита `helpdesk.ticket_deleted` (number + subject + previous_status). 204 без тела. Несуществующий id → 404 (через `_load_agent_ticket`). Pending-записи outbox (`email_outbox`/`messenger_outbox`) по исчезнувшему тикету не удаляются — cron-DLQ прибирает; жёсткая связь outbox↔ticket отсутствует сознательно (outbox-инвариант). |
...
- **Не входит**: глобальный поиск портала (Cmd-K) helpdesk не подключён (фронт `useGlobalSearch` идёт тремя отдельными запросами; подключение тикетов в палитру — отдельная UI-задача). На странице архива агентский `q` ищет по сохранённому тексту JSONB-снимка; это совместимый substring-поиск, не FTS-ранжирование live-инбокса.
...
- **Блокировка за назначенным агентом (assignee-lock)** — правило «одна заявка — один исполнитель». Если на тикете назначен агент (`assignee_user_id IS NOT NULL`), то работу с ним (комментарий, смена статуса, reopen) ведёт **только он**: остальные агенты получают **403** `{"message": "Ticket is assigned to another agent", "assignee_user_id": ...}`. Чтобы продолжить работу, не-владелец обязан сначала сменить ответственного на себя через `assign`/`take` (см. выше). **Админ подчиняется тому же правилу** — admin-bypass'а нет, даже администратор должен переназначить тикет на себя. Неназначенный тикет (`assignee_user_id IS NULL`) пропускается — любой агент может взять его в работу (`take` или первым ответом, который авто-назначает). Проверка — `_require_ticket_owner` в роутере (`./backend/app/api/helpdesk/tickets.py`); ставится на `POST /tickets/{id}/messages` (агентский ответ), `PATCH /tickets/{id}/status`, `POST /tickets/{id}/reopen`. **Не затрагивает:** `assign`/`take`/`assignable-agents` (доступны всем агентам всегда — это канал «передать заявку»), заявительские `/tickets/my/*`, IMAP-ingress (системная операция — заявитель всегда может ответить по почте, тикет реопенится как обычно), download вложений, просмотр карточки. На фронте (`HelpdeskAgentTicketDetailPage.vue`) форма ответа заменяется на `<n-alert>` «Заявку ведёт {assignee_name}», status-select и reopen блокируются; assign-popover в `TicketInfoCard.vue` остаётся доступным. Inline-перехват 403 → специфичный toast + перезагрузка карточки (assignee мог смениться в другой вкладке).
- **Удаление заявки (`DELETE /tickets/{id}`)** — **только администратор** (`AdminDep` = строго `role == "admin"`, иначе 403). Не доступно helpdesk-агентам и заявителю — это необратимая hard-delete-операция (БД + файлы), в отличие от `close`/`reopen`/`assign` которые делает агент. Контраст с остальными тикет-эндпоинтами (gated `HelpdeskAgentDep`) сознательный: удаление — административная санкция (спам/GDPR), не операционная работа агента. Единственный admin-only-эндпоинт над тикетом (как и CRUD агентов/mailbox-настроек). На фронте кнопка рендерится только при `auth.isAdmin` под `NPopconfirm`-диалогом (см. §14).
...
- **Serve** — `GET /tickets/{ticket_id}/inline-media/{filename}` → заголовок `X-Accel-Redirect: /internal/helpdesk-media/TKT-{number}/inline/{file}` (nginx location `alias /data/helpdesk/`). После переноса в архив endpoint делает read-only fallback к архивной записи, поэтому исторические картинки не ломаются. `no-store` + `nosniff` — картинки приватны (ACL по тикету).
...
5. **Инициатор**: `From` → нормализованный email → `LOWER(users.email)`; найден → `requester_user_id`, иначе гостевая заявка. **Участники-получатели** (миграция `083`): заголовки `To` ∪ `Cc` парсятся `threading.extract_cc` (фикс 2026-08-26: раньше только `Cc` — получатель в `To` рядом с ящиком поддержки терялся, прод-тикет TKT-269) → `[{"email","name"}]` сохраняется в `helpdesk_messages.cc`. Из списка выкидываются `support_address`, его plus-алиасы (`support+TKT-123@…`, RFC 5233), `support_reply_to` и сам отправитель (иначе ответ «всем» уйдёт в собственный ящик → петля/дубль). `Bcc` не парсится (невидим получателю по RFC 5322). Участники тикета «в сборе» (`participants`) агрегируются в рантайме в сериализаторе карточки — не хранятся в БД.
6. Тело: `text/plain` предпочитается, иначе деривация из sanitized `text/html` (`nh3`). **Отсечение цитаты** предыдущего письма — маркер-разделитель `REPLY_MARKER_TOKEN` + эвристика quoted-reply (см. ниже). **Forward для новых заявок не отрезается** (`keep_forward=True`): forward-блок часто является сутью обращения. Приложенный forward `message/rfc822` сохраняется целиком как `.eml`, включая внутреннюю переписку и файлы, и не flatten'ится в основной MIME. **Картинки и вложения:** `cid:` временно разрешён только в промежуточном sanitized HTML; точный `Content-ID` переписывается на локальный URL, затем перед записью выполняется обычная повторная санитизация. Неоднозначные inline-части сохраняются скачиваемыми вложениями вместо позиционной вставки; известные логотипы корпоративной подписи отбрасываются как мусор. Обычные `Content-Disposition: attachment`-части сохраняются независимо от очистки текста через `save_image_bytes` с MIME- и size-проверками.
...
- **Единый читаемый шаблон письма** (`email_template.py` → `render_reply_email`): Times New Roman 14px, компактная шапка, таймлайн ответа и истории, вложения и футер. Перед содержимым публичного ответа/системного письма ставится видимый reply-separator «Ответьте выше этой линии». Он переживает Outlook/Gmail/SOGo round-trip и позволяет точно удалить нашу процитированную историю, не затронув новый forward пользователя выше separator.
...
- **Email-уведомление агенту о новом сообщении от заявителя несёт историю переписки** (симметрично письму-ответу инициатору выше): `notify_requester_reply_email` (`./backend/app/services/helpdesk/notifications.py`) через outbox `kind=generic` отправляет письмо, в котором после нового ответа заявителя идёт предшествующая переписка теми же таймлайн-блоками (`render_history_block`). Агент видит в почтовом клиенте тот же диалог, что и инициатор — контекст заявки без открытия портала. История собирается переиспользуемым механизмом: `_load_ticket_messages_with_attachments` (явный `SELECT … ORDER BY created_at` с `selectinload(attachments)`, т.к. в ingress-пути `ticket.messages` не подгружен) → `build_thread_history(messages, exclude_id=message.id, …)` → прокидывается в `render_requester_reply_agent_email(history_html=…, history_plain=…)`. Лимит `HISTORY_MAX_MESSAGES=20` тот же, что у письма инициатору. Картинки истории корректно встраиваются воркером `_embed_helpdesk_images_into_generic` (маркер `payload.smtp_source=helpdesk` + `ticket_number` уже стоят). Первый ответ заявителя (предыстории нет) → блок истории пустой, письмо остаётся как раньше. Письмо идёт через outbox `kind=generic` (без threading-заголовков — агент работает через портал/инбокс, не отвечая на это письмо).
...
Реализована preservation-first схема в `./backend/app/services/helpdesk/email_quote.py`:

1. **Наш reply-separator (основной слой)**: `REPLY_MARKER_TOKEN = "Ответьте выше этой линии"` ставится в HTML и plain публичных ответов, писем о регистрации и назначении. Всё ниже него в следующем reply — точно наша история.
2. **Ticket-aware legacy fallback**: старый unmarked quote удаляется только если тот же quote-блок содержит fingerprint текущего тикета (`[#TKT-N]`, `TKT-N`, `#N —`). Ambiguous forward без fingerprint сохраняется. Поддерживаются клиентские паттерны:
   - Outlook (en): `From:/Sent:/To:/Subject:` блок + `-----Original Message-----`
   - Outlook (ru): `От:/Отправлено:/Кому:/Тема:` + `----- Исходное сообщение -----`
   - Gmail (en): `On … wrote:`
   - Gmail (ru): `… написал(а):`
   - SOGo / Mail.ru-веб (en): `<date>, <author> wrote:` — без ведущего `On` (корпоративный webmail mail.mage.ru на базе SOGo формирует именно такой заголовок).
   - SOGo / Mail.ru-веб (ru): `<дата>, <отправитель> писал(а):` / `писал:` / `писала:` — **без приставки «на-»** (в отличие от Gmail-ru `написал(а)`). Регрессия прод-тикета №60: ответы через веб-клиент mail.mage.ru сохранялись с неотрезанной историей переписки, т.к. краткая форма `писал(а)` не покрывалась Gmail-ru паттерном.
   - HTML: quote-контейнеры с классами `gmail_quote`/`moz-cite-prefix`/`WordSection1/2`
   - HTML (SOGo/Mail.ru-веб): текст-разделитель `писал(а):`/`wrote:` внутри `<p>`/`<div>` **перед голым `<blockquote>`** (без классов). SOGo оборачивает процитированное письмо в `<blockquote><figure class="table">...#N — тема...</figure></blockquote>`, а над блоком ставит строку-разделитель. Сам `<blockquote>` не трогается (легитимное форматирование) — отсечение идёт по предшествующему разделителю (`_HTML_SOGO_HEADER_RE`), аналог Outlook `From:/Sent:` header.
   - HTML (Roundcube / Outlook-веб): forward/reply-маркер `-----Original Message-----` / `----- Исходное сообщение -----` внутри `<div>`/`<p>` (HTML-аналог первого plain-паттерна, `_HTML_ORIGINAL_MESSAGE_RE`). Roundcube webmail оформляет цитату как `<div>-------- Исходное сообщение --------</div><div>От:</div><div>Дата:</div>...` — без `<b>` (как у Outlook-Desktop) и без quote-классов, поэтому `_HTML_OUTLOOK_HEADER_RE` и `_HTML_QUOTE_RE` её пропускали. Регрессия прод-тикета №72.

**Точка вызова** — `_extract_bodies` (`ingress.py`): strip применяется к `plain` и `html` до санитизации, и повторно — к деривации plain ← html. MIME body выбирается структурно без обхода вложенных `message/rfc822`; inline-карта следует той же границе и не забирает картинки из приложенного письма. Сырьё (неочищенное тело) **не сохраняется** — при `delete_after_fetch=false` оригинал доступен в почтовом ящике.

**Numeric HTML entities** (регрессия прод-тикета №93): SOGo и некоторые веб-клиенты кодируют весь русский текст письма в numeric entities (`писал(а):` → `&#1087;&#1080;&#1089;&#1072;&#1083;(&#1072;):`). Поскольку `strip_quoted_html`/`strip_quoted_reply` гоняются по **сырому** письму (до `nh3`-декодирования), regex с буквальным русским текстом не матчатся. Решение — `_decode_numeric_entities` на входе обеих функций: декодирует только `&#NNN;`/`&#xNNN;` (сущности с русским текстом), **не трогая именованные** `&lt;`/`&gt;`/`&amp;`/`&nbsp;` (они структурно значимы: email в `&lt; &gt;`, разделители). Делает все 8 кириллических паттернов рабочими на сырых SOGo-письмах. Безопасно: `strip_quoted_*` работают regex'ом по тексту (не исполняют HTML), а `sanitize_html` (nh3) всё равно декодирует entities и вычищает всё лишнее при сохранении в БД.
...
- **Ответ на существующий тикет** → separator режется точно; для legacy-письма удаляется только quote с fingerprint этого тикета. Новый nested forward без fingerprint сохраняется.
...
**Ограничение:** неуверенно распознанная legacy-цитата намеренно остаётся в ленте. Лишний текст можно скрыть/посмотреть в UI; потерянный контекст восстановить нельзя.

### Отсечение корпоративной email-подписи (`email_signature.py`)

Корпоративная подпись удаляется из тел **до** сохранения в БД. Новые PC/Apple/Web/Phone-подписи несут `data-portal-email-signature="mage-v1"`; legacy table-layout распознаётся по известному логотипу, включая Outlook-вариант `cid:` с именем/`Content-Location` в MIME, либо полному сочетанию нормализованных border + name-color (`hex`/`rgb`) + corporate mailto. Одиночный `mailto`, цвет или border ничего не удаляет. Удаляется только контейнер подписи, поэтому forward после него сохраняется. Для plain-text поддерживаются RFC 3676 delimiter `-- ` и sign-off фразы только в авторском сегменте перед forward.
...
**Картинки заявки в письме агентам (CID-встраивание).** Текст заявки (`description_html`) может содержать картинки — локализованные email-attachments из ingress (`<img src="/api/v1/helpdesk/attachments/{id}">`) или rich-картинки из формы создания. В веб-ленте портала они видны (session-cookie есть), но в почтовом клиенте — **нет**: endpoint `/attachments/{id}` требует аутентификации, почтовик cookie не передаёт → 401 → битая картинка. Поскольку письмо агентам идёт через `kind=generic` (не `kind=helpdesk`), оно не проходит через `_build_helpdesk_mime`, где CID-встраивание уже сделано (см. ниже). Фикс: продюсер ставит в `payload` связку `smtp_source="helpdesk"` + `ticket_number` (маркер + путь к файлам), а воркер `process_email_outbox` для таких generic-писем вызывает `_embed_helpdesk_images_into_generic` — те же embed-функции, что у helpdesk-ветки (`_embed_helpdesk_inline_images` + `_embed_helpdesk_attachment_images`), переписывают `src` на `cid:` и кладут результат в `payload.inline_images`, после чего `_build_mime` собирает `multipart/related`. Сводка (digest) `ticket_number` не ставит → преобработка её пропускает. Best-effort: нет картинок / файл не найден / неподдерживаемый формат → `inline_images` не пишется, `_build_mime` возвращает обычный `alternative` (0 regressions); письмо не роняется. Поддерживаемые форматы: jpeg/png/gif/webp.
...
| `cleanup_helpdesk_attachments_task` | `hour=4, minute=0` | нет | `helpdesk:cleanup:lock` / 10 мин | Compatibility no-op: задача сохранена для безопасного rollback, файлы архива не удаляет |
...
| `HELPDESK_MAX_TOTAL_OUTBOUND_MB` | 15 | Максимум суммы файлов в ответе агента; запас учитывает base64-рост SMTP-письма |
...
| `HELPDESK_ATTACHMENT_ALLOWED_MIMES` | frozenset | Разрешённые MIME вложений (png/jpeg/pdf/docx/xlsx/…) |
...
- **Красная подсветка непрочитанных** (оба пункта): при `unread > 0` цифра получает модификатор `.menu-count-badge--unread` — красный pill (`--color-danger`), tooltip дополняется (у «Поддержки» — «есть непрочитанные ответы поддержки» из `GET /tickets/my/counts`; у «Инбокса» — «есть непрочитанные ответы» из `GET /tickets/counts`). Гаснет сразу после открытия карточки (`markTicketRead`/`markMyTicketRead` → инвалидация `agentTicketCounts`/`myTicketCounts`), не дожидаясь поллинга.
...
- **`HelpdeskAgentInboxPage`**: **двухблочный вид** (не плоский список):
  ...
  - **Живое обновление**: оба блочных запроса (новые / в работе) опрашивают сервер раз в 60 c (`refetchInterval` у `useAgentInboxQuery`, синхронно с поллингом счётчика меню) — новые заявки, ответы и цифры секций подтягиваются без F5, пока страница открыта. В неактивной вкладке поллинг приостанавливается (дефолт `refetchIntervalInBackground=false`). Поиск — on-demand, без поллинга.
  ...
  - **Подсветка непрочитанных** (миграция 080): каждая строка инбокса приходит с бэка с полем `unread: bool` (один запрос `enrich_with_unread` на весь список — без N+1). `unread=true` для тикетов, у которых есть публичные входящие сообщения (ответы заявителя) новее, чем `last_seen_at` агента — то, что пришло за ночь и ещё не открывалось. Визуально: красная точка `--color-brand-red` перед `#номером` + полупрозрачный фон строки + жирный subject (единый язык с `NotificationsDropdown`). `unread=undefined` (для не-агентских списков, например `/tickets/my` у заявителя) — рендерится как прочитанное. Снятие подсветки — при открытии карточки: `HelpdeskAgentTicketDetailPage.load()` вызывает `markTicketRead(ticketId)` (best-effort, не блокирует UI) и после успеха инвалидирует `agentTicketCounts` — красный бейдж меню гаснет сразу.
- **`HelpdeskAgentTicketDetailPage`**: действия `take` / смена статуса (через select) / `reopen`; email-метаданные сообщений (agent-mode в `TicketMessageList`). При открытии — `markTicketRead` (снять подсветку в инбоксе, см. выше). **Удаление заявки** (кнопка `type="error"` в `#actions` шапки под `NPopconfirm`-диалогом) — **admin-only** (`v-if="auth.isAdmin"`): вызывает `deleteTicket`, после успеха — toast + переход в инбокс (карточка исчезнувшего тикета больше недоступна, `load()` не вызывается). Не-админ кнопку не видит. См. §4 `DELETE /tickets/{id}`, §5 ACL.
