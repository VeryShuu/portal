...
- **Email-ingress** (картинки + обычные вложения): при IMAP-фетче все картинки письма и `Content-Disposition: attachment`-части локализуются в локальный FS через `attachments.save_image_bytes` (источник — `bytes`, те же magic/лимиты/guard, что у web-upload). Картинки:
  ...
  - **Внешние `http(s)://`** — httpx-выкачка с SSRF-guard (private/loopback/link-local blocked, DNS-rebinding guard через `getaddrinfo`), таймаут 10 c, лимит 25 MiB; content-type проверяется до сохранения.
...
4. **Matching**: по `In-Reply-To`/`References` → `helpdesk_messages.email_message_id`; fallback по токену `[#TKT-{number}]` в теме; последний (опциональный) fallback — plus-маркер `+TKT-{number}` в адресе получателя (`Delivered-To`/`X-Original-To`/`To`). Нет матча → новый тикет (`source=email`). `[#TKT-N]` найден, но живого тикета нет (в архиве) → новый тикет с `references_archived_ticket_number=N`.
