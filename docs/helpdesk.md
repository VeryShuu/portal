...
| Service | `./backend/app/services/helpdesk/email_template.py` | Единый HTML-шаблон исходящих helpdesk-писем: `render_reply_email` (ответ агента + история) и `render_system_email` (назначение) — жёстко зафиксированный шрифт Times New Roman 14px (единый размер во всём письме, иерархия через `font-weight`/`color`), компактная шапка (единый заголовок «#номер — тема» — без статуса/исполнителя/обновления), минималистичный таймлайн переписки (имя автора в accent/grey + приглушённая подпись роли «Специалист поддержки»/«Исполнитель»; горизонтальные `<hr>`-разделители на всю ширину письма между сообщениями; без левых вертикальных полос/карточек/бейджей), эвристическое приглушение email-подписей отправителей («↧ Подпись скрыта»), компактный блок вложений «📎 Вложения» с размерами, заметный reply-разделитель с токеном (двойная линия 3px + ↩, увеличенные отступы), футер (призыв «Вы можете оставить комментарии по заявке ответив на это письмо» — по центру, жирный + ссылка на портал). Inline-стили, табличная вёрстка 600px (паттерн news), совместимость с Outlook/Gmail/Apple Mail. |
...
- **Единый читаемый шаблон письма** (`email_template.py` → `render_reply_email`): жёстко зафиксированный шрифт Times New Roman 14px (единый размер во всём письме, иерархия через `font-weight`/`color`, не через размеры), компактная шапка (единый заголовок «#номер — тема» — без статуса/исполнителя/обновления; белый фон), таймлайн-блок ответа агента (accent-имя + приглушённая подпись «Специалист поддержки»/«Исполнитель»), заметный reply-разделитель (двойная линия 3px + ↩, увеличенные отступы 18px 20px, видимая инструкция «Ответьте выше этой линии» — она же якорь отсечения цитаты), минималистичная история-таймлайн (имя автора в accent для специалиста / grey для заявителя + приглушённая подпись роли; горизонтальные `<hr>`-разделители на всю ширину письма между сообщениями; без левых вертикальных полос/карточек/бейджей/теней — различение участников только цветом имени и подписью роли), эвристическое приглушение email-подписей отправителей (маркеры RFC 3676 `--`, «С уважением»/«Best regards», `<hr>`-разделители → компактный блок «↧ Подпись отправителя скрыта» — подпись в БД и веб-версии тикета остаётся полностью, отсекается только в письме), компактный блок вложений «📎 Вложения» с именами и размерами, футер (призыв «Вы можете оставить комментарии по заявке ответив на это письмо» — по центру письма, жирный + ссылка на портал). Inline-стили, табличная вёрстка 600px (паттерн `news/email_share.py`). Письмо о назначении ответственного — через `render_system_email` (та же шапка/футер, без разделителя/истории). Совместимость с Outlook Desktop/Web/Gmail/Apple Mail (только inline-стили и таблицы, без CSS Grid/Flexbox).
...
1. **Маркер-разделитель в исходящих письмах** (`build_reply_marker_plain`/`_html`) — добавляется в конец `body_text`/`body_html` **только в outbox-копии** тела в `enqueue_reply_outbound` (`./backend/app/services/helpdesk/outbound.py`), **не** в сохраняемое в БД `HelpdeskMessage` (чтобы в ленте портала агент видел свой чистый ответ). Точка отсечения — видимый текст инструкции `REPLY_MARKER_TOKEN = "Ответьте выше этой линии"`: именно эта фраза служит якорем для `strip_quoted_reply`/`strip_quoted_html`. Видимый текст выбран намеренно: скрытые якоря (`font-size:0` / HTML-комментарии) ненадёжно переживают ответ в Outlook (Word-рендер их вырезает), видимый текст — всегда; в этом случае и якорь не виден «как служебный» (это естественная инструкция заявителю), и round-trip гарантирован.
