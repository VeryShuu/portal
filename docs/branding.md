...
- **Фоны Hero главной (`hero-bg-morning` / `hero-bg-day` / `hero-bg-evening`)**: три слота фона по времени суток, расширения `[".png", ".jpg", ".webp"]`. Границы слотов — настройки `hero_morning_hour` / `hero_day_hour` / `hero_evening_hour`; позиционирование — focal-point `hero_*_focal_x/y` (0–100) + `hero_*_zoom` (100–300, `%`), редактор drag+zoom — `HeroBgFocalEditor.vue`.
...
| GET | `/api/v1/branding/hero-bg-{morning\|day\|evening}` | Получить фон Hero-блока для времени суток. Поддерживает HEAD. | Публичный |
| POST | `/api/v1/admin/branding/hero-bg-{morning\|day\|evening}` | Загрузить фон Hero-слота (multipart/form-data). | Редактор (`EditorDep`) |
| DELETE | `/api/v1/admin/branding/hero-bg-{morning\|day\|evening}` | Сбросить фон Hero-слота. | Редактор (`EditorDep`) |
...
- **Связь с системными настройками (ADR-037, ADR-052)**:
  В эндпойнте получения настроек брендинга `get_settings()` динамически подгружаются глобальные системные настройки с помощью `load_system_settings()`. Список `allowed_iframe_origins` (санитайзер iframe'ов в новостях/БЗ) берётся из runtime-настройки `system.json → video_iframe_origins` (Admin UI → System → «Видео в iframe (CSP)»); если включена интеграция внешней видеогалереи (`sys.video_gallery_url`), этот домен дополнительно добавляется в список. Тот же список является CSP `frame-src` (nginx-sidecar) и гейтом видео-плеера материалов обучения.
...
- `"hero_bg_morning"` / `"hero_bg_day"` / `"hero_bg_evening"` — загрузка/удаление фонов Hero-слотов.
