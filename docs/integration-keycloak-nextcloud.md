...
**Проверка мониторинга.** Учётная запись `portal-svc` должна видеть Office в
`GET /ocs/v1.php/cloud/capabilities`. Worker извлекает оттуда внутренний
`richdocuments.config.wopi_url` и отдельно проверяет read-only endpoint
`/hosting/capabilities` самого Collabora/CODE. Ответ Nextcloud 302/404 или
страница входа не считаются доступностью редактора. Для встроенного CODE
поддерживается опубликованный URL вида `.../proxy.php?req=`. Если Office
ограничен группами, включите `portal-svc` в разрешённую группу; скрытая от probe
capability считается DOWN.

Эта проверка подтверждает конфигурацию Office и доступность Collabora, но не
создаёт документ и не проверяет WOPI callback, federation display name или
отрисовку редактора в браузере — это отдельная тестовая приёмка.
