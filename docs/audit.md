...
- Исключения при записи логов через `push_audit_event(...)` или `log(...)` перехватываются, логируются как предупреждения и отправляются в Sentry, но никогда не прерывают выполнение основного HTTP-запроса или транзакции пользователя.
