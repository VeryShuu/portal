...
- **Path traversal (defense-in-depth):** помимо regex-валидации имени, все операции с файлами инлайн-медиа, галереи и вложений (delete/download) проходят через `safe_join_within` (`./backend/app/core/uploads.py`) — жёсткая проверка, что итоговый путь не выходит за пределы директории новости.
- **Встраивание видео:** iframe из доверенного origin `https://video.mage.ru` (константа `TRUSTED_VIDEO_IFRAME_ORIGINS` в `./backend/app/core/constants.py`) разрешён в санитизации тела новости (`nh3`/DOMPurify-контур) и в TipTap-расширении `IframeEmbed` на фронте. Прочие iframe-origin вырезаются.
