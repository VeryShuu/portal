...
Все четыре layout-варианта обёрнуты стабильным marker
`data-portal-email-signature="mage-v1"`. Helpdesk-ingress использует его для
точного удаления подписи без обрезания следующего за ней forwarded-контекста.
...
3. **Добавить ярлык** (рекомендуется): Admin → «Ярлыки сервисов» → создать
   `ServiceLink` с внутренним относительным путём `/signature` (host-agnostic:
   работает в dev/prod/за прокси без правок). Такой ярлык открывается
   SPA-навигацией в той же вкладке (см. `./docs/links-bookmarks.md`, §
   «Внутренние ссылки»). Внешний абсолютный URL (`https://<хост>/signature`)
