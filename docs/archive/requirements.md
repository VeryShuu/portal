...
| **Phase 3.5** | **KB ACL + медиа/импорт/экспорт + UI вложений** | **✅ Завершено** |
| **Phase 4** | **Email + In-app уведомления (SSE)** | **✅ Завершено** |
| **Phase Admin-UI** | **Runtime-настройки (Система/Keycloak/Email/TLS/Branding) через Admin UI** | **✅ Завершено** |
| **Phase 4.5** | **Собственный модуль фотогалереи (ACL, WebP/AVIF, lightbox, sharing)** | **✅ Завершено** |
| **Phase 4.6** | **Видеогалерея — iframe embed в редакторе, внешняя ссылка** | **✅ Завершено** |
| Phase 5 | Файлы через Nextcloud + Collabora | ✅ Код завершён; тесты — Phase 7; деплой требует NC на Keycloak OIDC |
| **Phase 6** | **Аудит-дашборд + Аналитика + Observability** | **☐ Запланировано (следующий шаг)** |
...
- [ ] **Обложка новости** — загрузка изображения (JPEG/PNG/WebP/GIF, max 10 МБ), хранение в local volume `/data/news_media`, URL `/media/news/{filename}`
...
**Подход (Вариант A — service account):** все файловые операции выполняются через единый сервисный аккаунт `portal-svc` с App Password (`NC_SERVICE_APP_PASSWORD`). Nextcloud используется как тупое хранилище; права доступа хранятся и проверяются на стороне портала (таблица `file_permissions` в PostgreSQL). Keycloak `user_oidc` и impersonation через Bearer JWT для файлового модуля **не требуются**.

Полное обоснование и детали реализации: см. `docs/adr.md` **ADR-032** («Nextcloud — service account (Вариант A)»).

Краткие характеристики:
- Аутентификация в NC: `Authorization: Basic base64(portal-svc:APP_PASSWORD)`
- ACL на портале: таблица `file_permissions` (не NC ACL)
- `NC_SERVICE_APP_PASSWORD` хранится в secrets (`.env`), ротация каждые 90 дней
- Keycloak impersonation / user JWT к Nextcloud не применяются
...
| `GET /search` (search abuse) | 60 req / мин / user |
