...
> PostgreSQL 16
> Последнее обновление: сентябрь 2026 (v1.19 — миграции 001..112: к v1.18
> добавлены learning-миграции **103_learning_role_outbox_fix** (column-grant
> SELECT(id) на `email_outbox`, снапшот личности участника),
> **104_learning_attempts_composite_indexes**, **105_learning_certificates**
> (сертификаты PDF), **106_learning_deadlines** (`deadline_at` +
> `deadline_notified_at`), **107_learning_attempt_timer**
> (`time_limit_minutes`), **108_learning_item_description** (rich-описание
> элемента), **109_learning_course_categories** (категории курсов +
> `category_id`), **110_learning_item_type_section** (разделы-заголовки,
> type `section`), **111_learning_for_all_staff** (обязательные курсы «для
> всех сотрудников»), **112_learning_categories_grants** (фикс грантов роли
> на категории — 109 дала только SELECT), **113_learning_login_codes**
> (passwordless-вход: таблица кодов + GRANT), **114_drop_learning_password_relics**
> (дроп парольных колонок учёток и таблицы токенов самовосстановления —
> паролей у внешних учёток больше нет; секция «Модуль обучения», детально —
> `docs/learning.md`).
> Ранее — v1.18 (миграции 001..102:
> **100_meetings_rsvp**, **101_add_learning** (все таблицы модуля обучения —
> секция «Модуль обучения»),
> **102_learning_db_role** (DB-роль `learning_app` + гранты: RW на `learning_*`,
> INSERT-only на `email_outbox`; отдельный пул learning-роутов — env
> `LEARNING_DB_PASSWORD`)).
> Ранее — v1.17 (миграции 001..090: к v1.16 добавлены
...
CREATE TABLE users (
    ...
    avatar_url       VARCHAR(512),                   -- /media/avatars/<user_id>_<ms>.webp (WebP ≤512px, версионированный URL)
    -- Фокальная точка + зум аватара (миграция 096): NULL = центр 50/50, зум 100%.
    -- Паттерн news.cover_focal_*: хранятся числа, отрисовка на фронтенде CSS'ом.
    avatar_focal_x     SMALLINT CHECK (avatar_focal_x IS NULL OR avatar_focal_x BETWEEN 0 AND 100),
    avatar_focal_y     SMALLINT CHECK (avatar_focal_y IS NULL OR avatar_focal_y BETWEEN 0 AND 100),
    avatar_focal_zoom  SMALLINT CHECK (avatar_focal_zoom IS NULL OR avatar_focal_zoom BETWEEN 100 AND 300),
...
| `messenger_outbox` | Transactional outbox для не-email каналов — полный аналог `email_outbox`: `provider` (`'max'`, `'matrix'` — миграция 097), `chat_id`, `text`, `payload` JSONB (attachments + format / formatted_body), `status` (`PENDING/SENDING/SENT/FAILED/DLQ/CANCELLED`), retry/backoff/DLQ. Воркер `process_messenger_outbox` (cron 15с, distributed lock + `FOR UPDATE SKIP LOCKED`, группировка по провайдеру), cleanup — nightly. Retry-классификация: 429/5xx/timeout → transient, 4xx → permanent (DLQ). Для matrix: `chat_id` хранит MXID получателя, `txn_id` = UUID строки (идемпотентность Synapse). |

## Matrix-уведомления (миграция 097)

Персональные уведомления в корпоративный чат Matrix (Synapse + MAS).

| Таблица | Назначение |
|---|---|
| `matrix_bot_settings` | Singleton (id=1, `CHECK (id=1)`) конфигурации Matrix-бота: `homeserver_url` (API endpoint Synapse), `server_name` (server-часть MXID, default `matrix.mage.ru`), `access_token_enc` (Fernet, токен `mct_...` из `mas-cli manage issue-compatibility-token` — MAS-конфигурация не даёт shared-secret/password-login), `bot_user_id` (`@portal-bot:...`, нужен для account data `m.direct`). `enabled=False` по умолчанию. |

MXID-конвенция: `borzihin.vs@mage.ru` → `@borzihin.vs:matrix.mage.ru` (localpart
email в lowercase + `server_name`). Личный opt-in переключатель —
`users.preferences.chat_notifications_enabled` (JSONB, дефолт выключен; меняет
сам пользователь или админ через `/users/admin/{id}/notification-preferences`).

DM-комнаты кэшируются в account data бота `m.direct` (стандартное хранилище
DM — бот видит и комнаты, созданные пользователем вручную). Keyset-индекс
`ix_messenger_outbox_created_id (created_at DESC, id DESC)` — для админской
вкладки «Очередь мессенджеров» (зеркало миграции 091).
...
## Directum (миграция 098)

Интеграция с СЭД Directum (OData, `https://sed.mage.ru/Integration/odata`):
cron-прогон «Просроченные задачи» → ФИО-матчинг → дайджесты в личный чат Matrix
+ email-сводка админам. Подробно — [`./directum.md`](./directum.md).

| Таблица | Назначение |
|---|---|
| `directum_settings` | Singleton (id=1, `CHECK (id=1)`), сеется миграцией: `enabled` («Модуль включён» — гейтит и cron, и manual), `base_url` (default `https://sed.mage.ru/Integration/odata`), `auth_username` + `auth_password_enc` (Fernet, write-only в API — сервисная AD-учётка), `overdue_run_hours INTEGER[]` (миграция 099: часы запуска 0–23 в московском времени, пусто = только вручную), `expected_interval_days` (watchdog), `notify_emails` (NULL = все админы с `notify_email=true`). Per-задачная группа: `overdue_enabled` («Просроченные задачи»). Мастер-гейтинг — `modules.directum.enabled` в modules.json. |
| `directum_runs` | Лог прогонов: `triggered_by('cron'/'manual')`, `status('success'/'partial'/'failed'/'skipped')`, счётчики `tasks_total`/`performers_total`/`users_notified`/`users_skipped_opt_in`/`users_unmatched`/`users_ambiguous`/`errors`, `report` JSONB (notified/skipped_opt_in/unmatched/ambiguous/matrix_disabled/error, кап 200). Skip-причины прогонов (модуль выкл./интервал/lock) run-строк не создают. |
...
| 093 | `add_current_status` | `users.current_status` (`working`/`vacation`/`sick`/`business_trip`, CHECK) — статус отсутствия сотрудника | §«users» |
| 094 | `helpdesk_requester_name_fts` | Расширение `helpdesk_tickets.search_tsvector`: FTS по имени/фамилии инициатора (снимок `requester_name`) | `./helpdesk.md` §3, §4 |
| 095 | `add_helpdesk_attachment_warnings` | `helpdesk_messages.attachment_warnings` (JSONB, default `[]`) — диагностика отклонённых ingress-вложений | `./helpdesk.md` §3 |
| 096 | `add_avatar_focal` | `users.avatar_focal_*` — точка фокуса аватара | §«users» |
| 097 | `add_matrix_bot_settings_and_provider` | `matrix_bot_settings` (singleton Matrix-бота) + `messenger_outbox.provider 'matrix'` (CHECK) + keyset-индекс | [§«Matrix-уведомления»](#matrix-уведомления-миграция-097) |
| 098 | `add_directum` | `directum_settings` (singleton OData-интеграции) + `directum_runs` (лог прогонов «просроченные задачи») | [§«Directum»](#directum-миграция-098) |
| 099 | `directum_schedule_hours` | `directum_settings`: расписание вместо интервала — DROP `poll_interval_seconds`, ADD `overdue_run_hours INTEGER[]` (часы 0–23, CHECK-подмножество; московское время) | [§«Directum»](#directum-миграция-098) |
| 100 | `meetings_rsvp` | RSVP-отметки участников встреч (см. `./meetings.md`) | `./meetings.md` |
| 101 | `add_learning` | Все таблицы модуля обучения: учётки/сбросы/методисты/курсы/элементы/участники/тесты/вопросы/варианты/попытки/прогресс (XOR-участник `user_id` XOR `learning_account_id`) | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 102 | `learning_db_role` | Роль `learning_app` + гранты: RW на `learning_*`, INSERT-only на `email_outbox`; отдельный пул learning-роутов (`LEARNING_DB_PASSWORD`) | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 103 | `learning_role_outbox_fix` | Column-grant `SELECT (id)` на `email_outbox` (INSERT … RETURNING), снапшот личности участника `display_name`/`email` | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 104 | `learning_attempts_composite_indexes` | Составные индексы попыток `(test_item_id, user_id)` и `(test_item_id, learning_account_id)` — история/лимит фильтруют по тесту+участнику (§7 ТЗ) | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 105 | `learning_certificates` | Таблица `learning_certificates` (XOR-участник, частичные уникальные индексы «один сертификат на участника курса»); гранты роли в самой миграции | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 106 | `learning_deadlines` | `learning_courses.deadline_at` + `deadline_notified_at` на строке зачисления (одно напоминание на участника) | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 107 | `learning_attempt_timer` | `learning_tests.time_limit_minutes` (1–600, NULL = без лимита); серверный контроль submit | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 108 | `learning_item_description` | `learning_course_items.description` — rich-описание элемента (Markdown, sanitize на записи) | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 109 | `learning_course_categories` | Справочник категорий `learning_course_categories` + `learning_courses.category_id` (FK SET NULL); гранты роли — только SELECT, дополнены в 112 | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 110 | `learning_item_type_section` | CHECK типа элементов расширен: `type ('material'/'test'/'section')` — разделы-заголовки (в прогрессе не участвуют) | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 111 | `learning_for_all_staff` | `learning_courses.for_all_staff` — обязательный курс: доступ виртуальный, строки участников материализуются (включение флага + Keycloak-синк) | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
| 112 | `learning_categories_grants` | Фикс 109: гранты роли `learning_app` INSERT/UPDATE/DELETE на `learning_course_categories` (без них POST /learning/admin/categories → 500 на контуре с `LEARNING_DB_PASSWORD`) | | [§«Модуль обучения»](#модуль-обучения-миграции-101102) |
...
## Модуль обучения (миграции 101–112)

LMS-модуль: курсы для сотрудников портала и внешних учёток (публичный контур
`learn.<домен>`, ADR-051). Два типа участников во всех
зачислениях/прогрессе/попытках: `user_id` XOR `learning_account_id`
(CHECK «ровно один заполнен»). Детальный модульный док — `docs/learning.md`;
полное ТЗ и журнал решений — `docs/wip/learning.md`.

| Таблица | Назначение |
|---|---|
| `learning_accounts` | Внешние учётки обучаемых (вне Keycloak и вне `users`): email (unique lower среди живых), `status('active'/'blocked')`, `last_login_at`. Passwordless (113–114): паролей нет — вход по одноразовому коду из письма; парольные колонки дропнуты (114). Создаёт только методист. |
| `learning_login_codes` | Одноразовые коды входа (113): только SHA-256 хэш, `expires_at`, `attempts` (≤5 неверных вводов убивают код), `used_at`; новый запрос гасит предыдущий код учётки. Ретеншен — cron `learning_cleanup` (30 дней). |
| `learning_admins` | Методисты (= админы модуля, паттерн `helpdesk_agents`): `user_id` PK; admin портала — суперсет. |
| `learning_courses` | Курсы: `slug` (unique), `status('draft'/'published')`, soft-delete; `cover_path` (WebP-превью в `/data/learning/covers/`), `deadline_at` (106), `category_id` → `learning_course_categories` (109), `for_all_staff` (111, обязательный курс «для всех сотрудников»). |
| `learning_course_items` | Элементы курса: `type('material'/'test'/'section')` (section — раздел-заголовок, 110, в прогрессе не участвует), `sort_order`, `description` (rich-описание, 108); материал — `url` и/или `file_path` (PDF в `/data/learning/`); тест — `test_config_used` (замок правок после попыток). |
| `learning_course_participants` | Зачисление (XOR-участник) + снапшот личности `display_name`/`email` (103); исключение — soft (`deleted_at`), история попыток сохраняется; уникальность пары — частичными индексами; `deadline_notified_at` (106 — одно напоминание на участника). |
| `learning_tests` | Настройки теста 1:1 с item: `pass_score` (%), `max_attempts` (0 = безлимит), `shuffle_questions`/`shuffle_answers`, `time_limit_minutes` (107, таймер попытки). |
| `learning_questions` / `learning_question_options` | Вопросы (single/multi) и варианты (`is_correct`) с `sort_order`. Тест с отправленными попытками не редактируется (правится копией). |
| `learning_test_attempts` | Попытки (XOR-участник): `started_at`/`submitted_at`/`abandoned_at` (лениво через 24ч), `score`, `passed`, `answers` JSONB — снапшот порядка `{version, order, option_order}` + `responses`. Лимит по отправленным; результат — «последний». Индексы: `(test_item_id, submitted_at)`, составные `(test_item_id, user_id)` и `(test_item_id, learning_account_id)` (миграция 104), по участнику. |
| `learning_item_progress` | Отметка «ознакомлен» материала (XOR-участник, unique участник×элемент). % курса = отметки материалов + passed-попытки. |
| `learning_certificates` | Сертификаты о прохождении (105): XOR-участник, `certificate_uid`, `serial` (`LC-XXXXXXXX`), `pdf_path` (`/data/learning/certificates/<course_id>/`); частичные уникальные индексы «один сертификат на участника курса» закрывают гонку ленивой выдачи. |
| `learning_course_categories` | Плоский упорядоченный справочник категорий (109): `title`, `sort_order`, soft-delete (курсы отвязываются в той же транзакции — FK SET NULL срабатывает лишь при hard delete). |

DB-изоляция (§10.8): роль `learning_app` — RW на таблицы модуля + INSERT-only
на `email_outbox`; learning-роуты ходят через отдельный пул
(`app/core/database.py::LearningSessionLocal`, env `LEARNING_DB_PASSWORD` —
обязателен на проде). SELECT штатных таблиц под ролью невозможен —
`tests/integration/test_learning_db_role.py`. **Грабль:** гранты 102 —
табличные, на новые таблицы не переносятся: каждая новая `learning_*`-таблица
GRANT-ит роль в своей же миграции (инцидент 109→112: у категорий был только
SELECT — создание категории падало 500 на контуре с паролем роли).
