...
**Индексы:** `ix_notifications_user_id` на `user_id`; `ix_notifications_created_at` (миграция `085`) — для глобального скана автоочистки.

**Soft-delete:** не предусмотрен — строки удаляются физически через `DELETE /api/v1/notifications/{id}`, а также ежедневным cron'ом `cleanup_notifications` (04:25): прочитанные старше `notifications_read_retention_days` (30 дн) и непрочитанные старше `notifications_unread_retention_days` (90 дн); `0` = отключено (настройки в Admin UI).
