...
photo_folders
  ...
  parent_id (UUID, FK)       -- Ссылка на родительскую папку (photo_folders.id, ON DELETE RESTRICT — миграция 031, nullable)
...
- **Окно хранения:** Удалённые файлы окончательно стираются воркером `purge_expired(ttl_days=30)` после **30 дней** в корзине; листинг корзины (`TrashService.list_trashed_photos` → `fetch_deleted_photos_with_folders` репозитория `./backend/app/services/photos_photo_repo.py`) за окно не фильтрует.
...
| DELETE | `/api/v1/photos/{photo_id}/purge` | Физически удалить фотографию и её файлы. | admin / автор / manager папки |
...
| Метод | Путь | Описание | Права |
|---|---|---|---|
| GET | `/api/v1/photos/thumbnail/{photo_id}/{size}` | Получить миниатюру WebP/AVIF. Размеры: `200`, `400`, `600`, `1000`, `1600`. Если миниатюра не сгенерирована, API ставит задачу в ARQ и отдаёт **сам оригинал** как fallback с заголовками `X-Thumb-Status: original-fallback`, `Cache-Control: no-store`. Если AVIF отсутствует, а WebP есть — отдаёт `404 X-Thumb-Status: no-avif`. | `viewer` |
...
1. **Разгрузка HTTP-процессов:** Любые операции с изображениями (PIL) запрещены внутри API-контроллеров. Если миниатюры нет на диске, API ставит задачу в ARQ и сразу отдаёт оригинал как fallback (`X-Thumb-Status: original-fallback`) — клиент не ждёт генерации.
...
- `./backend/app/api/photos/thumbnails.py` — fallback на оригинал (`X-Thumb-Status: original-fallback`) вместо "на лету" PIL генерации.
...
| `widget_mode` | `recent` | Режим выборки для виджета на главной: `recent` (последние добавленные) или `random` (случайные). |
...
| `max_share_ttl_days` | `365` | Верхняя граница TTL публичных share-ссылок (дней); превышение → `400` (enforcement в `sharing.py`). |
...
**Выключение — жёсткий kill-switch (PA-008, решение владельца 2026-09-02):** guard на родительском роутере (`app/api/photos/__init__.py`) закрывает весь `/api/v1/photos/*` до auth/ACL — 404 получают и авторизованные пользователи, и публичные share-ссылки. Уже розданные ссылки перестают работать до повторного включения модуля.
...
| `photos.photo_downloaded` | Скачивание оригинала (`?download=1`). | `GET /photos/thumbnail/{id}/{size}` |
