...
> **Ключевой код:** `.zcode/config.json` (workspace, в `.gitignore`),
> `~/.zcode/cli/config.json` (user), `scripts/mcp/` (wrapper-скрипты с секретами).
> **Связано:** таблица MCP-серверов в [`../AGENTS.md`](../AGENTS.md) §«Доступные инструменты (MCP)».
>
