...
- **standard** (по умолчанию) — профиль (own) и прочие страницы без плотных сеток.
- **`--reading`** — проза: KB-статья, новость-детейл, профиль (view-mode).
- **`--wide`** — плотные сетки/таблицы: Staff, Photos, Files, Admin; после расширения 1280→1600 также переведены списочные разделы и крупные формы: Home, News-list, KB-list, Links, NewsForm, KbArticleForm, Trash, MyFeedback, photos/MyShares.
