...
- **Скрытые пользователи**: параметр `include_hidden=True` в `GET /users` разрешен **только** для роли `admin`. Попытка вызова не-администратором возвращает **403 Forbidden**.
...
- **Параметры**:
  ...
  - `include_hidden`: загрузить ли скрытых пользователей (только для админов, только при `sort=staff_custom`).
...
- `./docs/user-attribute-mappings.md` — Документация модуля кастомных атрибутов пользователей
