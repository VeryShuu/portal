...
- Монтируется в глобальный layout безусловно (`<FeedbackModal />` в `./frontend/src/components/AppLayout.vue`); видимость плавающей кнопки вычисляется внутри компонента (`showButton`: пользователь авторизован И маршрут не в `HIDDEN_ROUTES` — публичные страницы авторизации).
...
| `./frontend/src/components/AppLayout.vue` | Безусловное монтирование `<FeedbackModal />` (видимость вычисляется внутри компонента) |
