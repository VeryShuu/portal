...
setup() {
    ...
    # download_bundle / fetch_latest_release_tag авторизуются через FORGEJO_TOKEN
    # (репо приватный, ADR-048). Даём тестовый токен по умолчанию; тесты, проверяющие
    # отсутствие токена, снимают его явно (unset FORGEJO_TOKEN).
    export FORGEJO_USER="test-ci"
...
@test "bundle_url формирует корректный URL для v1.1.0" {
    load_setup
    [[ "$(bundle_url v1.1.0)" == \
       "https://forgejo.mage.ru/mage/portal/releases/download/v1.1.0/portal-deploy-bundle-v1.1.0.tar.gz" ]]
}

@test "bundle_url для rc-тега v1.2.3-rc1" {
    load_setup
    [[ "$(bundle_url v1.2.3-rc1)" == \
       "https://forgejo.mage.ru/mage/portal/releases/download/v1.2.3-rc1/portal-deploy-bundle-v1.2.3-rc1.tar.gz" ]]
}
...
@test "apply_bundle повторно обновляет monitoring/ merge'ем, без вложенных каталогов" {
    load_setup
    # Regression (amendment-2 ADR-046): `cp -r src/monitoring ./monitoring` при
    # существующем ./monitoring клал копию ВНУТРЬ (./monitoring/monitoring/...),
    # т.е. со второго bundle monitoring фактически не обновлялся.
    make_bundle_tarball v1.1.0 "monitoring/prometheus.yml=V1"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.1.0.tar.gz" >/dev/null

    make_bundle_tarball v1.2.0 "monitoring/prometheus.yml=V2"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.2.0.tar.gz" >/dev/null

    [[ "$(cat monitoring/prometheus.yml)" == "V2" ]]   # реально обновился
    [[ ! -e monitoring/monitoring ]]                   # нет вложенного дубля
    # diff -r увидел изменение → .bak создан и содержит старую версию.
    [[ "$(cat monitoring.bak-pre-v1.2.0/prometheus.yml)" == "V1" ]]
    [[ ! -e monitoring.bak-pre-v1.2.0/monitoring ]]    # .bak не вложен сам в себя

    # Повторный apply ТОГО ЖЕ тега: старый .bak заменяется, а не вкладывается.
    make_bundle_tarball v1.2.0 "monitoring/prometheus.yml=V3"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.2.0.tar.gz" >/dev/null
    [[ "$(cat monitoring/prometheus.yml)" == "V3" ]]
    [[ "$(cat monitoring.bak-pre-v1.2.0/prometheus.yml)" == "V2" ]]
}

@test "apply_bundle копирует system_data/nginx/ (nginx.conf + entrypoint.sh)" {
    load_setup
    make_bundle_tarball v1.1.0 \
        "system_data/nginx/nginx.conf=NEW_CONF" \
        "system_data/nginx/entrypoint.sh=NEW_ENTRYPOINT"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.1.0.tar.gz"

    [[ "$(cat system_data/nginx/nginx.conf)" == "NEW_CONF" ]]
    [[ "$(cat system_data/nginx/entrypoint.sh)" == "NEW_ENTRYPOINT" ]]
}

@test "apply_bundle обновляет существующий system_data/nginx/ с .bak" {
    load_setup
    mkdir -p system_data/nginx
    printf 'OLD' > system_data/nginx/nginx.conf
    make_bundle_tarball v1.1.0 "system_data/nginx/nginx.conf=NEW"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.1.0.tar.gz"

    [[ "$(cat system_data/nginx/nginx.conf)" == "NEW" ]]
    [[ "$(cat system_data/nginx.bak-pre-v1.1.0/nginx.conf)" == "OLD" ]]
}

@test "apply_bundle НЕ трогает runtime-каталоги system_data (не входят в bundle)" {
    load_setup
    mkdir -p system_data/settings system_data/secrets system_data/nginx_reload
    printf '{"smtp":"runtime"}' > system_data/settings/system.json
    printf 'keep' > system_data/nginx_reload/reload-trigger
    make_bundle_tarball v1.1.0 \
        "docker-compose.yml=x" \
        "system_data/nginx/nginx.conf=NEW_CONF"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.1.0.tar.gz"

    [[ "$(cat system_data/settings/system.json)" == '{"smtp":"runtime"}' ]]
    [[ "$(cat system_data/nginx_reload/reload-trigger)" == "keep" ]]
    [[ ! -e system_data/settings.bak-pre-v1.1.0 ]]
    [[ ! -e system_data/nginx_reload.bak-pre-v1.1.0 ]]
}

@test "apply_bundle заменяет фантом-каталог на месте файла (bind-mount без файла)" {
    load_setup
    # Docker, не найдя bind-mount-файл, создаёт на его месте каталог — лечим заменой.
    mkdir -p system_data/nginx/nginx.conf
    make_bundle_tarball v1.1.0 "system_data/nginx/nginx.conf=REAL_FILE"
    apply_bundle "$TEST_CWD/portal-deploy-bundle-v1.1.0.tar.gz"

    [[ -f system_data/nginx/nginx.conf ]]
    [[ "$(cat system_data/nginx/nginx.conf)" == "REAL_FILE" ]]
}
...
# Запрос к Forgejo Releases API с авторизацией через FORGEJO_TOKEN (репо приватный,
# ADR-048). curl мокается через stub.

# Stub curl для Forgejo API: отдаёт JSON с заданным tag_name.
...
cat <<JSON
{
  "tag_name": "${tag}",
  "name": "Release ${tag}",
  "html_url": "https://forgejo.mage.ru/mage/portal/releases/tag/${tag}"
}
JSON
exit 0
EOF
...
@test "fetch_latest_release_tag: без FORGEJO_TOKEN → тихий skip (return 1, не фатально)" {
    load_setup
    unset FORGEJO_TOKEN
    run fetch_latest_release_tag
    [[ "$status" -ne 0 ]]
    [[ -z "$output" ]]   # никакого err-сообщения — bump-чек не критичен
}

# ─── forgejo_token / forgejo_user (ADR-048 auth) ──────────────────────────────

@test "forgejo_token: читает токен из env" {
    load_setup
    [[ "$(forgejo_token)" == "test-token" ]]
}

@test "forgejo_token: читает токен из .env если нет в env" {
    load_setup
    unset FORGEJO_TOKEN
    printf "FORGEJO_USER=ci-bot\nFORGEJO_TOKEN=from-dotenv\n" > .env
    [[ "$(forgejo_token)" == "from-dotenv" ]]
}

@test "forgejo_token: return 1 если токена нет ни в env, ни в .env" {
    load_setup
    unset FORGEJO_TOKEN
    run forgejo_token
    [[ "$status" -ne 0 ]]
...
@test "download_bundle: без FORGEJO_TOKEN → фатальная ошибка с подсказкой" {
    load_setup
    unset FORGEJO_TOKEN
    run download_bundle v1.1.0 "$TEST_CWD/dl"
    [[ "$status" -ne 0 ]]
    [[ "$output" == *"FORGEJO_TOKEN не задан"* ]]
}
