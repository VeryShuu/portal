...
[![CI](https://forgejo.mage.ru/mage/portal/actions/workflows/ci.yml/badge.svg)](https://forgejo.mage.ru/mage/portal/actions)
...
| Инфраструктура | Docker Compose, Nginx, Forgejo Actions |
...
> ⚠️ **Миграция GitHub → Forgejo в процессе (ADR-049).** Ниже — **target-state**
> (Forgejo Container Registry + Forgejo Release). Функциональный prod-переключатель
> (`setup.sh`) пока на GitHub и переводится отдельно — подробности в `docs/deploy.md`.

На проде **клон репозитория не нужен** — образы едут из Forgejo Container Registry
(ADR-045/049), а каркас запуска (compose, .env, setup.sh, monitoring-overlay)
приезжает одним tarball из Forgejo Release (ADR-046).

```bash
# 1. Скачать deploy-bundle (аттачится CI к каждому релизу v*):
curl -fL -H "Authorization: token $FORGEJO_TOKEN" \
  -o portal-deploy-bundle-v1.2.3.tar.gz \
  https://forgejo.mage.ru/mage/portal/releases/download/v1.2.3/portal-deploy-bundle-v1.2.3.tar.gz
...
```bash
git clone https://forgejo.mage.ru/mage/portal.git
cd portal
...
- **`IMAGE_PREFIX=forgejo.mage.ru/mage/`** (prod-контур) — pull готовых CI-образов: `docker compose pull && docker compose up -d`.
...
Доступные теги: [releases](https://forgejo.mage.ru/mage/portal/releases).
...
```bash
git clone https://forgejo.mage.ru/mage/portal.git portal-staging
cd portal-staging
bash setup.sh
...
- registry (`forgejo.mage.ru/mage/`) → `docker compose pull` готовых образов,
