...
- Работа с БД → [`docs/db-schema.md`](./docs/db-schema.md) (95 миграций, актуальная схема)
- Новый/изменённый API → [`docs/api-contracts.md`](./docs/api-contracts.md) (319 путей / 401 операция)
- Права доступа → [`docs/roles-matrix.md`](./docs/roles-matrix.md)
- Спорное решение → [`docs/adr.md`](./docs/adr.md) (ADR-001–052)
- **Модульная задача → модульный док `docs/<module>.md`** (helpdesk/files/photos/kb/news/meetings/matrix/… — полный список в docs/README.md). Не начинай модуль без чтения его дока.
...
- При крупных изменениях — пересборка контейнеров без кэша (`docker compose build --no-cache <service>`). На проде образы не собираются локально — пушятся в Container Registry через CI и тянутся `docker compose pull` (ADR-045/049); локальная сборка — только dev/staging.
- **Prod-контур не требует клон репозитория** (ADR-046): образы из Container Registry + deploy-bundle (`docker-compose.yml` + `.env.example` + `setup.sh` + `monitoring/` + `system_data/nginx/` — статичный nginx-конфиг, bind-mount с хоста) из Release. `setup.sh` качает bundle и логинится в registry через `FORGEJO_USER`/`FORGEJO_TOKEN` из `.env` (PAT, репо приватный — ADR-049). Профиль контура фиксируется в `.portal-profile` (`prod`/`dev`, default `dev`); `setup.sh` гейтит пункты меню и блокирует local-build в prod-контуре. `init.sql` запечён в postgres-образ (`/docker-entrypoint-initdb.d/01-init.sql`), bind-mount убран.
...
Конфигурация: **workspace** серверы — в `.zcode/config.json` (auto-connect при открытии проекта; коммитится — секретов нет, только абсолютные пути к wrapper'ам); **user** серверы — в `~/.zcode/cli/config.json`. Секреты **никогда** в конфиг-файлах — только через env / wrapper-скрипты.
...
| **grafana** | workspace | Логи Loki, метрики Prometheus, дашборды/алерты Grafana (Viewer, read-only) | Разбор инцидентов: всплески 401/429/5xx в логах, проверка алертов. Требует поднятого monitoring-оверлея (`setup.sh` → п.10, ADR-044); токен `GRAFANA_MCP_TOKEN` из `.env` |
| ~~`github`~~ | — | **Убран (2026-08):** GitHub заблокирован, репо переехал на Forgejo | Операции с Git-хостингом (PR, релизы, статусы CI) — через **REST API Forgejo** (`https://forgejo.mage.ru/api/v1/...`) с админ-токеном из env `FORGEJO_TOKEN`. Примеры — `docs/mcp-setup.md` §«Работа с Forgejo через API» |
...
- После ручного обновления ruff/mypy/radon (см. ниже) → сразу `--recreate` + прогнать +
...
> правила → нужен отдельный PR с правкой кода. **Dependabot удалён** (GitHub-only, репо
> переехал на Forgejo — см. ADR-048), поэтому бампы версий ruff/mypy/radon/node-зависимостей
> теперь **ручные**: разработчик (или агент) поднимает версию в `pyproject.toml`/`package.json`,
> прогоняет `ci_lint.sh --recreate` локально и чинит новые ошибки. Future-опция — поднять
> Renovate через Forgejo Actions (отдельной задачей).
...
| Typecheck юнит-тестов (ratchet) | `npm run typecheck:tests` |
...
3. В зависимости от задачи дополнительно читай:
   - Работа с БД → `docs/db-schema.md` (95 миграций; перед новой — проверь следующий свободный номер)
   ...
   - **Пришёл алерт — что делать** → `docs/runbooks.md` (14 групп алертов + чек-лист владельца; ADR-053)
   - Спорное архитектурное решение → `docs/adr.md` (ADR-001–052)
...
> Ветка `main` защищена (Forgejo: **Settings → Branches → `main`**): прямой
> `git push origin main` невозможен, весь код идёт через PR + обязательные status
> checks. Это формализует принцип «коммитит и пушит — только пользователь» — git-сервер
> физически не даст агенту обойти ревью. ⚠️ После переезда с GitHub (ADR-048) branch
> protection нужно **(пере)настроить на Forgejo** — required checks добавляются после
> первого зелёного прогона (список ниже).
...
- **Процесс (агент):**
  ...
  # PR через Forgejo API (gh CLI недоступен — GitHub заблокирован, репо на Forgejo):
  ...
  # статус чеков — /repos/mage/portal/actions/runs (или UI forgejo.mage.ru/mage/portal/actions)
  ```
- **Процесс (пользователь):** видит в Forgejo UI (`forgejo.mage.ru/mage/portal`)
  `🟢 All checks passed` → нажимает **Merge**. Если `🔴 failed` — не мёрджит, агент
  чинит и пушит в **ту же ветку** (`git push`, без нового PR — CI перезапускается
  автоматически).
- **20 обязательных чеков** (без зелёного мёрдж заблокирован; список = branch protection,
  аудит 2026-08-23, drift-слияние 2026-09-07): `backend / ruff + mypy`,
  `backend / pytest unit + security`, `backend / pytest integration`, `frontend / eslint + tsc + i18n + build`,
  `frontend / vitest`, `frontend / playwright e2e` (production-preview + гейты pageerror/console/skip),
  `compose / up + healthcheck smoke`,
  `drift / openapi + types.gen + db-schema` (бывшие `openapi / drift check` + `frontend / types_gen drift check` +
  `docs / db_schema drift check`), `docs / tests.generated.md drift check`,
  ...
  `monitoring / config validation`, `backend / authed_client_factory allowlist (fake_db)`,
  `security / deps / pip-audit + npm audit`, `security / trivy / filesystem scan`,
  `coverage / diff-coverage gate (≥80% new code)` — новый/изменённый код в PR обязан быть покрыт ≥80% (см. ниже «Diff-coverage гейт»).
  `apply_to_admins=true` — правила защиты распространяются и на админа (аудит 2026-08-23: прежде
  админ-токен обходил гейты). Информационный (не блокирует): `backend / merged coverage`.
...
- **Единый drift-скрипт (предпочитать):** `./scripts/check-drift.sh` гоняет все три
  генерации + diff одной командой — до пуша, чтобы не ловить 3 из 17 обязательных
...
- Хранится **в репозитории (под git)**: `docs/wip/<feature>.md`. Виден следующей сессии и переносится через Forgejo.
...
КОММИТ: <готовый workflow для пользователя: имя ветки + git add/commit/push + создание PR через Forgejo API + текст сообщения>
```
...
**Infra:** PostgreSQL 16 · Redis 7 · Nginx · Docker Compose · Forgejo Actions (self-hosted runner, ADR-048) · **Keycloak** (IdP) · **Nextcloud** (files) · **Collabora Online** (editor) · Postfix (SMTP)

**Observability** (опциональный overlay `monitoring/`, не в базовом compose): **Grafana** (UI) · **Loki** (логи) · **Prometheus** (метрики) · **Alloy** (сборщик Docker-логов) · **Alertmanager** (email-алерты). Запуск: `setup.sh` → пункт 10. Подробности: `docs/monitoring.md` §7-9, ADR-044. Новый алерт = правило + runbook-секция (`docs/runbooks.md`) + semantic promtool-сценарий; дашборды валидируются CI (`scripts/validate-grafana.py`); см. ADR-053.
...
- **Перед коммитом** (backend): `./scripts/ci_lint.sh` (ruff+mypy в точном CI-окружении, см. §«CI-эквивалент локально») + `pytest tests/unit`. Не полагайся на локальные ruff/mypy — их версии могут расходиться с CI (урок 2026-07-24). Frontend: `npm run lint:check && npm run typecheck && npm run typecheck:tests && npm run test:unit && npm run i18n:check`. **Новый код в PR — покрыть ≥80%**: локально `./scripts/diff-cover.sh backend` / `frontend` (см. §«Работа между сессиями» → «Diff-coverage гейт»).
...
> Полный разбор: `./docs/helpdesk.md`. Миграции `075`–`095`.
...
### Корпоративный чат (Matrix)
> Полный разбор: `./docs/matrix.md`.

- Персональные уведомления в Matrix (Synapse+MAS) через общий `messenger_outbox` (provider `'matrix'`, миграция 097). Бот `@portal-bot:matrix.mage.ru` **без пароля**, auth — long-lived токен `mct_...` (`mas-cli manage issue-compatibility-token`, пишет сессию в БД MAS напрямую — password-login не нужен).
- **Gotcha:** MXID = конвенция из email (`borzihin.vs@mage.ru` → `@borzihin.vs:matrix.mage.ru`), localpart lowercase; opt-in через `users.preferences.chat_notifications_enabled` (дефолт выключен; сам сотрудник или админ в его профиле).
- **Gotcha:** отправка идемпотентна (txnId = UUID outbox-строки); `m.notice` не использовать; `chat_id` matrix-строк хранит MXID, не room_id.

### Обучение (LMS)
> Полный разбор: `./docs/learning.md` (модульный док), `./docs/wip/learning.md` (полное ТЗ + журнал), ADR-051/052.

- Два контура: портал (сотрудники, SSO) + публичный `learn.<домен>` (внешние учётки `learning_accounts`, отдельная learn-сборка `dist-learn`). Роль модуля — методист (`learning_admins`, паттерн `helpdesk_agents`).
- **Gotcha (гранты DB-роли):** learning-роуты работают под ролью `learning_app` (env `LEARNING_DB_PASSWORD`, обязателен на проде). Гранты миграции 102 табличные — **каждая новая `learning_*`-таблица GRANT-ит роль в своей миграции** (инцидент 109→112: категории с SELECT-only падали 500 на create).
- **Gotcha (письма):** строители `services/learning/emails.py` обязаны возвращать непустой `(subject, text, html)` — пустой html в multipart/alternative = пустое письмо в Outlook (прод-кейс 2026-09-03).
- Замок теста §15 (правка только копией), publish-gate, ленивые abandoned, for_all_staff (виртуальный доступ + материализация строк), сертификаты лениво через screenshot-service.

### Согласование документов (1С ERP)
> Полный разбор: `./docs/approvals.md` (модульный док). Миграции 115 (настройки), 116 (token_base_url).

- Прокси к HTTP-сервисам 1С: список/карточка/согласование/вложения; данные в БД портала не хранятся. Вход — SSO, токен сотрудника = `GETTokenByLogin?Login=<email в lower>` под учёткой Portal (адреса в `approvals_settings`: `base_url` — документы `…/hs/Auth`, `token_base_url` — токены `…/hs/PortalAuth`).
- **Gotcha:** владение документом = наличие в списке пользователя (чужой GUID → 404 ещё на портале); массовое согласование — строго последовательно с паузой; `requires_manager` исключён из bulk.
- **Gotcha (FastAPI 0.140):** `Depends(<Annotated-алиас>)` в router-level dependencies даёт фантомные query args/kwargs (всё 422) — только сырая функция; `GET /approvals/settings` включать РАНЬШЕ `/approvals/{document_uuid}` (иначе перекрыт). Регресс-тесты `TestRouting`.
- 1С отдаёт JSON-ошибки без Content-Type — эвристика контракт/инфра по телу. Несопоставленный логин → 404 (мягко).
- Настройки/подключение — Admin UI вкладка «Согласование 1С» (пароль Fernet, write-only); сверка логинов — `scripts/export-portal-logins.sh`.
...
│   ├── adr.md / adr-archive.md          ← ADR (активные 001–052 / архивные)
...
│   ├── runbooks.md           ← что делать при тревогах (по группам алертов)
│   ├── mcp-setup.md           ← настройка MCP-серверов ZCode на новой машине
│   ├── <module>.md            ← модульные доки: helpdesk, files, photos, kb, news, meetings, matrix, directories, ...
...
│   ├── api/                   ← типизированные API-клиенты (+ types.gen.d.ts — auto-gen из openapi.json, КОММИТИТСЯ и проверяется drift-гейтом ci.yml)
...
- **Образ backend вкомпилирован** (target `production`): volume-mount только для `/data/*`. После правок backend-кода в **dev** — `docker compose build backend`, иначе `restart` не подхватит изменения. На **проде** образы не собираются локально — CI пушит в Forgejo Container Registry, прод тянет `docker compose pull` (ADR-045/049). Это же касается сертификатов в `backend/certs/` — добавление/обновление `russian_trusted_root_ca.crt` требует пересборки образа (на проде — следующий CI-билд + pull).
